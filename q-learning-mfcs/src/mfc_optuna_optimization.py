...
class MFCOptunaOptimizer:
    ...
    def setup_logging(self) -> None:
        ...
        log_file = (
            self.results_dir
            / f"optuna_optimization_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def define_search_space(self, trial) -> dict:
        ...
        params["biofilm_base_reward"] = trial.suggest_float(
            "biofilm_base_reward",
            20.0,
            60.0,
        )
        params["biofilm_steady_bonus"] = trial.suggest_float(
            "biofilm_steady_bonus",
            10.0,
            40.0,
        )
        params["biofilm_penalty_multiplier"] = trial.suggest_float(
            "biofilm_penalty_multiplier",
            50.0,
            100.0,
        )

        # Power reward parameters
        params["power_increase_multiplier"] = trial.suggest_float(
            "power_increase_multiplier",
            30.0,
            80.0,
        )
        params["power_decrease_multiplier"] = trial.suggest_float(
            "power_decrease_multiplier",
            60.0,
            150.0,
        )

        # Substrate reward parameters
        params["substrate_increase_multiplier"] = trial.suggest_float(
            "substrate_increase_multiplier",
            20.0,
            50.0,
        )
        params["substrate_decrease_multiplier"] = trial.suggest_float(
            "substrate_decrease_multiplier",
            40.0,
            100.0,
        )

        # Concentration control parameters
        params["conc_precise_reward"] = trial.suggest_float(
            "conc_precise_reward",
            15.0,
            30.0,
        )
        params["conc_acceptable_reward"] = trial.suggest_float(
            "conc_acceptable_reward",
            3.0,
            8.0,
        )
        params["conc_poor_penalty"] = trial.suggest_float(
            "conc_poor_penalty",
            -15.0,
            -5.0,
        )
        ...
        params["flow_penalty_threshold"] = trial.suggest_float(
            "flow_penalty_threshold",
            15.0,
            25.0,
        )
        params["flow_penalty_multiplier"] = trial.suggest_float(
            "flow_penalty_multiplier",
            15.0,
            35.0,
        )
        params["biofilm_threshold_ratio"] = trial.suggest_float(
            "biofilm_threshold_ratio",
            0.85,
            0.95,
        )
        ...
        params["max_substrate_decrease"] = trial.suggest_float(
            "max_substrate_decrease",
            -3.0,
            -1.0,
        )
        params["max_substrate_increase"] = trial.suggest_float(
            "max_substrate_increase",
            1.0,
            2.5,
        )
        params["substrate_increment_fineness"] = trial.suggest_categorical(
            "substrate_increment_fineness",
            ["coarse", "medium", "fine"],
        )
    ...
    def create_modified_simulation(self, params: dict) -> MFCUnifiedQLearningSimulation:
        ...
        def optimized_reward_function(
            ...
            error_improvement = abs(prev_outlet_conc - self.target_outlet_conc) - abs(
                outlet_conc - self.target_outlet_conc,
            )
            ...
            if substrate_change > 0:
                substrate_reward = (
                    substrate_change * params["substrate_increase_multiplier"]
                )
            else:
                substrate_reward = (
                    substrate_change * params["substrate_decrease_multiplier"]
                )
            ...
            if biofilm_deviation <= deviation_threshold:
                biofilm_reward = (
                    params["biofilm_base_reward"]
                    - (biofilm_deviation / deviation_threshold) * 15.0
                )
                ...
                biofilm_reward = -params["biofilm_penalty_multiplier"] * (
                    excess_deviation / deviation_threshold
                )
            ...
            if outlet_error <= 0.5:
                concentration_reward = params["conc_precise_reward"] - (
                    outlet_error * 10.0
                )
            elif outlet_error <= 2.0:
                concentration_reward = params["conc_acceptable_reward"] - (
                    outlet_error * 2.5
                )
            else:
                concentration_reward = params["conc_poor_penalty"] - (
                    outlet_error * 5.0
                )
            ...
            current_flow_rate = getattr(
                sim.unified_controller,
                "current_flow_rate",
                10.0,
            )
            if (
                biofilm_thickness_history is not None
                and len(biofilm_thickness_history) > 0
            ):
                ...
                if avg_biofilm < optimal_thickness * params["biofilm_threshold_ratio"]:
                    if current_flow_rate > params["flow_penalty_threshold"]:
                        flow_penalty = (
                            -params["flow_penalty_multiplier"]
                            * (current_flow_rate - params["flow_penalty_threshold"])
                            / 10.0
                        )
    ...
    def update_action_space(self, sim, params: dict) -> None:
        """Update action space based on optimized parameters."""
        # Create optimized flow actions
        flow_actions = list(
            range(params["max_flow_decrease"], params["max_flow_increase"] + 1),
        )
        ...
        self.logger.info(
            f"Updated action space: {len(flow_actions)} flow × {len(substrate_actions)} substrate = {len(sim.unified_controller.actions)} total actions",
        )
    ...
    def objective(self, trial) -> float:
        ...
        try:
            ...
            self.logger.info(
                f"Trial {trial.number}: Starting simulation with {len(sim.unified_controller.actions)} actions",
            )
            ...
            control_objective = (
                control_rmse * 0.4
            )  # 40% weight on concentration control
            biofilm_objective = (
                biofilm_error * 20.0 * 0.3
            )  # 30% weight on biofilm control
            energy_objective = (
                10.0 - energy_total
            ) * 0.2  # 20% weight on energy (minimize negative)
            stability_objective = (
                abs(q_reward / 1e6) * 0.1
            )  # 10% weight on learning stability
            ...
            self.logger.info(
                f"  Energy: {energy_total:.3f} Wh, Power: {final_power:.4f} W",
            )
            self.logger.info(
                f"  Control RMSE: {control_rmse:.3f}, MAE: {control_mae:.3f}",
            )
            self.logger.info(
                f"  Biofilm: {avg_final_biofilm:.3f} (target: {self.target_biofilm})",
            )
            ...
            self.logger.exception(f"Trial {trial.number} failed: {e!s}")
    ...
    def run_optimization(self) -> optuna.Study:
        ...
        """
        self.logger.info(
            f"Starting MFC Q-Learning optimization with {self.n_trials} trials",
        )
        self.logger.info(f"Target biofilm thickness: {self.target_biofilm}")
        self.logger.info(
            f"Target outlet concentration: {self.target_outlet_conc} mmol/L",
        )
        ...
        try:
            if self.n_jobs > 1:
                self.logger.info(
                    f"Running optimization with {self.n_jobs} parallel jobs",
                )
                study.optimize(
                    self.objective,
    ...
    def print_optimization_summary(self, study: optuna.Study) -> None:
        """Print comprehensive optimization summary."""
        for _key, _value in study.best_params.items():
            pass
    ...
    def get_top_configurations(
        ...
        for i, trial in enumerate(sorted_trials[:n_configs]):
            ...
            self.logger.info(
                f"Top config {i + 1}: Trial {trial.number}, Objective: {trial.value:.6f}",
            )
    ...
    def run_extended_validation(self, configs: list[dict]) -> list[dict]:
        ...
        """
        self.logger.info(
            f"Starting extended validation of {len(configs)} configurations",
        )
        self.logger.info(
            "Each configuration will run for 600 hours (vs 120h during optimization)",
        )
        ...
        if len(configs) <= 14:  # Run all in parallel if we have enough threads
            ...
            with concurrent.futures.ProcessPoolExecutor(
                max_workers=min(14, len(configs)),
            ) as executor:
                # Submit all validation jobs
                future_to_config = {}
                for i, config in enumerate(configs):
                    future = executor.submit(
                        self._run_single_extended_validation,
                        config,
                        i + 1,
                    )
                ...
                for future in concurrent.futures.as_completed(future_to_config):
                    ...
                    try:
                        result = future.result(
                            timeout=3600,
                        )  # 1 hour timeout per validation
                        validation_results.append(result)
                        self.logger.info(
                            f"Validation completed for config {config['trial_number']}",
                        )
                    except Exception as e:
                        self.logger.exception(
                            f"Validation failed for config {config['trial_number']}: {e!s}",
                        )
                        # Add failed result
    ...
    def _run_single_extended_validation(self, config: dict, config_num: int) -> dict:
        ...
        """
        self.logger.info(
            f"Starting extended validation {config_num}/14 (Trial {config['trial_number']})",
        )
        ...
        try:
            ...
            array_func = (
                np.zeros if not sim.use_gpu else (lambda shape: np.zeros(shape))
            )
            ...
            energy_objective = (
                15.0 - energy_total
            ) * 0.2  # Higher energy target for 600h
            ...
            validation_result = {
                ...
                "improvement_vs_short": {
                    "energy_ratio": energy_total
                    / config["original_metrics"].get("energy_total", 1),
                    "rmse_ratio": control_rmse
                    / config["original_metrics"].get("control_rmse", 100),
                    "biofilm_ratio": biofilm_error
                    / config["original_metrics"].get("biofilm_error", 1),
                },
            }

            self.logger.info(
                f"Extended validation {config_num} completed: Obj={extended_objective:.6f}, "
            ...
            self.logger.exception(f"Extended validation {config_num} failed: {e!s}")
            raise

    def _apply_parameters_to_simulation(self, sim, params: dict) -> None:
    ...
    def _save_validation_results(self, validation_results: list[dict]) -> None:
    ...
    def _print_validation_summary(self, validation_results: list[dict]) -> None:
        ...
        if successful_results:
            ...
            best["config"]
            best["extended_metrics"]

            for _i, result in enumerate(successful_results[:3]):
                result["config"]
