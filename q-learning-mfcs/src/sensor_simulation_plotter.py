...
try:
    ...
    def get_simulation_data_path(filename) -> str:
        return f"data/simulation_data/{filename}"
...
class SensorMFCPlotter:
    ...
    def __init__(self, timestamp: str | None = None) -> None:
        ...
        self.cell_colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]
    ...
    def _plot_power_evolution(self, ax, data) -> None:
        ...
        if len(time) == 0 or len(power) == 0:
            ...
            power = (
                0.001
                + 0.0005 * np.sin(time * 0.1)
                + 0.0002 * np.random.randn(len(time))
            )
        ...
        ax.annotate(
            f"Peak: {max_power:.2f} mW",
            xy=(0.7, 0.8),
            xycoords="axes fraction",
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "lightblue", "alpha": 0.7},
        )

    def _plot_sensor_status_dashboard(self, ax, data) -> None:
    ...
    def _plot_biofilm_sensor_validation(self, ax, data) -> None:
    ...
    def _plot_current_voltage_eis(self, ax, data) -> None:
    ...
    def _plot_qcm_mass_monitoring(self, ax, data) -> None:
    ...
    def _plot_sensor_fusion_performance(self, ax, data) -> None:
        """Plot sensor fusion performance metrics."""
        time = np.array(data.get("time_hours", []))

        # Simulated fusion metrics
        fusion_confidence = (
            0.8 + 0.1 * np.sin(time * 0.05) + 0.05 * np.random.randn(len(time))
        )
        agreement_score = (
            0.9 + 0.05 * np.cos(time * 0.03) + 0.02 * np.random.randn(len(time))
        )
    ...
    def _plot_system_performance_heatmap(self, ax, data) -> None:
    ...
    def _plot_eis_impedance_evolution(self, ax, data) -> None:
    ...
    def _plot_qcm_frequency_response(self, ax, data) -> None:
    ...
    def _plot_sensor_fusion_confidence(self, ax, data) -> None:
    ...
    def _plot_biofilm_growth_validation(self, ax, data) -> None:
    ...
    def _estimate_thickness_from_eis(self, time, true_thickness):
        ...
        for i, (t, thickness) in enumerate(zip(time, true_thickness, strict=False)):
            if t <= 20:  # Lag phase: EIS overestimates due to electrode surface effects
                ...
                t <= 60
            ):  # Growth phase: EIS underestimates due to biofilm heterogeneity
    ...
    def _estimate_thickness_from_qcm(self, time, true_thickness):
        ...
        for i, (t, thickness) in enumerate(zip(time, true_thickness, strict=False)):
            if (
                t <= 20
            ):  # Lag phase: QCM slightly underestimates due to low cell density
    ...
    def _plot_substrate_concentration_monitoring(self, ax, data) -> None:
        ...
        if len(substrate_conc) == 0:
            ...
            acetate_conc = self._generate_acetate_accumulation_profile(
                time,
                substrate_conc,
            )
        ...
        if addition_times:
            for add_time in addition_times:
                ...
                ax.annotate(
                    "Substrate\nAddition",
                    xy=(add_time, max(substrate_conc) * 0.9),
                    xytext=(10, 10),
        ...
        ax.set_title(
            "Mixed Culture Syntrophy - Lactate→Acetate (Shewanella) → Geobacter Consumption",
        )
    ...
    def _generate_substrate_profile_with_additions(self, time):
        ...
        for i, _t in enumerate(time):
            ...
            max_consumption_rate = (
                0.8  # mM/h (increased from normal due to high cell density)
            )
            km = 2.0  # Half-saturation constant (mM)
            consumption_rate = (max_consumption_rate * current_conc) / (
                km + current_conc
            )
    ...
    def _generate_acetate_accumulation_profile(self, time, lactate_conc):
        ...
        for i, t in enumerate(time):
            ...
            if i > 0:  # Start consuming acetate after some accumulation
                ...
                consumption_rate = (
                    vmax_acetate * current_acetate * geobacter_fraction
                ) / (km_acetate + current_acetate)
                ...
                acetate_conc[i] = max(
                    0,
                    acetate_produced
                    - min(acetate_consumed * (t / dt), acetate_produced),
                )
            else:
    ...
    def _plot_mass_transfer_correlation(self, ax, data) -> None:
        ...
        if len(time) == 0 or len(substrate_conc) == 0 or len(current) == 0:
            ...
            current = (
                0.001
                + 0.00005 * substrate_conc
                + 0.00001 * np.random.randn(len(substrate_conc))
            )
        ...
        scatter = ax.scatter(
            ...
            c=(
                time
                if len(time) == len(substrate_conc)
                else np.linspace(0, 100, len(substrate_conc))
            ),
            cmap="viridis",
            alpha=0.7,
            s=50,
        ...
        if len(substrate_conc) > 2:
            ...
            ax.plot(x_fit, p(x_fit), "r--", linewidth=2, label=f"k_La = {k_La:.2f} h⁻¹")
    ...
    def _plot_sensor_quality_metrics(self, ax, data) -> None:
    ...
    def _plot_multicell_sensor_comparison(self, ax, data) -> None:
        ...
        for i in range(n_cells):
            ...
            eis_impedance = (
                100
                + 20 * np.exp(-time / 50) * amplitude_factor
            ...
            qcm_freq = -np.cumsum(
                np.random.exponential(0.4 * amplitude_factor, len(time)),
            )

            cell_data[f"Cell {i + 1}"] = {"eis": eis_impedance, "qcm": qcm_freq}
    ...
    def _plot_key_performance_indicators(self, ax, data) -> None:
    ...
    def _plot_efficiency_metrics(self, ax, data) -> None:
        ...
        if len(substrate_data) > 0 and len(time) > 0:
            ...
            if len(substrate) == len(time):
                ...
                utilization = np.maximum(
                    0,
                    (initial_substrate - substrate) / initial_substrate * 100,
                )
    ...
    def _plot_power_distribution(self, ax, data) -> None:
    ...
    def _plot_cell_performance_comparison(self, ax, data) -> None:
    ...
    def _plot_final_system_state(self, ax, data) -> None:
        ...
        table = ax.table(
            cellText=list(
                zip(final_metrics["Metric"], final_metrics["Value"], strict=False),
            ),
    ...
    def _add_simulation_info_box(self, fig, data) -> None:
...
• Sensor Fusion Accuracy: {data.get("fusion_accuracy", 0):.1f}%"""
    ...
    def save_simulation_data(self, data: dict[str, Any]) -> tuple[str, str]:
        ...
        max_length = (
            max(len(v) for v in time_series_data.values()) if time_series_data else 0
        )
