...
class CathodeParameters:
    ...
    oxygen_concentration: float = (
        8.0e-3  # Dissolved oxygen in mol/L (air-saturated water)
    )
...
class BaseCathodeModel(ABC):
    ...
    def __init__(self, parameters: CathodeParameters) -> None:
    ...
    def calculate_equilibrium_potential(
        ...
        RT_over_4F = (
            ...
            * self.temperature_K
            / (4.0 * self.params.faraday_constant)
        )
        RT_over_F = (
            self.params.gas_constant * self.temperature_K / self.params.faraday_constant
        )
        ph_correction = (
            -RT_over_F * jnp.log(10.0) * ph
        )  # Temperature-dependent pH correction
        concentration_term = RT_over_4F * jnp.log(
            oxygen_conc / self.params.oxygen_reference_conc,
        )
    ...
    def calculate_power_loss(
        ...
        return abs(overpotential * current)

    def calculate_oxygen_consumption_rate(
        self,
        overpotential: float,
        oxygen_conc: float | None = None,
    ) -> float:
        ...
        n_electrons = 4.0
        return current / (n_electrons * self.params.faraday_constant)

    def update_temperature(self, new_temperature_K: float) -> None:
    ...
    def update_area(self, new_area_m2: float) -> None:
        """Update cathode area."""
...
class ButlerVolmerKinetics:
    ...
    def calculate_current_density(
        ...
        one_minus_alpha_F_eta_RT = (
            (1.0 - transfer_coefficient) * F * overpotential / (R * temperature_K)
        )
    ...
    def calculate_tafel_current(
        ...
        return exchange_current_density * concentration_ratio * (10.0**decade_factor)
