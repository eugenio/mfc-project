...
class PlatinumParameters:
    ...
    exchange_current_density_ref: float = (
        3.0e-5  # A/m² (3.0e-9 A/cm²) - Khan et al. 2024
    )
...
class PlatinumCathodeModel(BaseCathodeModel):
    ...
    def __init__(
        ...
        platinum_params: PlatinumParameters | None = None,
    ) -> None:
    ...
    def _setup_kinetic_parameters(self) -> None:
        ...
        surface_area_factor = (
            self.pt_params.platinum_surface_area / 50.0
        )  # Normalized to 50 m²/g
        loading_factor = (
            self.pt_params.platinum_loading / 0.5
        )  # Normalized to 0.5 mg/cm²

        self.exchange_current_density = (
            i0_base
            * utilization_factor
            * jnp.sqrt(surface_area_factor * loading_factor)
        )

        # Temperature-corrected diffusion coefficient
        # D(T) = D_ref * (T/T_ref)^1.5 * (μ_ref/μ(T))
        temp_ratio = T / T_ref
        self.oxygen_diffusion_coeff = self.pt_params.oxygen_diffusion_coeff * (
            temp_ratio**1.5
        )
    ...
    def _setup_mass_transport_parameters(self) -> None:
        ...
        self.limiting_current_density = (
            1.0 / (1.0 / self.limiting_current_density + 1.0 / catalyst_layer_limiting)
        ) ** -1
    ...
    def calculate_current_density(
        ...
        boundary_layer_limiting = (
            n_electrons * F * self.pt_params.oxygen_mass_transfer_coeff * oxygen_conc
        )
        ...
        i_limiting = (
            1.0 / (1.0 / boundary_layer_limiting + 1.0 / catalyst_layer_limiting)
        ) ** -1
    ...
    def calculate_performance_metrics(
        ...
        mass_transport_utilization = (
            current_density / self.limiting_current_density * 100
        )
    ...
    def estimate_cost_per_area(self) -> float:
        ...
        # Material costs
        pt_loading_kg_m2 = (
            self.pt_params.platinum_loading * 1e-6 * 1e4
        )  # mg/cm² to kg/m²
        material_cost_per_m2 = pt_loading_kg_m2 * self.pt_params.platinum_price_per_kg

        # Total cost including manufacturing
        return material_cost_per_m2 * self.pt_params.manufacturing_cost_factor
    ...
    def estimate_cost_analysis(self) -> dict[str, float]:
        ...
        # Total cost including manufacturing
        total_cost_per_m2 = (
            material_cost_per_m2 * self.pt_params.manufacturing_cost_factor
        )

        # Cost per unit power at reference conditions
        ref_metrics = self.calculate_performance_metrics(0.2)  # 200 mV overpotential
        cost_per_kW = total_cost_per_m2 / (
            ref_metrics["power_density_mW_m2"] * 1e-6
        )  # $/kW
    ...
    def compare_to_benchmark(self) -> dict[str, Any]:
        ...
        return {
            "performance_vs_benchmark": {
                "power_density_ratio": metrics["power_density_mW_m2"]
                / benchmark_power_density,
                "exchange_current_ratio": metrics["exchange_current_density_A_m2"]
                / benchmark_exchange_current,
    ...
    def get_all_parameters(self) -> dict[str, Any]:
        ...
        return {
            "cathode_parameters": {
                ...
                "oxygen_concentration_mg_L": self.params.oxygen_concentration
                * 32.0
                * 1000,
                "ph": self.params.ph,
            },
            "platinum_kinetic_parameters": {
                ...
                "exchange_current_density_actual_A_m2": float(
                    self.exchange_current_density,
                ),
                ...
                "overpotential_transition_mV": self.pt_params.overpotential_transition
                * 1000,
            ...
            "catalyst_parameters": {
                ...
                "catalyst_layer_thickness_um": self.pt_params.catalyst_layer_thickness
                * 1e6,
            },
            "mass_transport_parameters": {
                "oxygen_diffusion_coeff_m2_s": float(self.oxygen_diffusion_coeff),
                "boundary_layer_thickness_um": self.pt_params.boundary_layer_thickness
                * 1e6,
...
def create_platinum_cathode(
    area_cm2: float = 1.0,
    temperature_C: float = 25.0,
    oxygen_mg_L: float = 8.0,
    platinum_loading_mg_cm2: float = 0.5,
    custom_pt_params: dict[str, float] | None = None,
