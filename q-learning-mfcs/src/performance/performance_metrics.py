...
def start_metrics_collection() -> None:
    """Start metrics collection using the default collector."""
    collector = get_default_collector()
    collector.start_collection()


def stop_metrics_collection() -> None:
    """Stop metrics collection using the default collector."""
    collector = get_default_collector()
    collector.stop_collection()


def record_metric(name: str, value: float, timestamp: datetime | None = None,
                 labels: dict[str, str] | None = None,
                 metric_type: MetricType = MetricType.GAUGE) -> None:
    """Record a metric using the default collector."""
    collector = get_default_collector()
    collector.record_metric(name, value, timestamp, labels, metric_type)
