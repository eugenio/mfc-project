...
class MembraneTransport:
    ...
    def _load_membrane_properties(self) -> MembraneProperties:
        ...
        if self.membrane_grade not in membrane_db:
            ...
            msg = (
                f"Membrane grade '{self.membrane_grade}' not available. "
    ...
    def calculate_oxygen_crossover(
        self,
        anode_o2_conc: float,
        ...
        temp_factor = np.exp(
            self.properties.permeability_temp_coeff
        ...
        return permeability * conc_gradient

    def calculate_proton_conductivity(
        self,
        temperature: float = 303.0,
        ...
        temp_correction = 1 + self.properties.conductivity_temp_coeff * (
            temperature - self.T_ref
        )

        # Humidity correction (empirical relationship)
        humidity_factor = (
            relative_humidity / self.RH_ref
        ) ** self.properties.conductivity_humidity_exp

        # Base conductivity with corrections
        conductivity = (
            self.properties.proton_conductivity * temp_correction * humidity_factor
        )
    ...
    def calculate_membrane_resistance(
        ...
        """
        conductivity = self.calculate_proton_conductivity(
            temperature,
            relative_humidity,
        )
        ...
        return thickness_m / (conductivity_si * area)
    ...
    def calculate_water_transport(
        ...
        water_uptake = self.properties.water_uptake * (
            1
            + self.properties.water_uptake_humidity_slope
            * (relative_humidity - self.RH_ref)
            / 100
        )
        ...
        return drag_coefficient * proton_flux

    def calculate_potential_drop(
        self,
        current_density: float,
        area: float,
        temperature: float = 303.0,
        ...
        """
        resistance = self.calculate_membrane_resistance(
            area,
            temperature,
            relative_humidity,
        )
    ...
    def calculate_oxygen_consumption_loss(
        self,
        oxygen_flux: float,
        area: float,
        substrate_flux: float,
    ) -> float:
    ...
    def optimize_membrane_thickness(
        self,
        ...
        return optimal_thickness_m * 1e6

    def calculate_crossover_current_loss(
        self,
        oxygen_crossover: float,
        area: float,
    ) -> float:
        ...
        return electrons_consumed * self.F  # A
