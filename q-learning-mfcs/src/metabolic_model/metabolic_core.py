...
class MetabolicModel:
    ...
    def _map_species_substrate(self) -> None:
        ...
        substrate_map = {"acetate": Substrate.ACETATE, "lactate": Substrate.LACTATE}
    ...
    def reset_state(self) -> None:
        ...
        if self.species_config and hasattr(
            self.species_config,
        ...
        self.fluxes = {reaction.id: 0.0 for reaction in self.current_pathway.reactions}
    ...
    def calculate_metabolic_fluxes(
        self,
        biomass: float,
        growth_rate: float,
        anode_potential: float,
        substrate_supply: float,
    ) -> dict[str, float]:
        ...
        for reaction in reactions:
            ...
            if (
                self.species_config
                and hasattr(self.species_config, "reactions")
                and len(self.species_config.reactions) > 0
            ):
                # Find matching reaction configuration
                reaction_config = next(
                    (r for r in self.species_config.reactions if r.id == reaction.id),
                    None,
                )
                if reaction_config:
            ...
            if isinstance(km, dict):
                ...
                substrate_key = (
                    "acetate" if self.substrate == Substrate.ACETATE else "lactate"
                )
    ...
    def update_metabolite_concentrations(
        self,
        fluxes: dict[str, float],
        dt: float,
        volume: float,
        ...
        for reaction_id, flux in fluxes.items():
            # Find reaction
            reaction = next(
                (r for r in self.current_pathway.reactions if r.id == reaction_id),
                None,
            )
    ...
    def calculate_oxygen_crossover_effects(
        self,
        cathode_o2_conc: float,
        membrane_area: float,
        ...
        return o2_flux * membrane_area * 3600 * 1000  # mmol/h

    def calculate_electron_shuttle_dynamics(
        self,
        biomass: float,
        growth_rate: float,
        ...
        species_name = (
            self.species.value if not self.is_mixed else "shewanella_oneidensis"
        )
    ...
    def calculate_current_output(
        ...
        cytochrome_flux = self.fluxes.get(
            "GSU_R004",
            0.0,
        ...
        if cytochrome_flux > 0:
            ...
            F = 96485  # C/mol
            direct_current = (
                cytochrome_flux * biomass * volume * F * et_efficiency
            ) / (3600 * electrode_area)
        else:
            direct_current = 0.0
    ...
    def calculate_coulombic_efficiency(
        ...
        if (
            ...
            and hasattr(self.substrate_config, "degradation_pathways")
            and len(self.substrate_config.degradation_pathways) > 0
        ):
            electrons_per_substrate = self.substrate_config.degradation_pathways[
                0
            ].electron_yield
        else:
        ...
        if self.species_config:
            max_efficiency = min(
                0.9,
                self.species_config.yield_coefficient * 10,
            )  # Scale yield coefficient
        ...
        if efficiency > 1.5 or efficiency < 0.001:
            ...
            substrate_utilization = min(
                1.0,
                substrate_consumed / 0.1,
            )  # Normalize to 0.1 mmol baseline
            efficiency = (
                min_efficiency
                + (max_efficiency - min_efficiency) * substrate_utilization
            )
    ...
    def step_metabolism(
        ...
        direct_current_density, mediated_current_density = (
            self.calculate_current_output(biomass, volume, electrode_area)
        )
        ...
        for reaction in self.current_pathway.reactions:
            if (
                substrate_key in reaction.stoichiometry
                and reaction.stoichiometry[substrate_key] < 0
            ):
                # Negative stoichiometry means consumption
                substrate_flux_contribution = abs(
                    reaction.stoichiometry[substrate_key],
                ) * self.fluxes.get(reaction.id, 0.0)
        ...
        if substrate_consumed <= 0:
            ...
            electrons_from_current = (
                (total_current * dt * 3600) / 96485 * 1000
            )  # mmol e-
            electrons_per_substrate = self.current_pathway.electron_yield
            substrate_consumed = (
                electrons_from_current / electrons_per_substrate
                if electrons_per_substrate > 0
                else 0
            )
        ...
        # Energy calculations
        atp_flux = sum(
            flux * reaction.stoichiometry.get("atp", 0)
            for reaction, flux in zip(
                self.current_pathway.reactions,
                self.fluxes.values(),
                strict=False,
            )
        )
        ...
        nadh_flux = sum(
            flux * reaction.stoichiometry.get("nadh", 0)
            for reaction, flux in zip(
                self.current_pathway.reactions,
                self.fluxes.values(),
                strict=False,
        ...
        electron_flux = sum(
            flux * reaction.stoichiometry.get("electron_anode", 0)
            for reaction, flux in zip(
                self.current_pathway.reactions,
                self.fluxes.values(),
                strict=False,
    ...
    def get_metabolic_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "nadh_ratio": self.metabolites["nadh"]
            / (self.metabolites["nadh"] + self.metabolites["nad_plus"]),
    ...
    def optimize_for_current_production(
        self,
