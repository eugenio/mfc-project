...
class AnionExchangeMembrane(BaseMembraneModel):
    ...
    def calculate_ionic_conductivity(self, temperature: Optional[float] = None,
        ...
        conductivity_OH = (self.aem_params.hydroxide_conductivity_ref * 
                          temp_factor * water_factor)
        
        # Account for carbonation
        # Mixed conductivity: σ = (1-x)σ_OH + x·σ_CO3
        carbonate_conductivity = (conductivity_OH * 
                                self.aem_params.carbonate_conductivity_ratio)
        
        effective_conductivity = ((1 - self.carbonate_fraction) * conductivity_OH + 
                                self.carbonate_fraction * carbonate_conductivity)
    ...
    def update_carbonation(self, co2_partial_pressure: float = 400e-6 * 101325,
    ...
    def calculate_degradation_rate(self, temperature: float,
                                  ph: float,
    ...
    def calculate_water_balance(self, current_density: float,
        ...
        effective_drag = (drag_coefficient * (1 - self.carbonate_fraction) +
                         2.0 * self.carbonate_fraction)  # CO3-- drags less
    ...
    def get_aem_properties(self) -> Dict[str, Any]:
        ...
        aem_specific = {
            ...
            'hydroxide_conductivity_S_cm': float(self.calculate_ionic_conductivity() / 100),
            'carbonate_fraction': float(self.carbonate_fraction),
            ...
            'carbonation_sensitivity': 'High' if self.carbonate_fraction > 0.1 else 'Low'
        }
        
        return {**base_properties, **aem_specific}


def create_aem_membrane(membrane_type: str = "Quaternary Ammonium",
