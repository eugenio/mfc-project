...
class QTableAnalyzer:
    ...
    def load_qtable(self, file_path: str | Path) -> np.ndarray | None:
        ...
        try:
            ...
            logger.exception(f"Error loading Q-table from {file_path}: {e}")
            return None
    ...
    def calculate_exploration_coverage(
        self,
        qtable: np.ndarray,
    ) -> tuple[float, int, int]:
    ...
    def determine_convergence_status(self, metrics: QTableMetrics) -> ConvergenceStatus:
        ...
        if (
            convergence_score >= self.convergence_thresholds["convergence_threshold"]
            and stability >= self.convergence_thresholds["stability_threshold"]
        ):
            return ConvergenceStatus.CONVERGED
        if convergence_score >= 0.7 and stability >= 0.8:
            return ConvergenceStatus.CONVERGING
        if convergence_score >= self.convergence_thresholds["divergence_threshold"]:
    ...
    def analyze_qtable(self, file_path: str | Path) -> QTableMetrics | None:
        ...
        try:
            ...
            action_diversity = (
                policy_entropy / np.log(total_actions) if total_actions > 1 else 0.0
            )
            state_value_variance = (
                np.var(np.max(qtable, axis=1)) if len(shape) >= 2 else 0.0
            )

            # Exploration metrics
            exploration_coverage, visited_states, unvisited_states = (
                self.calculate_exploration_coverage(qtable)
            )
            ...
            logger.exception(f"Error analyzing Q-table {file_path}: {e}")
            return None
    ...
    def compare_qtables(
        ...
        try:
            ...
            if qtable1.shape != qtable2.shape:
                logger.warning(
                    f"Q-tables have different shapes: {qtable1.shape} vs {qtable2.shape}",
                )
                return None
            ...
            convergence_improvement = (
                metrics2.convergence_score - metrics1.convergence_score
            )
            ...
            exploration_change = (
                metrics2.exploration_coverage - metrics1.exploration_coverage
            )
            ...
            logger.exception(f"Error comparing Q-tables: {e}")
            return None
    ...
    def batch_analyze_qtables(
        self,
        file_paths: list[Path] | None = None,
    ) -> dict[str, QTableMetrics]:
    ...
    def export_analysis_results(
        self,
        results: dict[str, QTableMetrics],
        output_file: str,
    ) -> None:
