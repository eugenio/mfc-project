...
class FeatureEngineer:
    ...
    def _extract_statistical_features(self) -> dict[str, float]:
        ...
        conductivities = [
            s.fused_measurement.conductivity_S_per_m for s in recent_states
        ]
        ...
        features["thickness_cv"] = features["thickness_std"] / max(
            0.01,
            features["thickness_mean"],
        )
        features["thickness_range"] = np.ptp(thicknesses)  # Peak-to-peak
        features["thickness_trend"] = np.polyfit(
            range(len(thicknesses)),
            thicknesses,
            1,
        )[0]
        ...
        features["conductivity_cv"] = features["conductivity_std"] / max(
            1e-6,
            features["conductivity_mean"],
        )
        features["conductivity_trend"] = np.polyfit(
            range(len(conductivities)),
            conductivities,
            1,
        )[0]
    ...
    def _extract_temporal_features(self) -> dict[str, float]:
        ...
        if len(thicknesses) >= 20:
            ...
            features["thickness_autocorr_lag1"] = (
            ...
            if len(thicknesses) >= 25:
                ...
                features["thickness_autocorr_lag5"] = (
                    lag5_corr if not np.isnan(lag5_corr) else 0
                )
        ...
        if self.enable_advanced_features and len(thicknesses) >= 32:
            ...
            features["spectral_centroid"] = np.sum(
                np.arange(1, len(power_spectrum) // 2)
                * power_spectrum[1 : len(power_spectrum) // 2],
            ) / np.sum(power_spectrum[1 : len(power_spectrum) // 2])
    ...
    def _extract_health_features(self, system_state: SystemState) -> dict[str, float]:
        ...
        return {
            ...
            "health_status_numeric": self._health_status_to_numeric(
    ...
    def _extract_control_features(self, system_state: SystemState) -> dict[str, float]:
        ...
        return {
            ...
            "concentration_efficiency": system_state.outlet_concentration
            / max(0.01, system_state.inlet_concentration),
            "power_density": system_state.power_output
            / max(0.01, system_state.current_density),
            "strategy_numeric": self._strategy_to_numeric(
                system_state.current_strategy,
            ),
            "adaptation_mode_numeric": self._adaptation_mode_to_numeric(
                system_state.adaptation_mode,
            ),
    ...
    def _extract_performance_features(
        ...
        return {
            ...
            "biofilm_health_score": performance_metrics.get(
    ...
    def _extract_advanced_features(self) -> dict[str, float]:
        ...
        if len(thicknesses) >= 20:
            ...
            features["short_vs_long_term_ratio"] = short_term_mean / max(
                0.01,
                long_term_mean,
            )
        ...
        if len(thicknesses) >= 10:
            ...
            for lag in lags:
                differences = [
                    thicknesses[i] - thicknesses[i - lag]
                    for i in range(lag, len(thicknesses))
                ]
    ...
    def _extract_derived_features(self, system_state: SystemState) -> dict[str, float]:
        ...
        return {
            ...
            "thickness_per_biomass": fused.thickness_um
            / max(0.01, fused.biomass_density_g_per_L),
            "conductivity_per_thickness": fused.conductivity_S_per_m
            / max(0.01, fused.thickness_um),
            "sensor_disagreement": abs(fused.eis_thickness - fused.qcm_thickness),
            "relative_sensor_disagreement": abs(
                fused.eis_thickness - fused.qcm_thickness,
            )
            / max(0.01, (fused.eis_thickness + fused.qcm_thickness) / 2),
            # Health efficiency metrics
            "health_per_thickness": health.overall_health_score
            / max(0.01, fused.thickness_um),
            "risk_weighted_health": health.overall_health_score
            * (
                1
                - max(
                    health.fouling_risk,
                    health.detachment_risk,
                    health.stagnation_risk,
                )
            ),
            "prediction_reliability": health.prediction_confidence
            * health.assessment_confidence,
            # System efficiency metrics
            "power_per_flow": system_state.power_output
            / max(0.01, system_state.flow_rate),
            "substrate_utilization": (
                system_state.inlet_concentration - system_state.outlet_concentration
            )
            / max(0.01, system_state.inlet_concentration),
            "current_efficiency": system_state.current_density
            / max(0.01, fused.conductivity_S_per_m),
            # Composite indicators
            "overall_system_efficiency": (
                system_state.power_output
                * health.overall_health_score
                * fused.fusion_confidence
            )
            / max(0.01, system_state.flow_rate),
            "biofilm_maturity_index": fused.thickness_um
            * fused.conductivity_S_per_m
            * health.stability_health,
            "sensor_fusion_quality": fused.sensor_agreement
            * fused.fusion_confidence
            * (1 - fused.cross_validation_error / 20.0),
        }
    ...
    def _classify_feature_type(self, feature_name: str) -> FeatureType:
        ...
        if any(
            word in feature_name for word in ["trend", "autocorr", "lag", "frequency"]
        ):
            return FeatureType.TEMPORAL
        if any(word in feature_name for word in ["health", "risk", "prediction"]):
            return FeatureType.DERIVED
        if any(
            word in feature_name for word in ["per_", "ratio", "efficiency", "index"]
        ):
...
class HyperparameterOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"Hyperparameter optimizer initialized with {strategy.value} strategy",
        )
    ...
    def optimize_controller_parameters(
        self,
        controller: AdaptiveMFCController,
        historical_data: list[dict[str, Any]],
        target_metric: str = "overall_performance",
    ) -> OptimizationResult:
        ...
        improvement = (result["best_score"] - baseline_score) / max(
            abs(baseline_score),
            0.01,
        )
    ...
    def _prepare_optimization_data(
        ...
        for data_point in historical_data:
            if (
                "system_state" not in data_point
                or "performance_metrics" not in data_point
            ):
                continue
    ...
    def _random_search_optimization(
        ...
        return {"best_params": best_params, "best_score": best_score, "converged": True}
...
class MLOptimizedMFCController:
    ...
    def __init__(
        self,
        base_controller: AdaptiveMFCController,
        optimization_strategy: OptimizationStrategy = OptimizationStrategy.BAYESIAN,
        reoptimization_interval: int = 100,
    ) -> None:  # Steps between reoptimization
        ...
        logger.info(
            f"ML-optimized MFC controller initialized with {optimization_strategy.value}",
        )

    def control_step_with_learning(
        self,
        eis_measurement,
        qcm_measurement,
        ...
        # Add ML insights
        control_results["ml_insights"] = self._generate_ml_insights(
            features,
            control_results,
        )
    ...
    def _perform_reoptimization(self) -> OptimizationResult:
        ...
        logger.info(
            f"Performing reoptimization with {len(self.control_data_history)} data points",
        )

        # Optimize parameters
        optimization_result = (
            self.hyperparameter_optimizer.optimize_controller_parameters(
        ...
        logger.info(
            f"Reoptimization completed. Improvement: {optimization_result.performance_improvement:.3f}",
        )
    ...
    def _get_recent_feature_importance(self) -> list[FeatureImportance]:
        ...
        performance_values = [
            d["performance_metrics"].get("overall_performance", 0.5)
    ...
    def _analyze_performance_trend(self) -> dict[str, Any]:
        ...
        recent_performance = [
            d["performance_metrics"].get("biofilm_health_score", 0.5)
        ...
        return {
            ...
            "current_performance": (
                recent_performance[-1] if recent_performance else 0.5
            ),
            "average_performance": (
                np.mean(recent_performance) if recent_performance else 0.5
            ),
            "performance_variance": (
                np.var(recent_performance) if recent_performance else 0.0
            ),
        }
        ...
        return {
            ...
            "learning_progress": {
                ...
                "next_optimization_in": max(
                    0,
                    self.reoptimization_interval - self.steps_since_optimization,
                ),
            },
            "performance_metrics": {
                "last_optimization_score": self.last_optimization_score,
            ...
            "feature_engineering": {
                "features_generated": (
                    len(self.control_data_history[-1]["features"])
                    if self.control_data_history
                    else 0
                ),
                "feature_types": [
                    "raw",
...
def create_ml_optimized_controller(
    ...
    base_controller = create_adaptive_mfc_controller(
        species,
        qlearning_config,
        sensor_config,
    )
    ...
    logger.info(
        f"ML-optimized MFC controller created with {optimization_strategy.value} optimization",
    )
