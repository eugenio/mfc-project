...
class SensorIntegratedMFCModel(IntegratedMFCModel):
        ...
        for _i in range(self.n_cells):
            ...
            if self.enable_eis and self.enable_qcm:
                ...
                fusion_model = SensorFusion(
                    method=fusion_method_map.get(
                        self.sensor_fusion_method,
    ...
    def step_sensor_integrated_dynamics(
        self,
        dt: float = 1.0,
    ) -> SensorIntegratedMFCState:
        ...
        for i in range(self.n_cells):
            ...
            if hasattr(self.biofilm_models[i], "step_biofilm_dynamics_with_sensors"):
                biofilm_state = self.biofilm_models[
                    i
                ].step_biofilm_dynamics_with_sensors(
                    dt=dt,
                ...
                biofilm_state.update(
                    {
                        "sensor_validated_thickness": biofilm_state[
                            "biofilm_thickness"
                        ],
        ...
        for i in range(self.n_cells):
            ...
            biomass = biofilm_states[i].get(
                "sensor_validated_biomass",
        ...
        substrate_utilization = (
            (inlet_conc - outlet_conc) / inlet_conc * 100 if inlet_conc > 0 else 0
        )

        base_state = self.flow_controller.discretize_enhanced_state(
            sum(
                [
                    0.35 * bs["biofilm_thickness"] * bs["biomass_density"] * 0.002
    ...
    def _collect_cell_sensor_data(
        ...
        sensor_data: dict[str, Any] = {"eis": {}, "qcm": {}, "fusion": {}}

        # Collect EIS data
        if self.enable_eis and cell_index < len(self.eis_models):
            try:
                thickness = biofilm_state.get(
                    "sensor_validated_thickness",
                    biofilm_state["biofilm_thickness"],
                )
                biomass = biofilm_state.get(
                    "sensor_validated_biomass",
                ...
                eis_properties = self.eis_models[cell_index].get_biofilm_properties(
                    eis_measurements,
                )
                ...
                sensor_data["eis"] = {
                    ...
                    "status": (
                        "good"
                        if eis_properties.get("measurement_quality", 0) > 0.7
                        else "degraded"
                    ),
                }
        ...
        if self.enable_qcm and cell_index < len(self.qcm_models):
            try:
                ...
                electrode_area_m2 = self.qcm_models[
                    cell_index
                ].electrode_area  # m² (get from QCM model)
                ...
                qcm_properties = self.qcm_models[
                    cell_index
                ].estimate_biofilm_properties(qcm_measurement)
                ...
                sensor_data["qcm"] = {
                    ...
                    "status": (
                        "good"
                        if qcm_properties.get("measurement_quality", 0) > 0.7
                        else "degraded"
                    ),
                }
        ...
        if (
            sensor_data["eis"].get("measurements")
            and sensor_data["qcm"].get("measurement")
            and cell_index < len(self.sensor_fusion_models)
        ):
            try:
                ...
                sensor_data["fusion"] = {
                    ...
                    "status": (
                        "good" if fused_result.fusion_confidence > 0.7 else "degraded"
                    ),
                }
    ...
    def _aggregate_sensor_data(self, sensor_measurements: list[dict]) -> dict:
        ...
        if qcm_masses:
            aggregated["qcm"] = {
                ...
                "frequency_shift_Hz": (
                    np.mean(qcm_freq_shifts) if qcm_freq_shifts else 0
                ),
    ...
    def _calculate_biofilm_current_enhancement(
        self,
        biofilm_state: dict,
        sensor_data: dict,
    ) -> float:
        ...
        if "fusion" in sensor_data and "thickness" in sensor_data["fusion"]:
            ...
            confidence = sensor_data["eis"]["properties"].get(
                "measurement_quality",
                0.5,
            )
        else:
        ...
        return base_current_density * thickness_factor * confidence_factor

    def _calculate_model_validation_score(
        self,
        ...
        for _i, (biofilm_state, sensor_data) in enumerate(
            zip(biofilm_states, sensor_measurements, strict=False),
        ):
            ...
            if "fusion" in sensor_data and "thickness" in sensor_data["fusion"]:
                ...
                thickness_error = abs(model_thickness - sensor_thickness) / max(
                    sensor_thickness,
                    1.0,
                )
                biomass_error = abs(model_biomass - sensor_biomass) / max(
                    sensor_biomass,
                    1.0,
                )
    ...
    def _handle_sensor_faults(self, sensor_measurements: list[dict]) -> None:
    ...
    def _create_sensor_integrated_state(
        self,
        biofilm_states: list[dict],
        metabolic_states: list,
        sensor_measurements: list[dict],
    ...
    def run_sensor_integrated_simulation(
        self,
        dt: float = 1.0,
        ...
        for hour in range(int(self.simulation_hours / dt)):
            ...
            self.step_sensor_integrated_dynamics(dt)
    ...
    def _save_sensor_checkpoint(self, hour: int) -> None:
        ...
        checkpoint = {
            ...
            "q_table": (
                dict(self.flow_controller.q_table)
                if hasattr(self.flow_controller, "q_table")
                else {}
            ),
    ...
    def _compile_sensor_results(self) -> dict[str, Any]:
        ...
        # Base results
        results = (
            super()._compile_results() if hasattr(super(), "_compile_results") else {}
        )
        ...
        if SENSING_AVAILABLE and self.history:
            ...
            sensor_agreements = [
                getattr(s, "model_sensor_agreement", 0.5) for s in self.history
            ]
            fusion_confidences = [
                np.mean(getattr(s, "fusion_confidence", [0.5])) for s in self.history
            ]
            thickness_errors = [
                np.mean(getattr(s, "thickness_prediction_error", [0]))
            ...
            results.update(
                {
                    ...
                    "adaptive_calibration_events": len(
                        self.adaptive_calibration_events,
                    ),
        ...
        if hasattr(self.flow_controller, "get_controller_performance_summary"):
            controller_performance = (
                self.flow_controller.get_controller_performance_summary()
            )
    ...
    def get_sensor_diagnostics(self) -> dict[str, Any]:
        ...
        diagnostics = {
            "sensor_availability": {
                ...
                "n_eis_models": (
                    len(self.eis_models) if hasattr(self, "eis_models") else 0
                ),
                "n_qcm_models": (
                    len(self.qcm_models) if hasattr(self, "qcm_models") else 0
                ),
                "n_fusion_models": (
                    len(self.sensor_fusion_models)
                    if hasattr(self, "sensor_fusion_models")
                    else 0
                ),
            },
            "sensor_performance": {
        ...
        if hasattr(self, "biofilm_models"):
            for i, model in enumerate(self.biofilm_models):
                if hasattr(model, "get_sensor_diagnostics"):
                    diagnostics[f"cell_{i}_biofilm_sensor_diagnostics"] = (
                        model.get_sensor_diagnostics()
                    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Sensor-Integrated MFC Model Simulation",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Enable EIS sensors",
    )
    parser.add_argument(
        "--qcm",
        action="store_true",
        default=True,
        help="Enable QCM sensors",
    )
    parser.add_argument(
        "--fusion",
        choices=["kalman_filter", "weighted_average", "maximum_likelihood", "bayesian"],
        default="kalman_filter",
        help="Sensor fusion method",
    )
