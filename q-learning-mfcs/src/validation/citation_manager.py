...
class Citation:
    ...
    def to_vancouver_format(self) -> str:
        ...
        return (
            f"{author_str}. {title_str}. {journal_str}. {date_str}{vol_str}{doi_str}."
        )

    def to_bibtex_format(self, cite_key: str | None = None) -> str:
...
class CitationManager:
    ...
    def create_citation_from_reference(
        self,
        ...
        if reference.authors:
            ...
            elif "," in reference.authors and reference.authors.count(",") != 1:
    ...
    def format_citations(
        ...
        for citation in citations:
            if style.lower() == "apa":
                ...
                warnings.warn(
                    f"Unknown citation style: {style}. Using APA.",
                    stacklevel=2,
                )
                formatted.append(citation.to_apa_format())
    ...
    def generate_bibliography(
        ...
        if pmids is None:
            ...
            citations_to_format = [
                self.citations[pmid] for pmid in pmids if pmid in self.citations
            ]
    ...
    def create_parameter_report(
        ...
        references: list[LiteratureReference] | None = None,
        ...
        if citations:
            ...
            if quality_scores:
                high_quality = sum(
                    1 for score in quality_scores if score.overall_score >= 0.8
                )
    ...
    def generate_validation_report(
        self,
        reports: list[CitationReport] | None = None,
        ...
        warnings.warn(
            f"Unknown output format: {output_format}. Using markdown.",
            stacklevel=2,
        )
        return self._generate_markdown_report(reports)
    ...
    def _generate_markdown_report(self, reports: list[CitationReport]) -> str:
        ...
        for i, param_report in enumerate(reports, 1):
            ...
            report += (
                f"- **Value:** {param_report.parameter_value} {param_report.units}\n"
            )
            ...
            if param_report.quality_scores:
                avg_quality = sum(
                    q.overall_score for q in param_report.quality_scores
                ) / len(param_report.quality_scores)
            ...
            if param_report.citations:
                ...
                formatted_citations = self.format_citations(
                    param_report.citations,
                    "apa",
                )
    ...
    def _generate_html_report(self, reports: list[CitationReport]) -> str:
        ...
        for param_report in reports:
            ...
            html += (
                f"<p><strong>Status:</strong> {param_report.validation_status}</p>\n"
            )
    ...
    def export_bibtex_file(self, filepath: str, pmids: list[str] | None = None) -> None:
    ...
    def get_citation_statistics(self) -> dict[str, Any]:
        ...
        for citation in citations:
            if citation.year > 0:
                stats["year_distribution"][citation.year] = (
                    stats["year_distribution"].get(citation.year, 0) + 1
                )
        ...
        for citation in citations:
            if citation.journal:
                journal_counts[citation.journal] = (
                    journal_counts.get(citation.journal, 0) + 1
                )
        stats["journal_distribution"] = dict(
        ...
        stats["author_distribution"] = dict(
