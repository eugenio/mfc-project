...
class MFCExperimentalDatabase:
    ...
    def _build_experimental_database(self) -> None:
    ...
    def _add_shewanella_datasets(self) -> None:
    ...
    def _add_geobacter_datasets(self) -> None:
    ...
    def _add_mixed_culture_datasets(self) -> None:
...
class ModelValidationFramework:
    ...
    def __init__(self, experimental_db: MFCExperimentalDatabase) -> None:
    ...
    def validate_polarization_curve(
        ...
        if len(pred_current) > 0 and len(exp_current) > 0:
            ...
            common_voltage = np.linspace(
                max(min(exp_voltage), min(pred_voltage)),
                min(max(exp_voltage), max(pred_voltage)),
                10,
            )

            exp_current_interp = np.interp(
                common_voltage,
                exp_voltage[::-1],
                exp_current[::-1],
            )
            ...
            rmse_current = np.sqrt(
                np.mean((exp_current_interp - pred_current_interp) ** 2),
            )
        ...
        if len(pred_power) > 0 and len(exp_power) > 0:
            ...
            validation_result["performance_metrics"] = {
                ...
                "max_power_relative_error": abs(exp_max_power - pred_max_power)
                / exp_max_power
                * 100,
                "optimal_current_experimental": exp_current[exp_max_power_idx],
                "optimal_current_predicted": (
                    pred_current[pred_max_power_idx]
                    if len(pred_current) > pred_max_power_idx
                    else None
                ),
            }
        ...
        if "current_density" in validation_result["statistical_analysis"]:
            r2 = validation_result["statistical_analysis"]["current_density"][
                "r_squared"
            ]
            if r2 > 0.9:
    ...
    def validate_biofilm_growth(
        ...
        exp_current = (
            np.array(dataset.current_density) if dataset.current_density else None
        )
        ...
        if len(pred_thickness) > 0 and len(exp_thickness) > 0:
            ...
            common_time = np.linspace(
                max(min(exp_time), min(pred_time)),
                min(max(exp_time), max(pred_time)),
                10,
            )
            ...
            # Calculate metrics
            mae_thickness = np.mean(
                np.abs(exp_thickness_interp - pred_thickness_interp),
            )
            rmse_thickness = np.sqrt(
                np.mean((exp_thickness_interp - pred_thickness_interp) ** 2),
            )
            ...
            validation_result["growth_analysis"] = {
                ...
                "final_thickness_predicted": (
                    pred_thickness[-1] if len(pred_thickness) > 0 else 0
                ),
                "max_growth_rate_experimental": np.max(exp_growth_rate),
                "max_growth_rate_predicted": (
                    np.max(pred_growth_rate) if len(pred_growth_rate) > 0 else 0
                ),
            }
        ...
        if exp_current is not None and len(pred_current) > 0:
            ...
            if len(exp_current) == len(exp_thickness):
                ...
                correlation_pred = (
                    np.corrcoef(pred_thickness[: len(pred_current)], pred_current)[0, 1]
                    if len(pred_thickness) >= len(pred_current)
                    else 0
                )
        ...
        if "thickness_rmse" in validation_result["growth_analysis"]:
            ...
            final_thickness = validation_result["growth_analysis"][
                "final_thickness_experimental"
            ]
    ...
    def generate_comprehensive_validation_report(self) -> dict[str, Any]:
        ...
        organisms = {
            dataset.organism for dataset in self.experimental_db.datasets.values()
        }
        ...
        for dataset_id, dataset in self.experimental_db.datasets.items():
            ...
            if dataset.current_density and dataset.voltage:
                ...
                noise_factor = 0.1
                simulated_current = [
                    c * (1 + np.random.normal(0, noise_factor))
                    for c in dataset.current_density
                ]
                simulated_voltage = [
                    v * (1 + np.random.normal(0, noise_factor * 0.5))
                    for v in dataset.voltage
                ]
                simulated_power = [
                    c * v
                    for c, v in zip(simulated_current, simulated_voltage, strict=False)
                ]
                ...
                validation_result = self.validate_polarization_curve(
                    dataset_id,
                    model_predictions,
                )
    ...
    def _generate_recommendations(self, validation_report: dict[str, Any]) -> list[str]:
        ...
        if total > 0:
            ...
            if excellent_ratio > 0.7:
                recommendations.append(
                    "✅ Model validation is excellent across datasets. Ready for experimental verification.",
                )
            elif good_ratio + excellent_ratio > 0.7:
                recommendations.append(
                    "✅ Model validation is good. Minor parameter adjustments may improve accuracy.",
                )
            else:
                recommendations.append(
                    "⚠️ Model validation shows mixed results. Review parameter sources and model assumptions.",
                )

            if poor_ratio > 0.3:
                recommendations.append(
                    "❌ Some datasets show poor validation. Focus on improving physics models or parameter calibration.",
                )
        ...
        if "Shewanella oneidensis MR-1" in organisms:
            recommendations.append(
                "🧬 Shewanella-specific validation completed. GSM model parameters well-supported.",
            )

        if len(organisms) > 1:
            recommendations.append(
                "🔬 Multi-organism validation provides broader model applicability.",
            )
