...
class LiteratureDatabase:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_lit_pmid ON literature_values(pmid)",
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_query_hash ON validation_queries(query_hash)",
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_article_year ON article_metadata(publication_year)",
            )
        ...
        with self._db_lock, sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                ...
                (
                    ...
                    pmid,
                    organism,
                    json.dumps(experimental_conditions),
                    extraction_method,
                    confidence_score,
                ),
            )
    ...
    def query_literature_values(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                # Parse JSON fields
                result["experimental_conditions"] = json.loads(
                    result["experimental_conditions"] or "[]",
                )
                results.append(result)

            return results

    def search_literature_for_parameter(
        self,
        query: ValidationQuery,
    ) -> list[PubMedArticle]:
        """Search literature for parameter validation."""
        # Build search terms
        search_terms = ["microbial fuel cell", query.parameter_name.replace("_", " ")]
        ...
        pubmed_query = PubMedQuery(
            ...
            date_range=(
                f"{datetime.now().year - query.max_literature_age_years}/01/01",
                "",
            ),
            max_results=50,
    ...
    def _store_article_metadata(self, articles: list[PubMedArticle]) -> None:
        ...
        if status == "NEEDS_REVIEW":
            if query.parameter_value > lit_max:
                recommendations.append(
                    f"Value {query.parameter_value} exceeds literature maximum {lit_max:.3f}",
                )
            elif query.parameter_value < lit_min:
                recommendations.append(
                    f"Value {query.parameter_value} below literature minimum {lit_min:.3f}",
                )
            recommendations.append(f"Consider using literature mean: {lit_mean:.3f}")

        elif status == "ACCEPTABLE":
            recommendations.append(
                f"Value within acceptable range. Literature mean: {lit_mean:.3f}",
            )
            ...
            if result:
                try:
                    ...
                    return ValidationResult(
                        ...
                        statistical_analysis=result_dict.get(
                            "statistical_analysis",
                            {},
                        ),
            ...
            for future in as_completed(future_to_query):
                ...
                try:
                    ...
                    self.logger.exception(
                        f"Validation failed for {query.parameter_name}: {e}",
                    )
                    # Create error result
    ...
    def generate_validation_report(
        ...
        summary["literature_coverage"] = (
            has_literature / len(results) if results else 0.0
        )
    ...
    def export_database(self, filepath: str) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                record["experimental_conditions"] = json.loads(
                    record["experimental_conditions"] or "[]",
                )
                export_data["literature_values"].append(record)

            # Export article metadata
            cursor = conn.execute(
                "SELECT * FROM article_metadata LIMIT 1000",
            )  # Limit for size
            columns = [desc[0] for desc in cursor.description]
            for row in cursor.fetchall():
                record = dict(zip(columns, row, strict=False))
