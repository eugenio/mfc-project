...
class MonteCarloAnalysis:
    ...
    def generate_parameter_samples(self, n_samples: int = None) -> pd.DataFrame:
        ...
        for param_name, uncertainty in self.parameter_uncertainties.items():
            ...
            if dist_type == 'normal':
                ...
                samples[param_name] = np.random.triangular(bounds[0], nominal, bounds[1], n_samples)
    ...
    def propagate_uncertainty(self, model_function: Callable, output_names: List[str]) -> Dict[str, Any]:
        ...
        print(f"Running Monte Carlo analysis with {len(parameter_samples)} samples...")
        ...
        for name in output_names:
            if len(outputs[name]) > 0:
                ...
                uncertainty_results['output_statistics'][name] = {
                    ...
                    'confidence_interval_95': (np.percentile(data, 2.5), np.percentile(data, 97.5)),
                    'coefficient_of_variation': np.std(data) / np.mean(data) if np.mean(data) != 0 else np.inf,
    ...
    def sensitivity_analysis(self, model_function: Callable, output_name: str) -> Dict[str, float]:
        ...
        if not self.results:
            print("Running Monte Carlo analysis first...")
        ...
        for param_name in self.parameter_uncertainties.keys():
            ...
            if len(param_values) > 1 and np.std(param_values) > 0:
                ...
                sensitivity_indices[param_name] = correlation**2  # R-squared as sensitivity measure
            else:
...
class UncertaintyQuantificationFramework:
    ...
    def _build_uncertainty_database(self):
        ...
        self.parameter_uncertainties['flavin_transfer_efficiency'] = ParameterUncertainty(
    ...
    def create_simplified_model_function(self):
        ...
        def simplified_mfc_model(params: Dict[str, float]) -> Dict[str, float]:
            ...
            # Extract parameters
            density = params.get('fluid_density', 1000.0)
            viscosity = params.get('dynamic_viscosity', 0.00089)
            ...
            # Combined effects
            current_density = (metabolic_flux / 4.11) * biofilm_factor * transport_factor * electrode_factor * flavin_efficiency
            
            # Substrate utilization
            substrate_utilization = min(1.0, (lactate_uptake / 4.11) * transport_factor * biofilm_factor)
            ...
            biofilm_thickness = min(100e-6, growth_rate * 24 * 7 * 1e-6)  # 1 week growth
            
            # System efficiency
            system_efficiency = current_density * substrate_utilization * flavin_efficiency
    ...
    def run_comprehensive_uncertainty_analysis(self) -> Dict[str, Any]:
        ...
        uncertainty_results = mc_analysis.propagate_uncertainty(model_function, output_variables)
        
        print(f"✅ Completed {uncertainty_results['successful_runs']} successful runs")
        print(f"   Success rate: {uncertainty_results['success_rate']:.2%}")
        ...
        for output_var in ['current_density', 'power_density', 'system_efficiency']:
            print(f"  Analyzing sensitivity for {output_var}...")
    ...
    def generate_uncertainty_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for output_name, stats in output_stats.items():
            ...
            summary['uncertainty_levels'][output_name] = {
                ...
                'relative_uncertainty': ci_width / stats['mean'] if stats['mean'] != 0 else np.inf
            }
            
            summary['confidence_intervals'][output_name] = stats['confidence_interval_95']
        ...
        for output_name, sens_dict in sensitivity.items():
            sorted_sensitivity = sorted(sens_dict.items(), key=lambda x: abs(x[1]), reverse=True)
            summary['most_sensitive_parameters'][output_name] = sorted_sensitivity[:3]  # Top 3
        
        return summary

if __name__ == "__main__":
    # Example usage
    print("🎲 Phase 5: Uncertainty Quantification Framework")
    print("=" * 60)
    
    # Create uncertainty framework
    uq_framework = UncertaintyQuantificationFramework()
    
    print(f"Parameter uncertainties loaded: {len(uq_framework.parameter_uncertainties)}")
    
    # Run comprehensive analysis
    results = uq_framework.run_comprehensive_uncertainty_analysis()
    
    # Generate summary
    print("\n📊 Generating uncertainty summary...")
    ...
    for output_name, params in summary['most_sensitive_parameters'].items():
        ...
        for i, (param, sensitivity) in enumerate(params):
            print(f"    {i+1}. {param}: {sensitivity:.3f}")
