...
class SemiCellDimensions:
    ...
    wall_thickness: float = 0.025  # m  (2.5 cm) — frame wall around chamber

    floor_thickness: float = 0.002  # m  (2 mm) — floor left at bottom of pocket

    # Flow ports
    flow_port_diameter: float = 0.006  # m  (6 mm bore)
...
class GasCathodeDimensions:
    ...
    ledge_width: float = 0.002  # m  (2 mm) shelf lip for electrode support
    ledge_depth: float = 0.002  # m  (2 mm) ledge lip depth
...
class CurrentCollectorSpec:
    ...
    groove_offset_from_end: float = 0.005  # m  (5 mm) O-ring groove offset
...
@dataclass(frozen=True)
class BackWallSpec:
    """Flat back wall plate for the sandwich semicell.

    Outer structural plate with flow ports placed diagonally
    for counter-flow between anode and cathode.
    """

    thickness: float = 0.015  # m (15 mm)
    material: str = "Polypropylene"
    port_offset_x: float = 0.015  # m from chamber edge to port centre (X)
    port_offset_y: float = 0.0054  # m from chamber edge to port centre (Y)
    pocket_depth: float = 0.006  # m (6 mm) flow distribution pocket on inner face
    groove_offset: float = 0.003  # m (3 mm from inner edge to groove centre)


@dataclass(frozen=True)
class ElectrodeHolderSpec:
    """Grid frame that presses the electrode flat.

    A plate with a grid of open cells allowing flow-through,
    while ribs provide mechanical support to the electrode.
    """

    thickness: float = 0.002  # m (2 mm)
    rib_width: float = 0.002  # m (2 mm rib between openings)
    grid_count: int = 7  # 7×7 open cells
    material: str = "Polypropylene"


@dataclass(frozen=True)
class TiStripSpec:
    """Thin titanium current-collector strip.

    Runs along Y-axis, protruding from the +Y side of the frame
    for external electrical connection. Three strips at evenly
    spaced X positions across the electrode.
    """

    thickness: float = 0.001  # m (1 mm)
    width: float = 0.005  # m (5 mm)
    count: int = 3  # strips per electrode face
    protrusion: float = 0.020  # m (20 mm tab beyond frame edge)
    material: str = "Ti Grade 2"


@dataclass(frozen=True)
class MembraneHolderSpec:
    """Wire-grid frame that supports and clamps the membrane.

    Thin wires span the active area to hold the membrane in place
    while allowing electrolyte access. O-ring groove on the
    membrane-facing side of the solid frame for sealing.
    """

    thickness: float = 0.002  # m (2 mm)
    wire_width: float = 0.001  # m (1 mm wire)
    grid_count: int = 5  # 5×5 wire grid
    material: str = "Polypropylene"
...
class SupportFeetSpec:
    ...
    foot_depth: float = 0.150  # m (matches stack outer_side)
...
class ReservoirLidSpec:
    ...
    seal_boss_wall: float = 0.003  # m (3 mm) wall around motor hole
    feed_port_count: int = 2
    feed_port_diameter: float = 0.008  # m (8 mm)
    nipple_boss_height: float = 0.008  # m (8 mm)
    nipple_boss_wall: float = 0.002  # m (2 mm) wall around feed port
...
class StackCADConfig:
    ...
    # --- sandwich semicell specs -----------------------------------------------
    back_wall: BackWallSpec = field(default_factory=BackWallSpec)
    electrode_holder: ElectrodeHolderSpec = field(
        default_factory=ElectrodeHolderSpec,
    )
    ti_strip: TiStripSpec = field(default_factory=TiStripSpec)
    membrane_holder: MembraneHolderSpec = field(
        default_factory=MembraneHolderSpec,
    )
    ...
    def cell_thickness(self) -> float:
        ...
        Sandwich layers (anode → cathode):
        back_wall + 2×holder + electrode + strip
        + 2×membrane_holder + membrane
        + strip + electrode + 2×holder + back_wall
        """
        return (
            2 * self.back_wall.thickness
            + 4 * self.electrode_holder.thickness
            + 2 * self.electrode.thickness
            + 2 * self.ti_strip.thickness
            + 2 * self.membrane_holder.thickness
    ...
    # --- sandwich-design derived properties -----------------------------------

    @property
    def ti_strip_x_positions(self) -> list[float]:
        """X-coordinates of Ti strip centres, evenly spaced [m].

        Strips run vertically (along Y) and protrude from the +Y side.
        """
        n = self.ti_strip.count
        side = self.electrode.side_length
        spacing = side / (n + 1)
        return [
            -side / 2 + spacing * (i + 1) for i in range(n)
        ]

    @property
    def ti_strip_length(self) -> float:
        """Total length of each Ti strip along Y [m].

        Spans electrode_side + wall_thickness + protrusion.
        """
        return (
            self.electrode.side_length
            + self.semi_cell.wall_thickness
            + self.ti_strip.protrusion
        )

    def port_positions(
        self,
        is_anode: bool,  # noqa: ARG002
    ) -> tuple[tuple[str, float], tuple[str, float]]:
        """Return (inlet, outlet) port descriptors for a back wall.

        Each descriptor is ``(face, y_centre)`` where *face* is
        ``"-X"`` or ``"+X"`` and *y_centre* is relative to the plate
        centre.

        Both anode and cathode plates are built with the same port
        positions: inlet bottom-left (-X, -y), outlet top-right (+X, +y).
        The cathode back wall is flipped 180° around X in the assembly,
        which inverts Y and automatically creates the diagonal
        counter-flow pattern (cathode ends up: left-top, right-bottom).
        """
        half = self.semi_cell.inner_side / 2
        off = self.back_wall.port_offset_y
        lo = -half + off
        hi = half - off
        return (("-X", lo), ("+X", hi))
    ...
    def validate(self) -> list[str]:
        ...
        # Back wall pocket must fit
        if self.back_wall.pocket_depth >= self.back_wall.thickness:
            warnings.append("Back wall pocket depth exceeds wall thickness")

        # Membrane holder wire width must be positive
        if self.membrane_holder.wire_width <= 0:
            warnings.append("Membrane holder wire_width must be > 0")
