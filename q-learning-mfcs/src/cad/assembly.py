...
_COLOURS: dict[str, tuple[float, float, float]] = {
    "back_wall_anode": (0.8, 0.3, 0.3),  # warm red
    "back_wall_cathode": (0.3, 0.4, 0.8),  # blue
    "electrode_holder": (0.6, 0.55, 0.45),  # tan PP
    "membrane_holder": (0.7, 0.65, 0.5),  # light tan PP
    "ti_strip": (0.78, 0.78, 0.82),  # titanium silver
...
class MFCStackAssembly:
    ...
    def build(self) -> cq.Assembly:
        ...
        # --- repeating cell units (13-layer sandwich) ---
        bw_t = _mm(cfg.back_wall.thickness)
        eh_t = _mm(cfg.electrode_holder.thickness)
        el_t = _mm(cfg.electrode.thickness)
        ts_t = _mm(cfg.ti_strip.thickness)
        mh_t = _mm(cfg.membrane_holder.thickness)
        mem_t = _mm(cfg.membrane.thickness)

        # Pre-build shared solids
        holder_back_solid = electrode_holder.build(cfg, is_front=False)
        holder_front_solid = electrode_holder.build(cfg, is_front=True)
        electrode_solid = electrode_placeholder.build(cfg)
        strip_solid = ti_strip_collector.build(cfg)
        mh_solid = membrane_holder.build(cfg)
        strip_x_positions = [_mm(x) for x in cfg.ti_strip_x_positions]
        # Y-shift so strip bottom aligns with electrode bottom, tab protrudes +Y
        strip_y_shift = (
            _mm(cfg.ti_strip_length) / 2 - _mm(cfg.electrode.side_length) / 2
        )

        for i in range(cfg.num_cells):
            # 1. Anodic back wall
            asm.add(
                back_wall.build(cfg, is_anode=True),
                name=f"back_wall_anode_{i}",
                loc=cq.Location(cq.Vector(0, 0, z + bw_t / 2)),
                color=cq.Color(*_COLOURS["back_wall_anode"]),
            )
            z += bw_t

            # 2. Back electrode holder
            asm.add(
                holder_back_solid,
                name=f"holder_back_anode_{i}",
                loc=cq.Location(cq.Vector(0, 0, z + eh_t / 2)),
                color=cq.Color(*_COLOURS["electrode_holder"]),
            )
            z += eh_t
            ...
            asm.add(
                ...
                loc=cq.Location(cq.Vector(0, 0, z + el_t / 2)),
                color=cq.Color(*_COLOURS["electrode_anode"]),
            )
            z += el_t

            # 4. Anode Ti strip collectors
            for j, sx in enumerate(strip_x_positions):
                asm.add(
                    strip_solid,
                    name=f"ti_strip_anode_{i}_{j}",
                    loc=cq.Location(cq.Vector(sx, strip_y_shift, z + ts_t / 2)),
                    color=cq.Color(*_COLOURS["ti_strip"]),
                )
            z += ts_t

            # 5. Front electrode holder (anode side — flipped so slots face -Z toward strips)
            asm.add(
                holder_front_solid,
                name=f"holder_front_anode_{i}",
                loc=cq.Location(
                    cq.Vector(0, 0, z + eh_t / 2),
                    cq.Vector(0, 1, 0),
                    180,
                ),
                color=cq.Color(*_COLOURS["electrode_holder"]),
            ...
            # 6. Anode membrane holder
            asm.add(
                mh_solid,
                name=f"membrane_holder_anode_{i}",
                loc=cq.Location(cq.Vector(0, 0, z + mh_t / 2)),
                color=cq.Color(*_COLOURS["membrane_holder"]),
            )
            z += mh_t

            # 7. Membrane (thin — use electrode_placeholder as visual stand-in)
            membrane_vis = (
                cq.Workplane("XY")
                .box(_mm(cfg.membrane.active_side),
                     _mm(cfg.membrane.active_side),
                     mem_t)
            )
            asm.add(
                membrane_vis,
                name=f"membrane_{i}",
                loc=cq.Location(cq.Vector(0, 0, z + mem_t / 2)),
                color=cq.Color(*_COLOURS["membrane"]),
            )
            z += mem_t

            # 8. Cathode membrane holder (flipped — groove faces membrane)
            asm.add(
                mh_solid,
                name=f"membrane_holder_cathode_{i}",
                loc=cq.Location(
                    cq.Vector(0, 0, z + mh_t / 2),
                    cq.Vector(1, 0, 0),
                    180,
                ),
                color=cq.Color(*_COLOURS["membrane_holder"]),
            ...
            # 9. Front electrode holder (cathode side)
            asm.add(
                holder_front_solid,
                name=f"holder_front_cathode_{i}",
                loc=cq.Location(cq.Vector(0, 0, z + eh_t / 2)),
                color=cq.Color(*_COLOURS["electrode_holder"]),
            )
            z += eh_t

            # 10. Cathode Ti strip collectors
            for j, sx in enumerate(strip_x_positions):
                asm.add(
                    strip_solid,
                    name=f"ti_strip_cathode_{i}_{j}",
                    loc=cq.Location(cq.Vector(sx, strip_y_shift, z + ts_t / 2)),
                    color=cq.Color(*_COLOURS["ti_strip"]),
                )
            z += ts_t
            ...
            # 12. Back electrode holder (cathode side)
            asm.add(
                holder_back_solid,
                name=f"holder_back_cathode_{i}",
                loc=cq.Location(cq.Vector(0, 0, z + eh_t / 2)),
                color=cq.Color(*_COLOURS["electrode_holder"]),
            )
            z += eh_t

            # 13. Cathodic back wall (flipped — groove faces cell interior)
            asm.add(
                back_wall.build(cfg, is_anode=False),
                name=f"back_wall_cathode_{i}",
                loc=cq.Location(
                    cq.Vector(0, 0, z + bw_t / 2),
                    cq.Vector(1, 0, 0),
                    180,
                ),
                color=cq.Color(*_COLOURS["back_wall_cathode"]),
            )
            z += bw_t
    ...
    def _add_labels(self, asm: cq.Assembly, cfg: StackCADConfig) -> None:
        ...
        for text, lx, ly, lz in label_specs:
            try:
                ...
                logger.warning("Skipping port label %r", text, exc_info=True)
        ...
        for sign, label_text in [(1, "CC+"), (-1, "CC-")]:
            try:
                ...
                logger.warning("Skipping CC label %r", label_text, exc_info=True)
        ...
        for circuit, colour_key in circuits:
            ...
            for i in range(len(outlets) - 1):
                try:
                    ...
                    logger.warning(
                        "Skipping %s U-tube segment %d", circuit, i, exc_info=True,
                    )
    ...
    def expected_part_count(self) -> int:
        ...
        n_strips = self.config.ti_strip.count

        parts = 0
        parts += 2  # end plates
        # Per cell: 2 back walls + 4 holders + 2 electrodes
        #           + 2×n_strips Ti strips + 2 membrane holders + 1 membrane
        parts += n * (2 + 4 + 2 + 2 * n_strips + 2 + 1)
        parts += 4  # tie rods
