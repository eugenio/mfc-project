...
def create_literature_comparison_plots():
    ...
    original_1000h, orig_meta_1000h = load_simulation_data(
        "mfc_recirculation_control_20250724_040215.csv",
    )

    # Load literature-validated data (100h and 1000h)
    literature_100h, lit_meta_100h = load_simulation_data(
        "mfc_recirculation_control_literature_validated_*_044346.csv",
    )
    literature_1000h, lit_meta_1000h = load_simulation_data(
    ...
    orig_biofilm_avg = np.mean(
        [original_1000h[f"cell_{i + 1}_biofilm"].values for i in range(5)],
        axis=0,
    )
    ax1.plot(
        original_1000h["time_hours"][orig_time_mask],
        orig_biofilm_avg[orig_time_mask],
    ...
    lit_biofilm_avg_100h = np.mean(
        [literature_100h[f"cell_{i + 1}_biofilm"].values for i in range(5)],
        axis=0,
    )
    ax1.plot(
        literature_100h["time_hours"],
        lit_biofilm_avg_100h,
        color=color_literature,
        linewidth=2.5,
        label="Literature-Validated",
        alpha=0.8,
    )
    ...
    ax2.plot(
        original_1000h["time_hours"][orig_time_mask],
        original_1000h["total_power"][orig_time_mask],
        color=color_original,
        linewidth=2.5,
        label="Original Parameters",
        alpha=0.8,
    ...
    orig_utilization = (
        (
            original_1000h["reservoir_concentration"]
            - original_1000h["outlet_concentration"]
        )
        / original_1000h["reservoir_concentration"]
        * 100
    )
    lit_utilization = (
        (
            literature_100h["reservoir_concentration"]
            - literature_100h["outlet_concentration"]
        )
        / literature_100h["reservoir_concentration"]
        * 100
    )
    ...
    ax3.plot(
        original_1000h["time_hours"][orig_time_mask],
        orig_utilization[orig_time_mask],
        color=color_original,
        linewidth=2.5,
        label="Original Parameters",
        alpha=0.8,
    ...
    lit_biofilm_avg_1000h = np.mean(
        [literature_1000h[f"cell_{i + 1}_biofilm"].values for i in range(5)],
        axis=0,
    )
    ax4.plot(
        literature_1000h["time_hours"],
        lit_biofilm_avg_1000h,
        color=color_long_term,
        linewidth=2.5,
        label="Literature-Validated (1000h)",
        alpha=0.8,
    )
    ...
    orig_final_biofilm = np.mean(
        [original_1000h[f"cell_{i + 1}_biofilm"].iloc[orig_100h_idx] for i in range(5)],
    )
    lit_final_biofilm_100h = np.mean(
        [literature_100h[f"cell_{i + 1}_biofilm"].iloc[-1] for i in range(5)],
    )
    lit_final_biofilm_1000h = np.mean(
        [literature_1000h[f"cell_{i + 1}_biofilm"].iloc[-1] for i in range(5)],
    )
    ...
    ax6.bar(
        x,
        literature_100h_values,
        width,
        label="Literature (100h)",
        color=color_literature,
        alpha=0.8,
    )
    ax6.bar(
        x + width,
        literature_1000h_values,
        width,
        label="Literature (1000h)",
        color=color_long_term,
        alpha=0.8,
    )
    ...
    ax8.plot(
        original_1000h["time_hours"][time_subset],
        orig_biofilm_avg[time_subset],
        color=color_original,
        linewidth=3,
        label="Original (0.001 h⁻¹)",
        marker="o",
        markersize=4,
    )
    ax8.plot(
        literature_100h["time_hours"][time_subset_lit],
        lit_biofilm_avg_100h[time_subset_lit],
        color=color_literature,
        linewidth=3,
        label="Literature (0.05 h⁻¹)",
        marker="s",
        markersize=4,
    )
    ...
    ((lit_final_power_100h - orig_final_power) / orig_final_power) * 100
    ((lit_final_biofilm_100h - orig_final_biofilm) / orig_final_biofilm) * 100
    (lit_utilization.iloc[-1] - orig_utilization.iloc[orig_100h_idx])
