...
class MFCOptimizationSimulation:
    def __init__(self, use_gpu=True) -> None:
        ...
        self.flow_rate_bounds = (
            5.0e-6,
            50.0e-6,
        )  # Flow rate bounds (m³/s) = 18-180 mL/h
    ...
    def calculate_objective_function(self, step):
        ...
        if self.use_gpu:
            biofilm_deviation = gpu_accelerator.mean(
                gpu_accelerator.abs(
                    self.biofilm_thickness[step, :] - self.optimal_biofilm_thickness,
                ),
            )
        else:
            biofilm_deviation = np.mean(
                np.abs(
                    self.biofilm_thickness[step, :] - self.optimal_biofilm_thickness,
                ),
            )
    ...
    def calculate_flow_rate(self, time_seconds):
        ...
        # Phase 3 (500-800h): Adaptive control based on biofilm state
            ...
            if hasattr(self, "biofilm_thickness"):
                ...
                if current_step > 0:
                    ...
                    biofilm_deviation = abs(
                        avg_biofilm - self.optimal_biofilm_thickness,
                    )

                    # Adjust flow rate based on biofilm deviation
                    if biofilm_deviation > 0.2:  # Too much deviation
                        base_flow = (
                            15.0e-6 + 10.0e-6 * biofilm_deviation
                        )  # Higher flow for cleaning
                    else:
                        base_flow = (
                            20.0e-6 - 5.0e-6 * biofilm_deviation
                        )  # Lower flow for efficiency
                else:
                    base_flow = 20.0e-6
            else:
                base_flow = 20.0e-6
        ...
        # Phase 4 (800-1000h): Long-term stable operation
            ...
            stability_factor = 1.0 + 0.05 * np.sin(
                2 * np.pi * time_hours / 100.0,
            )  # 2% oscillation
    ...
    def save_data(self):
        ...
        json_data = {
            ...
            "results": {
                ...
                "average_biofilm_thickness": float(
                    np.mean(self.biofilm_thickness[-1, :]),
                ),
                "biofilm_deviation_from_optimal": float(
                    np.mean(
                        np.abs(
                            self.biofilm_thickness[-1, :]
                            - self.optimal_biofilm_thickness,
                        ),
                    ),
                ),
    ...
    def generate_plots(self, timestamp) -> None:
        ...
        for i in range(self.num_cells):
            plt.plot(
                time_hours,
                self.acetate_concentrations[:, i],
                linewidth=1.5,
                label=f"Cell {i + 1} outlet",
            )
        plt.xlabel("Time (hours)")
        ...
        avg_biofilm_dev = np.mean(
            np.abs(self.biofilm_thickness[-1, :] - self.optimal_biofilm_thickness),
        )
    ...
    def generate_detailed_plots(self, timestamp, time_hours) -> None:
        ...
        biofilm_deviations = np.mean(
            np.abs(self.biofilm_thickness - self.optimal_biofilm_thickness),
            axis=1,
        )
        axes[1, 0].scatter(
            biofilm_deviations,
