...
class PhysicsAccurateBiofilmModel:
    ...
    def __init__(self, biofilm_params: GeobacterBiofilmParameters) -> None:
    ...
    def calculate_diffusion_limitation(
        self,
        thickness_um: float,
        ...
        phi = thickness_m * np.sqrt(
            p.reaction_rate_coeff * substrate_conc / p.diffusion_coeff,
        )
    ...
    def calculate_inactive_fraction(
        self,
        thickness_um: float,
        time_hours: float,
    ) -> float:
        ...
        time_factor = p.inactive_fraction_start * (
            1 + p.inactive_accumulation_rate * time_hours
        )

        return min(0.8, thickness_factor * time_factor)

    def biofilm_growth_ode(
        self,
        state: list[float],
        t: float,
        substrate_conc: float,
        ...
        diffusion_factor = self.calculate_diffusion_limitation(
            thickness_um,
            substrate_conc,
        )
        ...
        if thickness_um < p.optimal_thickness_um:
            ...
            excess_factor = (p.maximum_thickness_um - thickness_um) / (
                p.maximum_thickness_um - p.optimal_thickness_um
            )
        ...
        dbiomass_dt = (
            net_growth_rate * biomass_density * 0.1
            - p.decay_rate * biomass_density * inactive_fraction
        )
    ...
    def simulate_growth(
        ...
        substrate_conc: float = 1.0,
        initial_thickness_um: float = 0.1,
    ) -> dict:
        ...
        results = {
            ...
            "diffusion_efficiency": [
                self.calculate_diffusion_limitation(t, substrate_conc)
                for t in thickness_um
            ],
            "inactive_fraction": [
                self.calculate_inactive_fraction(t, time_hours[i])
        ...
        for i, thickness in enumerate(thickness_um):
            ...
            if thickness <= self.params.optimal_thickness_um:
                ...
                thickness_factor = np.exp(
                    -excess
                    / (
                        self.params.maximum_thickness_um
                        - self.params.optimal_thickness_um
                    ),
                )

            current_density = (
                self.params.max_current_density
                * thickness_factor
                * active_fraction
                * diffusion_eff
            )
            results["current_density_A_m2"].append(current_density)
            results["electrochemical_activity"].append(
                thickness_factor * active_fraction * diffusion_eff,
            )
...
class QCMSensorModel:
    ...
    def __init__(self, qcm_params: QCMSensorParameters) -> None:
    ...
    def sauerbrey_frequency_shift(self, mass_per_area: float) -> float:
        ...
        return -p.mass_sensitivity * mass_per_area * p.fundamental_freq
    ...
    def viscoelastic_correction(
        ...
        freq_shift_viscoel = (
            -p.fundamental_freq / (2 * Z_quartz) * np.real(Z_film) * thickness_m
        )
        dissipation_shift = (
            p.fundamental_freq / (2 * Z_quartz) * np.imag(Z_film) * thickness_m
        )
    ...
    def gaussian_sensitivity_distribution(self, r_normalized: float) -> float:
        ...
        return max(sensitivity, p.edge_factor)

    def measure_biofilm(
        self,
        biofilm_results: dict,
        ...
        for i, mass_per_area in enumerate(biofilm_results["mass_per_area_kg_m2"]):
            ...
            temp_correction = (
                p.temperature_coeff * (temperature_c - 25.0) * p.fundamental_freq
            )
            ...
            thickness_calculated = (
                mass_calculated / biofilm_results["biomass_density_kg_m3"][i] * 1e6
            )
...
class BiofilmQCMController:
    ...
    def pid_control(
        ...
        derivative = (
            kd * (error - self.previous_error) / dt_hours if dt_hours > 0 else 0.0
        )
...
def create_physics_visualization(biofilm_results: dict, qcm_measurements: dict) -> None:
    ...
    plt.subplot(3, 3, 1)
    plt.plot(
        biofilm_results["time_hours"],
        biofilm_results["thickness_um"],
        "b-",
        linewidth=2,
        label="True Thickness",
    )
    plt.plot(
        qcm_measurements["time_hours"],
        qcm_measurements["thickness_qcm_um"],
        "r--",
        linewidth=2,
        label="QCM Measured",
    )
    ...
    plt.subplot(3, 3, 2)
    current_mA_cm2 = (
        np.array(biofilm_results["current_density_A_m2"]) * 1000 / 10000
    )  # Convert to mA/cm²
    ...
    plt.plot(
        qcm_measurements["time_hours"],
        qcm_measurements["frequency_shift_Hz"],
        "red",
        linewidth=2,
    )
    plt.xlabel("Time (hours)")
    ...
    plt.plot(
        biofilm_results["time_hours"],
        biofilm_results["electrochemical_activity"],
        "green",
        linewidth=2,
    )
    plt.xlabel("Time (hours)")
    ...
    plt.plot(
        biofilm_results["time_hours"],
        biofilm_results["diffusion_efficiency"],
        "brown",
        linewidth=2,
    )
    plt.xlabel("Time (hours)")
    ...
    plt.plot(
        biofilm_results["time_hours"],
        biofilm_results["inactive_fraction"],
        "orange",
        linewidth=2,
    )
    plt.xlabel("Time (hours)")
    ...
    plt.plot(
        biofilm_results["time_hours"],
        biofilm_results["biomass_density_kg_m3"],
        "magenta",
        linewidth=2,
    )
    plt.xlabel("Time (hours)")
    ...
    plt.tight_layout()
    plt.savefig(
        get_figure_path("physics_accurate_biofilm_qcm_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def run_physics_simulation(
    ...
    biofilm_results["current_density_A_m2"][-1] * 1000 / 10000  # mA/cm²
    biofilm_results["electrochemical_activity"][-1]
