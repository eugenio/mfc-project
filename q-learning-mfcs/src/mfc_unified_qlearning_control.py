...
class UnifiedQLearningController:
    def __init__(
        ...
        target_outlet_conc: float | None = None,
    ) -> None:
        ...
        self.target_outlet_conc = (
            target_outlet_conc
            if target_outlet_conc is not None
            else config.stability_target_outlet_concentration
        )
    ...
    def setup_state_action_spaces(self) -> None:
        ...
        self.biofilm_bins = np.linspace(
            0,
            self.config.biofilm_max_deviation,
            self.config.biofilm_deviation_bins,
        )
        self.substrate_bins = np.linspace(
            0,
            self.config.substrate_utilization_max,
            self.config.substrate_utilization_bins,
        )
        self.outlet_error_bins = np.linspace(
            self.config.outlet_error_range[0],
            self.config.outlet_error_range[1],
            self.config.outlet_error_bins,
        )
        self.flow_rate_bins = np.linspace(
            self.config.flow_rate_range[0],
    ...
    def select_action(self, state, current_flow_rate, current_inlet_conc):
        ...
        new_inlet_conc = np.clip(
            current_inlet_conc + substrate_adjustment,
            self.min_substrate,
            self.max_substrate,
        )
    ...
    def update_q_table(self, state, action, reward, next_state) -> None:
        ...
        adaptive_lr = self.learning_rate * (
            1.0 + 0.1 * np.tanh(reward / 100.0)
        )  # Boost learning for good rewards
    ...
    def calculate_unified_reward(
        ...
        if biofilm_deviation <= deviation_threshold:
            ...
            if (
                biofilm_thickness_history is not None
                and len(biofilm_thickness_history) >= 3
            ):
        ...
        if outlet_error < 1.0:  # Within 1 mmol/L of target
            concentration_base = 50.0 * (
                1.0 - outlet_error
            )  # Max 50 points for perfect tracking
        ...
        stability_bonus = 0
        if (
            abs(power_change) < 0.001
            and abs(substrate_change) < 0.5
            and outlet_error < 1.0
            and biofilm_deviation <= deviation_threshold
        ):
            stability_bonus = 30.0  # Bonus for stable, on-target operation

        # 6. FLOW RATE PENALTY when biofilm is below optimal (NEW)
        flow_penalty = 0
        current_flow_rate = getattr(
            self,
            "current_flow_rate",
            10.0,
        )  # Default if not set
        if biofilm_thickness_history is not None and len(biofilm_thickness_history) > 0:
            ...
            if (
                avg_biofilm < optimal_thickness * 0.9
            ):  # If biofilm is significantly below optimal
                if current_flow_rate > 20.0:  # High flow rate creating excessive shear
                    flow_penalty = (
                        -25.0 * (current_flow_rate - 20.0) / 10.0
                    )  # Penalty for high flow

        # 7. COMBINED PENALTY for poor performance
        combined_penalty = 0
        if (
            power_change < 0
            and substrate_change < 0
            and error_improvement < 0
            and biofilm_deviation > deviation_threshold
        ):
...
def add_subplot_labels(fig, start_letter="a") -> None:
...
class MFCUnifiedQLearningSimulation:
    def __init__(self, use_gpu=True, target_outlet_conc=10.0) -> None:
        ...
        self.unified_controller = UnifiedQLearningController(
            target_outlet_conc=target_outlet_conc,
        )
    ...
    def initialize_arrays(self) -> None:
        ...
        array_func = self.gpu_acc.zeros if self.use_gpu else np.zeros
    ...
    def update_cell(self, cell_idx, inlet_concentration, flow_rate, biofilm):
        ...
        acetate_consumed = (
            (consumption_rate * residence_time) / self.V_a * 1000
        )  # mmol/L
        ...
        if self.use_gpu:
            outlet_concentration = self.gpu_acc.maximum(
                0.001,
                inlet_concentration - acetate_consumed,
            )
        else:
            outlet_concentration = np.maximum(
                0.001,
                inlet_concentration - acetate_consumed,
            )
        ...
        voltage_base = 0.8
        if self.use_gpu:
            concentration_factor = self.gpu_acc.log(
                1.0 + inlet_concentration / self.K_AC,
            )
            biofilm_voltage_loss = 0.05 * self.gpu_acc.abs(
                biofilm - self.optimal_biofilm_thickness,
            )
            cell_voltage = self.gpu_acc.maximum(
                0.1,
                voltage_base + 0.1 * concentration_factor - biofilm_voltage_loss,
            )
        else:
            concentration_factor = np.log(1.0 + inlet_concentration / self.K_AC)
            biofilm_voltage_loss = 0.05 * np.abs(
                biofilm - self.optimal_biofilm_thickness,
            )
            cell_voltage = np.maximum(
                0.1,
                voltage_base + 0.1 * concentration_factor - biofilm_voltage_loss,
            )
    ...
    def update_biofilm(self, step, dt) -> None:
        ...
        for cell_idx in range(self.num_cells):
            ...
            if self.use_gpu:
                control_factor = self.gpu_acc.where(
                    ...
                    0.5,
                    self.gpu_acc.where(
                        current_thickness < self.optimal_biofilm_thickness * 0.8,
                        1.5,
                        1.0,
                    ),
                )
            ...
            if self.use_gpu:
                new_thickness = self.gpu_acc.clip(
                    current_thickness + net_growth,
                    0.5,
                    3.0,
                )
            else:
    ...
    def simulate_step(self, step) -> None:
        ...
        inlet_concentration = float(
            self.inlet_concentrations[step - 1],
        )  # Use previous step concentration
        ...
        substrate_utilization = (
            (inlet_concentration - final_conc) / inlet_concentration * 100.0
            if inlet_concentration > 0
            else 0
        )
        concentration_error = (
            avg_outlet_conc - self.unified_controller.target_outlet_conc
        )
        ...
        if self.use_gpu:
            biofilm_deviation = float(
                self.gpu_acc.to_cpu(
                    self.gpu_acc.mean(
                        self.gpu_acc.abs(
                            self.biofilm_thickness[step, :]
                            - self.optimal_biofilm_thickness,
                        ),
                    ),
                ),
            )
        ...
        if step > 1 and step % 60 == 0:
            ...
            current_state = self.unified_controller.discretize_state(
                float(stack_power),
                biofilm_deviation,
                float(substrate_utilization),
                float(concentration_error),
                current_flow_ml,
                time_hours,
            )

            # Select unified action (both flow and concentration)
            action_idx, new_flow_rate, new_inlet_conc = (
                self.unified_controller.select_action(
                    current_state,
                    self.flow_rates[step - 1],
                    float(self.inlet_concentrations[step - 1]),
                )
            )
            ...
            if hasattr(self, "prev_state") and hasattr(self, "prev_action"):
                ...
                prev_biofilm_dev = float(
                    np.mean(
                        np.abs(
                            self.biofilm_thickness[step - 60, :]
                            - self.optimal_biofilm_thickness,
                        ),
                    ),
                )
        ...
        self.objective_values[step] = (
            self.w_power * power_objective
    ...
    def run_simulation(self) -> None:
        ...
        for step in range(self.num_steps):
            ...
            if step % 36000 == 0:  # Every 100 hours
                step * self.dt / 3600
                self.unified_controller.get_control_statistics()
                (float(self.inlet_concentrations[step]) if step > 0 else 20.0)
                outlet_conc = (
                    float(self.outlet_concentrations[step]) if step > 0 else 0.0
                )
                abs(
                    outlet_conc - self.unified_controller.target_outlet_conc,
                )
        ...
        if self.use_gpu:
            ...
            self.acetate_concentrations = self.gpu_acc.to_cpu(
                self.acetate_concentrations,
            )
            ...
            self.substrate_consumptions = self.gpu_acc.to_cpu(
                self.substrate_consumptions,
            )
            ...
            self.substrate_utilizations = self.gpu_acc.to_cpu(
                self.substrate_utilizations,
            )
    ...
    def save_data(self):
        ...
        csv_filename = get_simulation_data_path(
            f"mfc_unified_qlearning_{timestamp}.csv",
        )
        ...
        json_data = {
            "simulation_info": {
                ...
                "initial_inlet_concentration_mmol_l": float(
                    self.inlet_concentrations[0],
                ),
            ...
            "results": {
                "total_energy_wh": float(
                    np.trapezoid(self.stack_powers, dx=self.dt / 3600),
                ),
                ...
                "final_inlet_concentration_mmol_l": float(
                    self.inlet_concentrations[-1],
                ),
                "final_outlet_concentration_mmol_l": float(
                    self.outlet_concentrations[-1],
                ),
                "final_concentration_error_mmol_l": float(
                    self.concentration_errors[-1],
                ),
                "final_substrate_utilization_percent": float(
            ...
            "control_analysis": {
                "outlet_concentration_stats": {
                    "mean": float(
                        np.mean(
                            self.outlet_concentrations[self.outlet_concentrations > 0],
                        ),
                    ),
                    "std": float(
                        np.std(
        ...
        # Save JSON
        json_filename = get_simulation_data_path(
            f"mfc_unified_qlearning_{timestamp}.json",
        )
    ...
    def generate_plots(self, timestamp) -> None:
        ...
        plt.plot(
            ...
            "orange",
            linewidth=1.5,
            label="Outlet (Average)",
        )
        plt.axhline(
            y=self.unified_controller.target_outlet_conc,
            color="red",
            linestyle="--",
            linewidth=2,
            alpha=0.8,
            label=f"Target ({self.unified_controller.target_outlet_conc:.1f})",
        )
        ...
        plt.axhline(
            y=self.optimal_biofilm_thickness,
            color="black",
            linestyle="--",
            alpha=0.7,
            label="Optimal",
        ...
        if len(valid_outlet) > 0:
            ...
            plt.axvline(
                x=self.unified_controller.target_outlet_conc,
                color="red",
                linestyle="--",
                linewidth=2,
                label="Target",
        ...
        if np.any(valid_mask):
            plt.scatter(
                self.inlet_concentrations[valid_mask],
                self.outlet_concentrations[valid_mask],
                alpha=0.4,
                s=3,
                c=time_hours[valid_mask],
                cmap="viridis",
            )
        ...
        biofilm_obj = np.maximum(
            0.0,
            1.0
            - np.abs(
                np.mean(self.biofilm_thickness, axis=1)
                - self.optimal_biofilm_thickness,
            ),
        )
        ...
        final_error = abs(
            final_outlet_conc - self.unified_controller.target_outlet_conc,
        )
        control_stats = self.unified_controller.get_control_statistics()
        ...
        # Save main dashboard
        dashboard_filename = get_figure_path(
            f"mfc_unified_qlearning_dashboard_{timestamp}.png",
        )
...
def main() -> None:
    ...
    np.trapezoid(sim.stack_powers, dx=sim.dt / 3600)
    sim.unified_controller.get_control_statistics()
    abs(sim.outlet_concentrations[-1] - target_outlet)

    # Calculate control performance metrics
    valid_errors = sim.concentration_errors[sim.concentration_errors != 0]
    np.sqrt(np.mean(valid_errors**2)) if len(valid_errors) > 0 else 0
    np.mean(np.abs(valid_errors)) if len(valid_errors) > 0 else 0
