...
class OptimizedUnifiedQController:
    ...
    def __init__(self, target_outlet_conc=12.0) -> None:
    ...
    def discretize_state(
        ...
        substrate_idx = get_bin(
            substrate_utilization,
            self.state_bins["substrate_utilization"],
        )
    ...
    def select_action(self, state, current_flow_rate, current_inlet_conc):
        ...
        if np.random.random() < self.epsilon:
            ...
            if np.random.random() < 0.7:  # 70% chance of reasonable exploration
                ...
                flow_options = [
                    i for i, (f, s) in enumerate(self.actions) if abs(f) <= 4
                ]
                if flow_options:
    ...
    def update_q_table(self, state, action, reward, next_state) -> None:
        ...
        next_max_q = (
            max([self.q_table[next_state][a] for a in range(len(self.actions))])
        ...
        if (
            ...
            and len(self.performance_history) % 100 == 0
        ):
            ...
            if (
                avg_recent_reward > -1200
            ):  # Good performance - faster decay (realistic threshold)
                decay_factor = (
                    self.epsilon_decay * 0.998
                )  # Slightly more aggressive decay
    ...
    def calculate_optimized_reward(
        self,
        power,
        biofilm_deviation,
        substrate_utilization,
        ...
        if power_change > 0:
            power_reward = (
                power_change * self.reward_params["power_increase_multiplier"]
            )
        else:
            power_reward = (
                power_change * self.reward_params["power_decrease_multiplier"]
            )
        ...
        if substrate_change > 0:
            substrate_reward = (
                substrate_change * self.reward_params["substrate_increase_multiplier"]
            )
        else:
            substrate_reward = (
                substrate_change * self.reward_params["substrate_decrease_multiplier"]
            )
        ...
        if biofilm_deviation <= deviation_threshold:
            biofilm_reward = (
                self.reward_params["biofilm_base_reward"]
            ...
            biofilm_reward = -self.reward_params["biofilm_penalty_multiplier"] * (
        ...
        if outlet_error <= 0.5:
            concentration_reward = self.reward_params["conc_precise_reward"] - (
                outlet_error * 10.0
            )
        elif outlet_error <= 2.0:
            concentration_reward = self.reward_params["conc_acceptable_reward"] - (
                outlet_error * 2.5
            )
        else:
            concentration_reward = self.reward_params["conc_poor_penalty"] - (
                outlet_error * 5.0
            )
        ...
        if biofilm_thickness_history is not None and len(biofilm_thickness_history) > 0:
            ...
            if (
                avg_biofilm
                < optimal_thickness * self.reward_params["biofilm_threshold_ratio"]
            ) and current_flow_rate > self.reward_params["flow_penalty_threshold"]:
                flow_penalty = (
                    -self.reward_params["flow_penalty_multiplier"]
                    * (current_flow_rate - self.reward_params["flow_penalty_threshold"])
                    / 10.0
                )
...
class OptimizedMFCSimulation(MFCUnifiedQLearningSimulation):
    ...
    def __init__(self, use_gpu=True, target_outlet_conc=12.0) -> None:
        ...
        self.unified_controller.calculate_unified_reward = (
            self.unified_controller.calculate_optimized_reward
        )
