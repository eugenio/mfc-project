...
class MFCQLearningSimulation:
    def __init__(self, use_gpu=True) -> None:
        ...
        self.r_max = (
            1.0e-5  # Maximum reaction rate (mol/(m²·s)) - for mmol/L concentrations
        )
    ...
    def initialize_arrays(self) -> None:
        ...
        self.acetate_concentrations[0, :] = (
            20.0  # Starting acetate concentration (20 mmol/L = 20 mol/m³)
        )
        self.flow_rates[0] = (
            10.0 / 3.6e9
        )  # Starting flow rate (10 mL/h converted to m³/s)
    ...
    def generate_plots(self, timestamp) -> None:
        ...
        ax9.text(
            ...
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "lightgreen", "alpha": 0.5},
        )
        ...
        if np.any(valid_mask):
            plt.scatter(
                flow_rates_ml[valid_mask],
                self.substrate_utilizations[valid_mask],
        ...
        if np.any(valid_mask):
            ...
            flow_bins = np.linspace(
                np.min(flow_rates_ml[valid_mask]),
                np.max(flow_rates_ml[valid_mask]),
                10,
            )
            if len(flow_bins) > 1:
                bin_centers = []
                ...
                for i in range(len(flow_bins) - 1):
                    mask = (flow_rates_ml >= flow_bins[i]) & (
                        flow_rates_ml < flow_bins[i + 1]
                    )
                    if np.any(mask):
                ...
                if bin_centers:
                    ...
                    plt.title(
                        "Substrate Utilization vs Flow Rate\n(Binned Average ± Std)",
                    )
                    plt.grid(True, alpha=0.3)
