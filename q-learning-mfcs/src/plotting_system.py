...
def plot_mfc_simulation_results(
    data_path: str | Path,
    ...
    add_horizontal_line(
        ax,
        max_threshold,
        f"Max threshold ({max_threshold} mM)",
        color="orange",
        linestyle=":",
    )
    setup_axis(
    ...
    summary_text += (
        f"Reservoir conc.: {df['reservoir_concentration'].iloc[-1]:.1f} mM\n"
    )
    ...
    if "biofilm_thickness_cell_0" in df.columns:
        avg_biofilm = np.mean(
            [df[f"biofilm_thickness_cell_{i}"].iloc[-1] for i in range(5)],
        )
    ...
    if "power_cell_0" in df.columns:
        ...
        ax = axes2[5]
        scatter = ax.scatter(
            df["reservoir_concentration"],
            df["substrate_addition_rate"],
            c=df["time_hours"],
            cmap="viridis",
            alpha=0.5,
            s=10,
    ...
    if duration > 100:
        fig3, axes3, labeler3 = create_labeled_subplots(
            2,
            1,
            figsize=(14, 8),
        ...
        ax.axhspan(
            max_threshold,
            df["reservoir_concentration"].max(),
            alpha=0.1,
            color="red",
            label="Excess zone",
        )
        ...
        ax.plot(
            df["time_hours"],
            ma_conc,
            label=f"{window_size}h MA Concentration",
            color="blue",
            linewidth=2,
        )
        ax2 = ax.twinx()
        ax2.plot(
            df["time_hours"],
            ma_addition,
            label=f"{window_size}h MA Addition Rate",
            color="green",
            linewidth=2,
        )
        ...
        drift_rate = (
            df["reservoir_concentration"].iloc[-1]
            - df["reservoir_concentration"].iloc[0]
        ) / duration
...
def plot_latest_simulation(
    pattern: str = "mfc_recirculation_control_*.csv",
...
def plot_gpu_simulation_results(
    data_dir: str | Path,
    output_prefix: str = "gpu_simulation",
):
    ...
    ax.fill_between(
        ...
        alpha=0.1,
        color="yellow",
        label="±5 mM tolerance",
    )

    # Annotate control failure
    final_conc = performance.get(
    ...
    mean_power = (
        performance.get("mean_power", df["total_power"].mean()) * 1000
    )  # Convert to mW
    ax.annotate(
    ...
    save_figure(
        fig1,
        get_figure_path(f"{output_prefix}_control_failure_analysis_{timestamp}.png"),
    )
    ...
    ax.plot(
        window_centers,
        np.array(effectiveness_2mm) * 100,
        "b-",
        linewidth=2,
        marker="o",
        label="±2 mM",
    ...
    ax.plot(
        window_centers,
        np.array(effectiveness_5mm) * 100,
        "g-",
        linewidth=2,
        marker="s",
        label="±5 mM",
    ...
    save_figure(
        fig2,
        get_figure_path(f"{output_prefix}_detailed_analysis_{timestamp}.png"),
    )
...
if __name__ == "__main__":
    ...
    timestamp = plot_gpu_simulation_results(
        gpu_data_dir,
        output_prefix="gpu_1year_analysis",
    )
