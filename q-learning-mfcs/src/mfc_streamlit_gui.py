        ...
        if nadh_data and isinstance(nadh_data[0], list):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=(
                        nadh_data
                        if isinstance(nadh_data, list)
                        else [0.3] * len(time_data)
                    ),
                    name="Avg NADH Ratio",
                    line={"width": 2},
                ),
        ...
        if cell_voltages_data:
            if isinstance(cell_voltages_data, list):
                cell_voltages = (
                    ...
                    else cell_voltages_data
                    + [0.7] * (n_cells - len(cell_voltages_data))
                )
            else:
        ...
        if current_densities_data:
            if isinstance(current_densities_data, list):
                current_densities = (
                    ...
                    else current_densities_data
                    + [1.0] * (n_cells - len(current_densities_data))
                )
            else:
...
def create_performance_analysis_plots(df):
    ...
    # Coulombic efficiency
    if (
        "coulombic_efficiency" in data_dict
        or "coulombic_efficiency_per_cell" in data_dict
    ):
        ce_key = (
    ...
    if "total_power" in data_dict or "power_density_per_cell" in data_dict:
        power_key = (
            "total_power" if "total_power" in data_dict else "power_density_per_cell"
        )
        power_data = data_dict.get(power_key, [2.0] * len(time_data))
        # If per-cell data, sum or average as appropriate
        if (
            isinstance(power_data, list)
            and power_data
            and isinstance(power_data[0], list)
        ):
            if power_key == "total_power":
                power_data = [
                    sum(timepoint) if timepoint else 2.0 for timepoint in power_data
                ]
            else:
                power_data = [
                    sum(timepoint) / len(timepoint) if timepoint else 2.0
