...
class InterventionRecommendation:
    ...
    intervention_type: (
        str  # 'flow_adjustment', 'substrate_change', 'cleaning', 'monitoring'
    )
...
class BiofimHealthOptimizer:
    ...
    def _initialize_optimal_parameters(self) -> dict[str, float]:
        ...
        if self.species == BacterialSpecies.SHEWANELLA:
            return {
    ...
    def calculate_thickness_health(
        self,
        current_thickness: float,
        thickness_trend: float,
    ) -> float:
    ...
    def calculate_conductivity_health(
        self,
        current_conductivity: float,
        conductivity_trend: float,
    ) -> float:
        ...
        if current_conductivity > 0:
            ...
            health = np.exp(
                -0.5 * (log_ratio / 0.5) ** 2,
            )  # Narrower tolerance for conductivity
        else:
            health = 0.0
    ...
    def calculate_growth_health(self, growth_pattern: BiofimGrowthPattern) -> float:
        ...
        final_score = base_score * growth_pattern.pattern_confidence + 0.5 * (
            1 - growth_pattern.pattern_confidence
        )
    ...
    def assess_risks(
        ...
        fouling_risk = min(
            1.0,
            fouling_risk / 2.0,
        )  # Average of thickness and conductivity factors
...
class PredictiveBiofimHealthMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"Predictive biofilm health monitor initialized for {species.value}",
        )
    ...
    def assess_health(
        ...
        if growth_pattern:
            growth_health = self.health_optimizer.calculate_growth_health(
                growth_pattern,
            )
        ...
        if anomalies:
            anomaly_penalty = min(
                0.3,
                len([a for a in anomalies if a.severity in ["high", "critical"]]) * 0.1,
            )
        ...
        fouling_risk, detachment_risk, stagnation_risk = (
            self.health_optimizer.assess_risks(
                fused_measurement.thickness_um,
                fused_measurement.conductivity_S_per_m,
                growth_rate,
                stability_score,
        ...
        predicted_intervention_time = self._predict_intervention_time(
            overall_health,
            health_trend,
        )
    ...
    def _calculate_measurement_stability(self) -> float:
        ...
        thickness_cv = (
            np.std(thicknesses) / np.mean(thicknesses)
    ...
    def _calculate_health_trend(self) -> HealthTrend:
        ...
        recent_health = [
            h.overall_health_score for h in list(self.health_history)[-10:]
        ]
        ...
        if len(recent_health) >= 3:
            ...
            earlier_avg = (
                np.mean(recent_health[-6:-3]) if len(recent_health) >= 6 else recent_avg
            )
    ...
    def _predict_health_24h(self, current_health: float, trend: HealthTrend) -> float:
        ...
        if trend == HealthTrend.IMPROVING:
            ...
            )  # Volatility generally negative

        return np.clip(predicted_health, 0.0, 1.0)

    def _predict_intervention_time(
        self,
        ...
        if trend == HealthTrend.DECLINING:
            ...
            if len(self.health_history) >= 5:
                recent_scores = [
                    h.overall_health_score for h in list(self.health_history)[-5:]
                ]
    ...
    def generate_alerts(
        ...
        # Health decline alert
            ...
            health_metrics.health_trend == HealthTrend.DECLINING
            and health_metrics.overall_health_score < 0.6
        ):
            alerts.append(
                HealthAlert(
                    timestamp=timestamp,
        ...
        if (
            health_metrics.predicted_intervention_time is not None
            and health_metrics.predicted_intervention_time < 6.0
        ):
            severity = (
                "critical"
                if health_metrics.predicted_intervention_time < 1.0
                else "high"
            )
            alerts.append(
                HealthAlert(
                        ...
                        affected_components=list(
                            {
                                ...
                                for sensor in a.affected_sensors
                            },
                        ),
                        recommended_actions=[
                            a.recommended_action for a in critical_anomalies[:3]
                        ],  # Top 3
    ...
    def generate_intervention_recommendations(
        ...
        if health_metrics.overall_health_score < 0.4:
            recommendations.append(
                InterventionRecommendation(
                    ...
                    urgency=(
                        "immediate"
                        if health_metrics.overall_health_score < 0.3
                        else "within_hours"
                    ),
        ...
        recommendations.sort(
            key=lambda r: (urgency_order.get(r.urgency, 3), -r.success_probability),
        )
    ...
    def get_health_dashboard_data(self) -> dict[str, Any]:
        ...
        # Recent health trend
        recent_scores = [
            h.overall_health_score for h in list(self.health_history)[-20:]
        ]
        ...
        return {
            ...
            "active_alerts": len(
                [
                    ...
                    if a.timestamp > self.last_assessment_time - 1.0
                ],
            ),
...
def create_predictive_health_monitor(
    species: BacterialSpecies = BacterialSpecies.MIXED,
    config: SensorConfig | None = None,
