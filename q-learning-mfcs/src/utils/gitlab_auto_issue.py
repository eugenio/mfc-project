...
class AutoIssueDetector:
    ...
    def analyze_description(self, description: str) -> dict[str, Any]:
        ...
        bug_score = sum(
            1 for keyword in self.bug_keywords if keyword in description_lower
        )
        feature_score = sum(
            1 for keyword in self.feature_keywords if keyword in description_lower
        )
        ...
        if bug_score > feature_score:
            issue_type = "bug"
            confidence = (
                bug_score / (bug_score + feature_score)
                if (bug_score + feature_score) > 0
                else 0
            )
        elif feature_score > bug_score:
            issue_type = "enhancement"
            confidence = (
                feature_score / (bug_score + feature_score)
                if (bug_score + feature_score) > 0
                else 0
            )
        else:
        ...
        suggested_title = (
            title_match.group(1).strip() if title_match else "User reported issue"
        )
    ...
    def create_issue_from_description(
        self,
        description: str,
...
if __name__ == "__main__":
    ...
    for _i, desc in enumerate(test_descriptions, 1):
