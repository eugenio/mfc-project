...
try:
    ...
    warnings.warn(
        "Scikit-learn not available. Some ML features disabled.",
        stacklevel=2,
    )
...
class GaussianProcessSurrogate(SurrogateModel):
    ...
    def __init__(self, kernel=None, alpha=1e-6, n_restarts_optimizer=10) -> None:
        if not SKLEARN_AVAILABLE:
            msg = "Scikit-learn required for Gaussian Process surrogate"
...
class NeuralNetworkSurrogate(SurrogateModel):
    ...
    def __init__(
        ...
        if not TORCH_AVAILABLE:
            msg = "PyTorch required for Neural Network surrogate"
            raise ImportError(msg)
        ...
        for epoch in range(epochs):
            ...
            for batch_X, batch_y in dataloader:
                ...
                uncertainty_loss = torch.mean(
                    torch.exp(-log_var_pred) * (batch_y - mean_pred) ** 2
                    + log_var_pred,
                )
...
class BayesianOptimizer:
    ...
    def _sample_initial_points(self, n_initial: int = 10) -> np.ndarray:
        ...
        return qmc.scale(unit_samples, self.bounds[:, 0], self.bounds[:, 1])
    ...
    def _optimize_acquisition(self, acquisition_func: Callable) -> np.ndarray:
        ...
        for _ in range(n_starts):
            ...
            result = minimize(
                lambda x: -acquisition_func(x.reshape(1, -1))[
                    0
    ...
    def optimize(
        ...
        for _i, x in enumerate(X_initial):
        ...
        for iteration in range(n_iterations):
            ...
            acquisition_func = self.surrogate.get_acquisition_function(
        ...
        best_parameters = dict(
            zip(self.param_names, self.X_observed[best_idx], strict=False),
        )
        ...
        return OptimizationResult(
            ...
            convergence_metrics={
                ...
                "improvement_rate": (
                    (best_so_far - np.mean(y_initial[:5])) / n_iterations
                    if len(y_initial) >= 5
                    else 0
                ),
            },
            model_performance={
                ...
                "success_rate": sum(1 for y in self.y_observed if y > -1e5)
                / len(self.y_observed),
            },
            computational_cost={
...
class MultiObjectiveOptimizer:
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    ...
                    ) and np.any(objective_values[i] < objective_values[j])

                    if dominates:
                        dominated_solutions[i].append(j)
                    elif np.all(objective_values[j] <= objective_values[i]) and np.any(
    ...
    def _calculate_crowding_distance(
        ...
        for obj_idx in range(n_objectives):
            ...
            sorted_indices = sorted(
                range(len(front)),
                key=lambda i: objective_values[front[i], obj_idx],
            )
            ...
            obj_range = (
                objective_values[front[sorted_indices[-1]], obj_idx]
    ...
    def optimize(self, n_generations: int = 100) -> dict[str, Any]:
                ...
                best_fronts_history.append(
                    {
                        ...
                        "pareto_solutions": [
                            dict(zip(self.param_names, population[i], strict=False))
        ...
        return {
            ...
            "pareto_objectives": (
                objective_values[pareto_front].tolist() if len(pareto_front) > 0 else []
            ),
...
def run_optimization_example():
    ...
    cell_geom = CellGeometry(
        ...
        width=0.1,
        height=0.05,
        anode_chamber_volume=0.0002,
        cathode_chamber_volume=0.0002,
    )
    ...
    # Define objectives
    objectives = [
        OptimizationObjective(name="current_density", direction="maximize", weight=1.0),
    ...
    if SKLEARN_AVAILABLE:
        ...
        return optimizer.optimize(n_iterations=20, n_initial=5)
