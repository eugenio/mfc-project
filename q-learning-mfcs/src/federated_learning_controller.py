...
class DifferentialPrivacy:
    ...
    def __init__(self, noise_multiplier: float = 1.0, max_grad_norm: float = 1.0):
        ...
        logger.info(f"Differential privacy initialized: noise={noise_multiplier}, clip={max_grad_norm}")
    ...
    def compute_privacy_budget(self, steps: int, sampling_rate: float,
...
class SecureAggregation:
    ...
    def unmask_aggregate(self, masked_models: List[Dict[str, torch.Tensor]],
        ...
        aggregated = {}
        for name in masked_models[0].keys():
        ...
        final_aggregate = {}
        for name in aggregated.keys():
            final_aggregate[name] = (aggregated[name] - total_masks[name]) / len(masked_models)
...
class ModelCompression:
    ...
    def _quantize_tensor(self, tensor: torch.Tensor) -> np.ndarray:
        ...
        quantized = ((tensor - min_val) * scale).round().clamp(0, 2 ** self.quantization_bits - 1)
...
class FederatedClient:
    ...
    def __init__(self, client_info: ClientInfo, model: nn.Module,
    ...
    def add_local_data(self, system_states: List[SystemState], actions: List[int],
        ...
        for state, action, reward in zip(system_states, actions, rewards):
            ...
            features = self.feature_engineer.extract_features(state, performance_metrics)
    ...
    def local_train(self, global_model_params: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        for epoch in range(self.config.local_epochs):
            ...
            for i in range(0, len(self.local_data), batch_size):
                ...
                try:
                    ...
                    action_targets = F.one_hot(actions, num_classes=logits.size(-1)).float()
        ...
        model_params = {name: param.cpu().clone() for name, param in self.local_model.named_parameters()}
        ...
        with torch.no_grad():
            for data in test_data:
                ...
                try:
                    ...
                    # Calculate loss
                    action_target = F.one_hot(torch.tensor([true_action]),
...
class FederatedServer:
    ...
    def register_client(self, client_info: ClientInfo) -> bool:
        ...
        try:
            ...
            logger.info(f"Client registered: {client_info.client_id} from {client_info.site_name}")
            return True
    ...
    def select_clients(self, round_num: int) -> List[str]:
        ...
        """
        available_clients = [cid for cid, info in self.client_info.items()
        ...
        if self.config.client_selection == ClientSelectionStrategy.RANDOM:
            selected = np.random.choice(available_clients, num_select, replace=False).tolist()
            ...
            for cid in available_clients:
                ...
                score = info.reliability_score * (1.0 - info.local_loss / 10.0)  # Normalize loss
        ...
        logger.info(f"Round {round_num}: Selected {len(selected)} clients out of {len(available_clients)} available")
    ...
    def aggregate_models(self, client_updates: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        elif self.config.aggregation == AggregationMethod.MEDIAN_AGGREGATION:
            return self._median_aggregation(client_updates)
        elif self.config.aggregation == AggregationMethod.TRIMMED_MEAN:
            return self._trimmed_mean_aggregation(client_updates)
        elif self.config.aggregation == AggregationMethod.BYZANTINE_ROBUST:
    ...
    def _weighted_average_aggregation(self, client_updates: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for update in client_updates:
            ...
            if update.get('compression_used', False):
                model_params = self.clients[update['client_id']].compressor.decompress_model(model_params)
    ...
    def run_round(self, round_num: int) -> Dict[str, Any]:
        ...
        global_params = {name: param.cpu().clone()
        ...
        for client_id in selected_clients:
            ...
            try:
                ...
                if 'error' not in update:
                    ...
                    logger.warning(f"Client {client_id} failed training: {update['error']}")
    ...
    def train_federation(self) -> Dict[str, Any]:
        ...
        for round_num in range(1, self.config.num_rounds + 1):
            ...
            if 'error' in round_results:
                logger.warning(f"Round {round_num} had errors: {round_results['error']}")
                continue
            ...
            if len(self.global_metrics['loss']) >= 5:
                ...
                if np.std(recent_losses) < 0.001:  # Convergence threshold
                    logger.info(f"Early stopping at round {round_num} due to convergence")
                    break
        ...
        federation_results = {
            ...
            'final_loss': list(self.global_metrics['loss'])[-1] if self.global_metrics['loss'] else 0.0,
            'average_participation_rate': np.mean(list(self.global_metrics['participation_rate'])),
            'total_communication_cost_mb': sum(self.global_metrics['communication_cost']) / 1024 / 1024,
        ...
        logger.info(f"Federated training completed: {self.current_round} rounds, "
    ...
    def save_federation(self, path: str):
        ...
        save_dict = {
            ...
            'client_info': {cid: info.__dict__ for cid, info in self.client_info.items()},
...
def create_federated_system(global_model: nn.Module,
                          num_clients: int = 5,
    ...
    logger.info(f"Federated system created with {algorithm} algorithm for {num_clients} clients")
...
if __name__ == "__main__":
    ...
    print("Federated server created")
    print(f"Global model parameters: {sum(p.numel() for p in fed_system.global_model.parameters()):,}")
    ...
    for client_id, client in fed_system.clients.items():
        ...
        for _ in range(min(num_samples, 10)):  # Limit for testing
            ...
            print(f"Added dummy training data for {client_id}")
            break  # Skip actual data addition for this test

    print("\n📊 Federation Summary:")
    print(f"Total registered clients: {len(fed_system.clients)}")
    print(f"Algorithm: {fed_system.config.algorithm.value}")
    print(f"Clients per round: {fed_system.config.clients_per_round}")
    print(f"Planned rounds: {fed_system.config.num_rounds}")

    # Test client selection
    selected = fed_system.select_clients(1)
    print(f"Selected clients for round 1: {selected}")

    print("\n✅ Federated Learning System Test Completed!")
    print("\nNote: This test validates the federated learning infrastructure.")
    print("In practice, clients would have real MFC training data and")
    print("the federation would coordinate actual model training across sites.")
