...
class SauerbreyModel:
    ...
    def __init__(
        self,
        crystal_type: CrystalType = CrystalType.AT_CUT_5MHZ,
    ...
    def _load_crystal_properties(self) -> None:
    ...
    def calculate_mass_from_frequency(
        self,
...
class ViscoelasticModel:
    ...
    def calculate_viscoelastic_correction(
        self,
        frequency: float,
        shear_modulus: float,
        viscosity: float,
    ...
    def correct_sauerbrey_mass(
        self,
        sauerbrey_mass: float,
        frequency: float,
        biofilm_properties: dict[str, float],
    ) -> float:
        ...
        # Apply correction
        return sauerbrey_mass * freq_correction
...
class QCMModel:
    ...
    def _initialize_biofilm_properties(self) -> None:
    ...
    def set_biofilm_species(self, species: str) -> None:
    ...
    def simulate_measurement(
        ...
        mass_per_area = biofilm_mass / (
            self.electrode_area * 1e4
        )  # μg/cm² (electrode_area is in m²)
        mass_per_area_ng = mass_per_area * 1000  # Convert to ng/cm²

        # Calculate frequency shift using Sauerbrey equation
        frequency_shift_sauerbrey = self.sauerbrey.calculate_frequency_from_mass(
            mass_per_area_ng,
        )
        ...
        if biofilm_thickness > 5.0:  # μm threshold for viscoelastic effects
            corrected_mass = self.viscoelastic.correct_sauerbrey_mass(
                ...
                self.current_biofilm_props,
            )
            frequency_shift = self.sauerbrey.calculate_frequency_from_mass(
                corrected_mass,
            )
        else:
        ...
        # Temperature effect
        temp_shift = (
            self.temperature_drift * self.fundamental_frequency * (temperature - 298.0)
        )
    ...
    def estimate_biofilm_properties(
        self,
        measurement: QCMMeasurement,
    ) -> dict[str, float]:
        ...
        thickness_estimate = self.sauerbrey.estimate_thickness(
            mass_estimate,
            self.current_biofilm_props["density"],
        )

        # Biomass density estimate (considering porosity)
        porosity = self.current_biofilm_props["porosity"]
        biomass_density = (
            mass_estimate * (1 - porosity) / (thickness_estimate * 1e-4)
        )  # g/L

        # Viscoelastic properties from dissipation
        viscosity_estimate = (
            measurement.dissipation * 0.001 / (2 * np.pi * measurement.frequency)
        )
    ...
    def calibrate_for_biofilm(
        self,
        reference_data: list[tuple[float, float, QCMMeasurement]],
    ) -> None:
        ...
        if len(np.unique(thicknesses)) > 2:
            ...
            estimated_density = (
                density_slope * (self.electrode_area * 1e4) * 1e-7
            )  # Convert units (electrode_area is in m²)
            self.current_biofilm_props["density"] = max(
                0.5,
                min(2.0, estimated_density),
            )
        ...
        if len(np.unique(masses)) > 2:
            ...
            if abs(sensitivity_slope / self.sauerbrey.practical_sensitivity - 1) > 0.2:
                self.sauerbrey.practical_sensitivity = abs(sensitivity_slope) * (
                    self.electrode_area * 1e4
                )  # electrode_area is in m²
    ...
    def get_frequency_stability_metrics(
        self,
