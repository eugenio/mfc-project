...
class KalmanFilter:
    ...
    def __init__(self, dt: float = 0.1, config: SensorConfig | None = None) -> None:
        ...
        self.config = config
        self.n_states = (
            5  # [thickness, biomass, conductivity, d_thickness/dt, d_biomass/dt]
        )
    ...
    def assess_filter_performance(self) -> dict[str, float]:
        ...
        for innovation in recent_innovations:
            try:
                S_inv = np.linalg.inv(
                    np.dot(np.dot(self.H, self.covariance), self.H.T) + self.R,
                )
...
class SensorCalibration:
    ...
    def update_calibration(
        self,
        eis_measurements: list[EISMeasurement],
        ...
        if len(eis_thicknesses) == len(qcm_thicknesses):
            agreements = []
            for eis_thick, qcm_thick in zip(
                eis_thicknesses,
                qcm_thicknesses,
                strict=False,
            ):
                if max(eis_thick, qcm_thick) > 0:
                    agreement = 1.0 - abs(eis_thick - qcm_thick) / max(
                        eis_thick,
                        qcm_thick,
                    )
    ...
    def _update_with_reference(
        ...
        if abs(eis_estimate - reference_thickness) > 5.0:  # μm threshold
            error_ratio = (
                reference_thickness / eis_estimate if eis_estimate > 0 else 1.0
            )
        ...
        if abs(qcm_estimate - reference_thickness) > 2.0:  # μm threshold
            error_ratio = (
                reference_thickness / qcm_estimate if qcm_estimate > 0 else 1.0
            )
        ...
        self.calibration_history.append(
            {
                ...
                "qcm_correction": (
                    error_ratio
                    if abs(qcm_estimate - reference_thickness) > 2.0
                    else 1.0
                ),
            },
        )
    ...
    def _update_reliability(self) -> None:
        ...
        self.eis_reliability = min(
            1.0,
            self.eis_reliability * 0.99 + agreement_factor * 0.01,
        )

        # QCM reliability (affected by crystal aging)
        self.qcm_reliability = min(
            1.0,
            self.qcm_reliability * 0.995 + agreement_factor * 0.005,
        )
    ...
    def get_calibration_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_agreement": (
                np.mean(self.agreement_history[-5:]) if self.agreement_history else 0.5
            ),
            ...
            "needs_calibration": (
                self.eis_reliability < 0.8 or self.qcm_reliability < 0.8
            ),
        }
...
class SensorFusion:
    ...
    def __init__(
        ...
        self.kalman_filter = (
            KalmanFilter(config=self.config)
    ...
    def fuse_measurements(
        ...
        eis_uncertainty = self.calibration.get_measurement_uncertainty(
            "eis",
            eis_thickness,
        )
        qcm_uncertainty = self.calibration.get_measurement_uncertainty(
            "qcm",
            qcm_thickness,
        )
    ...
    def _kalman_fusion(
        ...
        eis_thickness: float,
        qcm_thickness: float,
        eis_conductivity: float,
        eis_uncertainty: float,
        qcm_uncertainty: float,
        time_hours: float,
    ) -> dict[str, float]:
        """Apply Kalman filter fusion."""
        if not self.kalman_filter.initialized:
            # Initialize with first measurements
            biomass_estimate = self._estimate_biomass_from_thickness(
        ...
        uncertainties = np.array(
            [eis_uncertainty, qcm_uncertainty, eis_conductivity * 0.1],
        )
        ...
        eis_weight = (
            qcm_uncertainty / total_thickness_uncertainty
            if total_thickness_uncertainty > 0
            else 0.5
        )
        qcm_weight = (
            eis_uncertainty / total_thickness_uncertainty
            if total_thickness_uncertainty > 0
            else 0.5
        )
    ...
    def _maximum_likelihood_fusion(
        self,
        eis_thickness: float,
        qcm_thickness: float,
        eis_conductivity: float,
        ...
        fused_thickness = (var_qcm * eis_thickness + var_eis * qcm_thickness) / (
            var_eis + var_qcm
        )
    ...
    def _bayesian_fusion(
        ...
        theta_range = np.linspace(
            min(eis_thickness, qcm_thickness)
            - 3 * max(eis_uncertainty, qcm_uncertainty),
            max(eis_thickness, qcm_thickness)
            + 3 * max(eis_uncertainty, qcm_uncertainty),
            1000,
        )
    ...
    def _assess_sensor_status(
        self,
        sensor_type: str,
        measurement: EISMeasurement | QCMMeasurement,
        ...
        if sensor_type == "eis":
            ...
            if quality > 0.8 and reliability > 0.8:
                return "good"
            if quality > 0.5 and reliability > 0.5:
                return "degraded"
            return "failed"
        ...
        if quality > 0.8 and reliability > 0.8 and q_factor > 5000:
            return "good"
        if quality > 0.5 and reliability > 0.5 and q_factor > 1000:
            return "degraded"
        return "failed"
    ...
    def _calculate_agreement(self, value1: float, value2: float) -> float:
        ...
        return max(0.0, 1.0 - relative_diff / 0.5)  # 50% diff gives 0 agreement
    ...
    def get_fusion_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "kalman_filter_performance": (
                self.kalman_filter.assess_filter_performance()
            ...
            "sensor_weights": {
                "recent_eis_weight": (
                    np.mean([w[0] for w in self.sensor_weights_history[-5:]])
                    if self.sensor_weights_history
                    else 0.5
                ),
                "recent_qcm_weight": (
                    np.mean([w[1] for w in self.sensor_weights_history[-5:]])
                    if self.sensor_weights_history
                    else 0.5
                ),
            },
    ...
    def detect_sensor_faults(self) -> dict[str, Any]:
        ...
        faults = {"eis_faults": [], "qcm_faults": [], "fusion_faults": []}
