...
class AdvancedKalmanFilter(KalmanFilter):
    ...
    def _apply_biofilm_dynamics(self):
        ...
        saturation_factor = max(0, saturation_factor)  # Prevent negative growth beyond saturation
    ...
    def _calculate_prediction_accuracy(self) -> float:
        ...
        accuracy = max(0, 1.0 - mape / 10.0)  # Assuming 10μm is terrible error

        return accuracy

    def update_with_prediction_validation(self, measurements: np.ndarray,
    ...
    def _validate_predictions(self, actual_measurements: np.ndarray):
        ...
        for i, prediction in enumerate(self.prediction_history):
            if abs(prediction.prediction_horizon_hours - self.dt) < 0.01:  # One-step prediction
                # Calculate prediction error
                predicted_thickness = prediction.predicted_values[0]
                actual_thickness = np.mean(actual_measurements[:2])  # Average EIS and QCM
    ...
    def _adapt_filter_parameters(self):
        ...
        logger.debug(f"Filter parameters adapted: R_scale={np.mean(np.diag(self.R)):.3f}, "
...
class StatisticalAnomalyDetector:
    ...
    def detect_anomalies(self, current_measurement: FusedMeasurement) -> list[AnomalyDetection]:
        ...
        cross_val_anomalies = self._detect_cross_validation_anomalies(current_measurement)
    ...
    def _detect_spc_anomalies(self, measurement: FusedMeasurement) -> list[AnomalyDetection]:
        ...
        if measurement.thickness_um > self.control_limits['thickness_upper']:
            anomalies.append(AnomalyDetection(
                ...
                anomaly_score=min(1.0, (measurement.thickness_um - self.control_limits['thickness_upper']) /
                                 self.baseline_stats['thickness_std']),
                anomaly_type='measurement_outlier',
                affected_sensors=['eis', 'qcm'],
                severity='medium' if measurement.thickness_um < self.control_limits['thickness_upper'] * 1.5 else 'high',
                confidence=0.9,
            ...
            anomalies.append(AnomalyDetection(
                timestamp=measurement.timestamp,
                anomaly_score=min(1.0, (self.control_limits['thickness_lower'] - measurement.thickness_um) /
                                 self.baseline_stats['thickness_std']),
                anomaly_type='measurement_outlier',
                affected_sensors=['eis', 'qcm'],
                severity='medium',
        ...
        if measurement.sensor_agreement < self.control_limits['agreement_lower']:
            anomalies.append(AnomalyDetection(
                timestamp=measurement.timestamp,
                anomaly_score=(self.control_limits['agreement_lower'] - measurement.sensor_agreement) /
        ...
        if measurement.fusion_confidence < self.control_limits['confidence_lower']:
            anomalies.append(AnomalyDetection(
                timestamp=measurement.timestamp,
                anomaly_score=(self.control_limits['confidence_lower'] - measurement.fusion_confidence) /
    ...
    def _detect_sensor_drift(self) -> list[AnomalyDetection]:
        ...
        if self.scipy_available and len(timestamps) >= 10:
            try:
                # EIS drift detection
                eis_slope, _, eis_r_value, eis_p_value, _ = stats.linregress(timestamps, eis_thicknesses)
                if abs(eis_slope) > 0.5 and eis_p_value < 0.05:  # Significant trend > 0.5 μm/hour
                    anomalies.append(AnomalyDetection(
                ...
                # QCM drift detection
                qcm_slope, _, qcm_r_value, qcm_p_value, _ = stats.linregress(timestamps, qcm_thicknesses)
                if abs(qcm_slope) > 0.3 and qcm_p_value < 0.05:  # Significant trend > 0.3 μm/hour
                    anomalies.append(AnomalyDetection(
                        timestamp=recent_measurements[-1].timestamp,
                ...
                agree_slope, _, agree_r_value, agree_p_value, _ = stats.linregress(timestamps, agreements)
                if abs(agree_slope) > 0.01 and agree_p_value < 0.05:  # Declining agreement
                    anomalies.append(AnomalyDetection(
                        timestamp=recent_measurements[-1].timestamp,
    ...
    def _detect_pattern_changes(self) -> list[AnomalyDetection]:
        ...
        historical_measurements = list(self.fusion_history)[-(historical_window + recent_window):-recent_window]

        # Calculate statistical differences
        recent_thickness_mean = np.mean([m.thickness_um for m in recent_measurements])
        historical_thickness_mean = np.mean([m.thickness_um for m in historical_measurements])
        historical_thickness_std = np.std([m.thickness_um for m in historical_measurements])

        # Detect significant mean shift
        if historical_thickness_std > 0:
            z_score = abs(recent_thickness_mean - historical_thickness_mean) / historical_thickness_std
        ...
        if historical_thickness_std > 0 and recent_thickness_std > 0:
            ...
            if variance_ratio > 2.0 or variance_ratio < 0.5:  # Significant variance change
                anomalies.append(AnomalyDetection(
                    timestamp=recent_measurements[-1].timestamp,
    ...
    def _detect_cross_validation_anomalies(self, measurement: FusedMeasurement) -> list[AnomalyDetection]:
        ...
        if measurement.cross_validation_error > 15.0:  # μm threshold
            severity = 'critical' if measurement.cross_validation_error > 30.0 else 'high'
            anomalies.append(AnomalyDetection(
                timestamp=measurement.timestamp,
        ...
        if len(self.fusion_history) >= 10:
            ...
            eis_bias = np.mean([m.eis_thickness - m.qcm_thickness for m in recent_measurements])
    ...
    def get_anomaly_summary(self) -> dict[str, Any]:
        ...
        recent_anomalies = [a for a in self.anomaly_history if a.timestamp > recent_cutoff]
        ...
        for anomaly in recent_anomalies:
            # Count by type
            anomaly_counts['by_type'][anomaly.anomaly_type] = anomaly_counts['by_type'].get(anomaly.anomaly_type, 0) + 1

            # Count by severity
            anomaly_counts['by_severity'][anomaly.severity] = anomaly_counts['by_severity'].get(anomaly.severity, 0) + 1

            # Count by affected sensors
            for sensor in anomaly.affected_sensors:
                anomaly_counts['by_sensor'][sensor] = anomaly_counts['by_sensor'].get(sensor, 0) + 1
        ...
        health_score = max(0, 1.0 - (critical_count * 0.3 + high_count * 0.2 + medium_count * 0.1))
...
class AdvancedSensorFusion(SensorFusion):
    ...
    def __init__(self, method: FusionMethod = FusionMethod.KALMAN_FILTER,
        ...
        logger.info(f"Advanced sensor fusion initialized with prediction horizon {prediction_horizon} steps")
    ...
    def fuse_measurements_with_prediction(self,
        ...
        if isinstance(self.kalman_filter, AdvancedKalmanFilter) and predict_steps > 0:
            try:
                ...
                self.advanced_metrics['prediction_accuracy'] = prediction.prediction_accuracy
        ...
        if self.anomaly_detector:
            try:
                ...
                self.advanced_metrics['system_health_score'] = anomaly_summary.get('system_health_score', 1.0)
    ...
    def get_system_health_assessment(self) -> dict[str, Any]:
        ...
        health_assessment['component_health']['fusion'] = {
            ...
            'calibration_quality': base_summary['calibration_status']['eis_reliability']
        }
        ...
        if isinstance(self.kalman_filter, AdvancedKalmanFilter):
            ...
            health_assessment['component_health']['kalman_filter'] = {
                'status': 'good' if consistency > 0.8 else 'degraded' if consistency > 0.5 else 'poor',
        ...
        if self.anomaly_detector:
            anomaly_summary = self.anomaly_detector.get_anomaly_summary()
            recent_anomalies = anomaly_summary.get('recent_anomalies', {}).get('total', 0)
        ...
        if health_assessment['overall_health_score'] < 0.7:
            health_assessment['recommendations'].append("System health below optimal - investigate sensor calibration")

        if base_summary['recent_agreement'] < 0.5:
            health_assessment['recommendations'].append("Poor sensor agreement - check for drift or interference")

        if self.advanced_metrics['prediction_accuracy'] < 0.6:
            health_assessment['recommendations'].append("Low prediction accuracy - consider model recalibration")

        # Recent critical anomalies
        recent_critical = [a for a in self.anomaly_history[-10:] if a.severity == 'critical']
        if recent_critical:
            health_assessment['recommendations'].append(f"Critical anomalies detected - {recent_critical[-1].recommended_action}")
    ...
    def _update_advanced_metrics(self):
        ...
        if isinstance(self.kalman_filter, AdvancedKalmanFilter):
            if hasattr(self.kalman_filter, 'adaptation_counter'):
                self.advanced_metrics['adaptive_improvements'] = self.kalman_filter.adaptation_counter
