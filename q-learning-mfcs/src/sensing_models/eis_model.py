...
class EISCircuitModel:
    ...
    def __init__(self, species: BacterialSpecies = BacterialSpecies.MIXED) -> None:
    ...
    def _load_species_parameters(self) -> None:
    ...
    def update_from_biofilm_state(
        self,
        thickness: float,
        biomass_density: float,
        porosity: float = 0.8,
        electrode_area: float = 1e-4,
    ) -> None:
        ...
        thickness_factor = max(
            0.1,
            1.0 - 0.8 * thickness / self.species_params["max_thickness"],
        )
    ...
    def calculate_impedance(self, frequency: float) -> complex:
        ...
        return self.Rs + Zparallel + self.Rct
...
class EISModel:
    ...
    def estimate_thickness(
        self,
        measurements: list[EISMeasurement],
        method: str = "low_frequency",
    ) -> float:
        ...
        if method == "low_frequency":
            ...
            avg_impedance = np.mean(
                [m.impedance_magnitude for m in low_freq_measurements],
            )

            # Linear calibration model
            thickness = (
                self.calibration["thickness_intercept"] - avg_impedance
            ) / self.calibration["thickness_slope"]
            ...
            thickness = (
                self.calibration["thickness_intercept"] - char_impedance
            ) / self.calibration["thickness_slope"]
            ...
            msg = f"Unknown estimation method: {method}"
            raise ValueError(msg)

        # Apply bounds
        return np.clip(thickness, 0.0, self.calibration["max_thickness"])

    def get_biofilm_properties(
        self,
        measurements: list[EISMeasurement],
    ) -> dict[str, float]:
        ...
        conductivity = (
            1.0 / fitted_params["Rbio"] * (thickness * 1e-6) / self.electrode_area
        )
        capacitance_per_area = fitted_params["Cbio"] / (
        ...
        biomass_estimate = max(
            0,
            (2000 - low_freq_impedance) / 100.0,
        )  # g/L (empirical)
    ...
    def calibrate_for_species(
        self,
        reference_measurements: list[tuple[float, list[EISMeasurement]]],
    ) -> None:
        ...
        slope = np.sum(
            (thicknesses - np.mean(thicknesses)) * (impedances - np.mean(impedances)),
