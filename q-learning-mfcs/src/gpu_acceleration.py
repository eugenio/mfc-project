...
class GPUAccelerator:
    ...
    def __init__(self, prefer_backend: str = 'auto', verbose: bool = True):
        """
        ...
        self._detect_backends(verbose)
        self._initialize_backend(prefer_backend, verbose)
    
    def _detect_backends(self, verbose: bool = True):
    ...
    def _initialize_backend(self, prefer_backend: str, verbose: bool = True):
        ...
        self._setup_backend(verbose)
    
    def _setup_backend(self, verbose: bool = True):
        """Setup the selected backend."""
        if self.backend == 'cuda':
            self._setup_cuda(verbose)
        elif self.backend == 'rocm':
            self._setup_rocm(verbose)
        else:
            self._setup_cpu(verbose)
        
        # Always try to initialize JAX for high-performance computing
        self._setup_jax(verbose)
    
    def _setup_cuda(self, verbose: bool = True):
    ...
    def _setup_rocm(self, verbose: bool = True):
    ...
    def _setup_cpu(self, verbose: bool = True):
    ...
    def _setup_jax(self, verbose: bool = True):
    ...
    def jax_array(self, data: Any, dtype=np.float32) -> Union[np.ndarray, Any]:
        ...
        if self.jax is None or self.jnp is None:
            return np.asarray(data, dtype=dtype)
    ...
    def test_gpu_functionality(self, verbose: bool = True) -> bool:
...
def get_gpu_accelerator(prefer_backend: str = 'auto', force_reinit: bool = False, test_functionality: bool = True, verbose: Optional[bool] = None) -> GPUAccelerator:
    """
    ...
    Args:
        ...
        verbose: Whether to print initialization messages (None = verbose only on first init)
    ...
    if _gpu_accelerator is None or force_reinit:
        # First initialization or forced reinit - be verbose unless explicitly silenced
        init_verbose = verbose if verbose is not None else True
        _gpu_accelerator = GPUAccelerator(prefer_backend, verbose=init_verbose)
        
        # Test GPU functionality and fallback to CPU if needed
        if test_functionality and _gpu_accelerator.is_gpu_available():
            _gpu_accelerator.test_gpu_functionality(verbose=init_verbose)
...
def benchmark_backends():
    ...
    for backend in ['cuda', 'rocm']:
        try:
            gpu_acc = GPUAccelerator(prefer_backend=backend, verbose=False)
...
if __name__ == '__main__':
    ...
    # Test basic operations
    backend = gpu_acc.backend or "Unknown"
    print(f"\n🧪 Testing basic operations with {backend.upper()} backend...")
