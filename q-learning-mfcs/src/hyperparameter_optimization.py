...
class SubstrateControlObjective:
    ...
    def __call__(self, config: dict[str, Any]) -> dict[str, float]:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not results:
                    msg = "Simulation returned empty results"
    ...
    def _build_qlearning_config(self, config: dict[str, Any]) -> QLearningConfig:
        ...
        qlearning_config.substrate_penalty_base_multiplier = config[
            "substrate_penalty_base_multiplier"
        ]
    ...
    def _calculate_performance_metrics(
        ...
        substrate_control_loss = (
            ...
            + 10.0
            * np.sum(
                target_deviation > (self.tolerance * 2),
            )  # Heavy penalty for large excursions
        )
        ...
        power_efficiency = min(
            1.0,
            100.0 / max(mean_power, 1.0),
        )  # Reward moderate power
...
def run_bayesian_optimization(
    ...
    for _key, _value in best_config.items():
        pass
...
def apply_optimized_config(best_config: dict[str, Any]) -> QLearningConfig:
    ...
    optimized_config.outlet_penalty_multiplier = best_config[
        "outlet_penalty_multiplier"
    ]
    optimized_config.substrate_penalty_base_multiplier = best_config[
        "substrate_penalty_base_multiplier"
    ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Q-learning Hyperparameter Optimization",
    )
    parser.add_argument(
        "--samples",
        type=int,
