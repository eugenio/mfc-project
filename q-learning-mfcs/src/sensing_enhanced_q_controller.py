...
class SensingEnhancedQLearningController(AdvancedQLearningFlowController):
    ...
    def setup_sensor_enhanced_state_space(self) -> None:
        ...
        if self.enable_sensor_state:
            ...
            self.sensor_agreement_bins = np.linspace(
                0,
                1,
                state_config.sensor_agreement_bins,
            )
            self.fusion_confidence_bins = np.linspace(
                0,
                1,
                state_config.fusion_confidence_bins,
            )
            self.sensor_status_bins = np.array(
                [0, 1, 2, 3],
            )  # good, degraded, failed, unavailable

        else:
            pass

    def discretize_sensor_enhanced_state(
        self,
        base_state: tuple,
        ...
        eis_thick_idx = np.clip(
            np.digitize(eis_thickness, self.eis_thickness_bins) - 1,
            0,
            len(self.eis_thickness_bins) - 2,
        )
        eis_cond_idx = np.clip(
            np.digitize(eis_conductivity, self.eis_conductivity_bins) - 1,
            0,
            len(self.eis_conductivity_bins) - 2,
        )
        eis_qual_idx = np.clip(
            np.digitize(eis_quality, self.eis_quality_bins) - 1,
            0,
            len(self.eis_quality_bins) - 2,
        )
        ...
        qcm_mass_idx = np.clip(
            np.digitize(qcm_mass, self.qcm_mass_bins) - 1,
            0,
            len(self.qcm_mass_bins) - 2,
        )
        qcm_freq_idx = np.clip(
            np.digitize(qcm_freq_shift, self.qcm_frequency_shift_bins) - 1,
            0,
            len(self.qcm_frequency_shift_bins) - 2,
        )
        qcm_diss_idx = np.clip(
            np.digitize(qcm_dissipation, self.qcm_dissipation_bins) - 1,
            0,
            len(self.qcm_dissipation_bins) - 2,
        )
        ...
        agreement_idx = np.clip(
            np.digitize(sensor_agreement, self.sensor_agreement_bins) - 1,
            0,
            len(self.sensor_agreement_bins) - 2,
        )
        confidence_idx = np.clip(
            np.digitize(fusion_confidence, self.fusion_confidence_bins) - 1,
            0,
            len(self.fusion_confidence_bins) - 2,
        )
    ...
    def choose_action_with_sensors(
        self,
        base_state: tuple,
        sensor_data: dict | None = None,
        ...
        # Exploitation: choose best action based on Q-values
            ...
            action_idx = self._choose_best_available_action(
                enhanced_state,
                available_actions,
            )
        else:
            ...
            q_values = [
                self.q_table[enhanced_state][a] for a in range(len(self.actions))
            ]
    ...
    def _choose_best_available_action(
        self,
        state: tuple,
    ...
    def calculate_sensor_enhanced_reward(
        self,
        base_reward: float,
        sensor_data: dict | None = None,
        ...
        biofilm_reward = self._calculate_biofilm_health_reward(
            sensor_data,
            system_state,
        )
    ...
    def _calculate_biofilm_health_reward(
        self,
        sensor_data: dict,
        system_state: dict,
    ) -> float:
        ...
        if eis_data:
            ...
            thickness_deviation = (
                abs(eis_thickness - optimal_thickness) / optimal_thickness
            )
            thickness_reward = max(0, 1.0 - thickness_deviation)

            # Conductivity reward (higher is better for electron transfer)
            conductivity_reward = min(
                1.0,
                eis_conductivity / rewards_config.conductivity_normalization_S_per_m,
            )
            ...
            if len(self.sensor_confidence_history) > 1:
                ...
                growth_reward = max(
                    ...
                    min(
                        1.0,
                        mass_growth_rate
                        * self.qlearning_config.reward_weights.mass_growth_rate_factor,
                    ),
                )
    ...
    def _calculate_sensor_agreement_reward(self, sensor_data: dict) -> float:
        ...
        return sensor_agreement * 0.6 + fusion_confidence * 0.4
    ...
    def _calculate_stability_reward(self, system_state: dict) -> float:
        ...
        return (flow_stability + conc_stability) / 2
    ...
    def update_q_value_with_sensors(
        ...
        if sensor_data:
            ...
            if sensor_confidence > self.min_sensor_confidence:
                enhanced_reward = self.calculate_sensor_enhanced_reward(
                    reward,
                    sensor_data,
                    {"flow_rate": 15.0, "outlet_concentration": 12.0},
                )
        else:
            adaptive_learning_rate = self.learning_rate
        ...
        max_next_q = max(
            [self.q_table[next_state][a] for a in range(len(self.actions))],
        )
    ...
    def handle_sensor_fault(self, fault_type: str, affected_sensor: str) -> None:
        ...
        if fault_type == "failed":
            ...
            self.sensor_degradation_factor = max(
                0.3,
                self.sensor_degradation_factor * 0.8,
            )
            ...
            self.sensor_degradation_factor = max(
                0.6,
                self.sensor_degradation_factor * 0.9,
            )
    ...
    def get_controller_performance_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "decision_statistics": {
                ...
                "sensor_guidance_ratio": (
                    self.sensor_guided_decisions / total_decisions
            ...
            "sensor_confidence": {
                "recent_confidence": (
                    np.mean(self.sensor_confidence_history[-10:])
    ...
    def reset_sensor_tracking(self) -> None:
    ...
    def adapt_to_sensor_availability(
        self,
        eis_available: bool,
        qcm_available: bool,
    ) -> None:
    ...
    def validate_sensor_enhanced_operation(self) -> dict[str, bool]:
        ...
        return {
            ...
            "state_space_extended": (
                len(self.eis_thickness_bins) > 0
                if hasattr(self, "eis_thickness_bins")
                else False
            ),
            "sensor_confidence_tracking": len(self.sensor_confidence_history) > 0,
            "multi_objective_rewards": any(
                v > 0 for v in self.total_reward_components.values()
            ),
