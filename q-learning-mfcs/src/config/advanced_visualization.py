...
try:
    ...
    HAS_PLOTLY = False
    warnings.warn(
        "Plotly not available. Interactive visualization will be limited.",
        stacklevel=2,
    )
...
try:
    ...
    warnings.warn(
        "Advanced statistical packages not available. Some features will be limited.",
        stacklevel=2,
    )
...
class AdvancedVisualizer(ABC):
    ...
    def __init__(self, config: VisualizationConfig) -> None:
        ...
        if HAS_MATPLOTLIB:
            plt.style.use(
                "seaborn-v0_8" if hasattr(plt.style, "library") else "seaborn",
            )
            plt.rcParams.update(
                {
                    "figure.figsize": (
                        config.plot_style.figure_width,
                        config.plot_style.figure_height,
                    ),
    ...
    def _prepare_data(
        ...
        if len(clean_data) == 0:
            msg = "No valid data remaining after cleaning"
            raise ValueError(msg)
    ...
    def _apply_color_scheme(self, n_colors: int = 10) -> list[str]:
        ...
        if self.config.color_scheme_type == "colorblind_friendly":
            return [
                "#1f77b4",
                "#ff7f0e",
                "#2ca02c",
        ...
        return (
            plt.cm.viridis(np.linspace(0, 1, n_colors)) if HAS_MATPLOTLIB else ["blue"]
        )
...
class MultiDimensionalPlotter(AdvancedVisualizer):
    ...
    def create_plot(
        ...
        **kwargs,
        ...
        if plot_config.plot_type == PlotType.SCATTER_3D:
            ...
            msg = f"Unsupported plot type: {plot_config.plot_type}"
            raise ValueError(msg)
    ...
    def _create_3d_scatter(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for 3D plotting"
    ...
    def _create_parallel_coordinates(
        ...
        if HAS_PLOTLY:
            ...
            color = (
                clean_data[class_column]
                if class_column and class_column in clean_data.columns
                else None
            )
            ...
            fig = go.Figure(
                data=go.Parcoords(
                    line=(
                        {"color": color, "colorscale": config.color_map}
        ...
        if not HAS_MATPLOTLIB:
            msg = "Either Plotly or Matplotlib required for parallel coordinates"
            raise ImportError(
                msg,
            )
        ...
        parallel_coordinates(
            clean_data[[*columns, class_column]],
            class_column,
            ax=ax,
            alpha=config.alpha,
        )
    ...
    def _create_radar_chart(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for radar charts"
            raise ImportError(msg)
...
class InteractiveAnalyzer(AdvancedVisualizer):
    ...
    def create_plot(
        ...
        **kwargs,
        ...
        if plot_config.plot_type == PlotType.INTERACTIVE_SCATTER:
            ...
            msg = f"Unsupported interactive plot type: {plot_config.plot_type}"
            raise ValueError(
                msg,
            )
    ...
    def _create_interactive_scatter(
        self,
        data: pd.DataFrame,
        config: PlotConfiguration,
        x_col: str,
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker={
                    ...
                    "colorbar": (
                        {"title": config.color_label or color_col}
                        if color_col
                        else None
                    ),
                },
    ...
    def _create_animated_time_series(
        self,
        data: pd.DataFrame,
        config: PlotConfiguration,
        time_col: str,
    ...
    def _create_static_fallback(
        ...
        **kwargs,
        ...
        if not HAS_MATPLOTLIB:
            msg = "Neither Plotly nor Matplotlib available for visualization"
            raise ImportError(
                msg,
            )
...
class StatisticalVisualizer(AdvancedVisualizer):
    ...
    def create_plot(
        ...
        **kwargs,
        ...
        if plot_config.plot_type == PlotType.CORRELATION_MATRIX:
            ...
            msg = f"Unsupported statistical plot type: {plot_config.plot_type}"
            raise ValueError(
                msg,
            )
    ...
    def _create_correlation_matrix(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for correlation matrix"
            raise ImportError(msg)
        ...
        mask = (
            ...
            if config.show_correlation_values
            else None
        )
    ...
    def _create_distribution_comparison(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for distribution plots"
            raise ImportError(msg)
        ...
        ax2 = axes[0, 1]
        box_data = [
            clean_data[clean_data[group_col] == group][value_col] for group in groups
        ]
    ...
    def _create_uncertainty_bands(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for uncertainty bands"
            raise ImportError(msg)
...
class ComparisonAnalyzer(AdvancedVisualizer):
    ...
    def create_optimization_comparison(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for comparison plots"
            raise ImportError(msg)
        ...
        best_scores = [
            r.best_overall_score for r in results if r.best_overall_score is not None
        ]

        ax2.bar(
            methods[: len(best_scores)],
            best_scores,
            color=colors[: len(best_scores)],
        )
        ...
        if results and results[0].best_parameters is not None:
            ...
            param_data = np.array(
                [r.best_parameters for r in results if r.best_parameters is not None],
            )

            if len(param_data) > 0:
                box_data = [
                    param_data[:, j] for j in range(min(5, n_params))
                ]  # Limit to 5 parameters
                ax4.boxplot(
                    box_data,
                    labels=[f"P{j + 1}" for j in range(len(box_data))],
                )
    ...
    def create_uncertainty_comparison(
        ...
        if not HAS_MATPLOTLIB:
            msg = "Matplotlib required for uncertainty comparison"
            raise ImportError(msg)
        ...
        if results and results[0].output_samples:
            first_output = next(iter(results[0].output_samples.keys()))
...
def create_pareto_frontier_plot(
    ...
    """
    if not HAS_MATPLOTLIB:
        msg = "Matplotlib required for Pareto frontier plots"
        raise ImportError(msg)
    ...
    if not all_objectives:
        msg = "No valid objective data found"
        raise ValueError(msg)
...
def create_sensitivity_tornado_plot(
    ...
    """
    if not HAS_MATPLOTLIB:
        msg = "Matplotlib required for tornado plots"
        raise ImportError(msg)
    ...
    return VisualizationResult(
        ...
        configuration=config,
        data_summary={
            "most_sensitive_param": params[0] if params else None,
