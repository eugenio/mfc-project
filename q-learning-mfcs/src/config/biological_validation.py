...
def validate_kinetic_parameters(
    params: KineticParameters,
    parameter_name: str = "kinetic_parameters",
) -> bool:
    ...
    validate_range(
        ...
        273.0,
        373.0,
        f"{parameter_name}.temperature_ref",
...
def validate_metabolic_reaction(
    reaction: MetabolicReactionConfig,
    reaction_name: str = "metabolic_reaction",
) -> bool:
    ...
    if not reaction.stoichiometry:
        msg = f"{reaction_name}.stoichiometry"
        raise ConfigValidationError(
            msg,
    ...
    for metabolite, coeff in reaction.stoichiometry.items():
        if abs(coeff) > 100:
            msg = f"{reaction_name}.stoichiometry[{metabolite}]"
            raise ConfigValidationError(
                msg,
                coeff,
    ...
    if reaction.flux_lower_bound >= reaction.flux_upper_bound:
        msg = f"{reaction_name}.flux_bounds"
        raise ConfigValidationError(
            msg,
    ...
    validate_range(
        abs(reaction.flux_lower_bound),
        0,
        10000.0,
        f"{reaction_name}.flux_lower_bound",
        "Flux bounds must be reasonable (< 10000 mmol/gDW/h)",
    )
    validate_range(
        reaction.flux_upper_bound,
        0,
        10000.0,
        f"{reaction_name}.flux_upper_bound",
        "Flux bounds must be reasonable (< 10000 mmol/gDW/h)",
    )
...
def validate_species_metabolic_config(
    config: SpeciesMetabolicConfig,
    config_name: str = "species_config",
) -> bool:
    ...
    validate_range(
        config.cytochrome_content,
        0.001,
        1.0,
        f"{config_name}.cytochrome_content",
    ...
    validate_range(
        ...
        0.001,
        2.0,
        f"{config_name}.max_growth_rate",
        "Maximum growth rate must be between 0.001-2.0 h⁻¹",
    )

    validate_range(
        config.maintenance_coefficient,
        0.0,
        0.5,
        f"{config_name}.maintenance_coefficient",
        "Maintenance coefficient must be between 0-0.5 h⁻¹",
    )

    validate_range(
        config.yield_coefficient,
        0.01,
        1.0,
        f"{config_name}.yield_coefficient",
        "Yield coefficient must be between 0.01-1.0 gDW/mmol",
    )

    # Validate biofilm parameters
    validate_range(
        config.attachment_rate,
        0.001,
        1.0,
        f"{config_name}.attachment_rate",
        "Attachment rate must be between 0.001-1.0 h⁻¹",
    )

    validate_range(
        config.detachment_rate,
        0.0001,
        0.5,
        f"{config_name}.detachment_rate",
        "Detachment rate must be between 0.0001-0.5 h⁻¹",
    )

    validate_range(
        config.max_biofilm_thickness,
        1.0,
        1000.0,
        f"{config_name}.max_biofilm_thickness",
    ...
    if temp_min >= temp_max or temp_min < 200 or temp_max > 400:
        msg = f"{config_name}.temperature_range"
        raise ConfigValidationError(
            msg,
    ...
    validate_range(
        config.salinity_tolerance,
        0.0,
        5.0,
        f"{config_name}.salinity_tolerance",
...
def validate_biofilm_kinetics_config(
    config: BiofilmKineticsConfig,
    config_name: str = "biofilm_config",
) -> bool:
    ...
    validate_range(
        config.biofilm_density,
        800.0,
        1500.0,
        f"{config_name}.biofilm_density",
    ...
    validate_range(
        monod["max_growth_rate"],
        0.001,
        2.0,
        f"{config_name}.monod_kinetics.max_growth_rate",
        "Maximum growth rate must be between 0.001-2.0 h⁻¹",
    )

    validate_range(
        monod["half_saturation"],
        0.001,
        100.0,
        f"{config_name}.monod_kinetics.half_saturation",
        "Half-saturation constant must be between 0.001-100 mmol/L",
    )

    validate_range(
        monod["yield_coefficient"],
        0.01,
        1.0,
        f"{config_name}.monod_kinetics.yield_coefficient",
        "Yield coefficient must be between 0.01-1.0 gDW/mmol",
    )
    ...
    validate_range(
        ...
        0.0,
        0.1,
        f"{config_name}.monod_kinetics.decay_rate",
    ...
    validate_range(
        nernst["standard_potential"],
        -1.0,
        1.0,
        f"{config_name}.nernst_monod.standard_potential",
        "Standard potential must be between -1.0 to 1.0 V vs SHE",
    )

    validate_range(
        nernst["electron_transfer_rate"],
        0.001,
        1000.0,
    ...
    validate_range(
        nernst["double_layer_capacitance"],
        1e-8,
        1e-3,
    ...
    validate_range(
        mass_transfer["boundary_layer_thickness"],
        0.001,
        10.0,
    ...
    for param_name in [
        "substrate_diffusivity",
        "oxygen_diffusivity",
        "product_diffusivity",
    ]:
        validate_range(
            mass_transfer[param_name],
            1e-12,
            1e-6,
    ...
    validate_range(
        structure["detachment_shear_stress"],
        0.001,
        10.0,
...
def validate_substrate_kinetics_config(
    config: SubstrateKineticsConfig,
    config_name: str = "substrate_kinetics",
) -> bool:
    ...
    validate_range(
        config.max_uptake_rate,
        0.1,
        200.0,
        f"{config_name}.max_uptake_rate",
        "Maximum uptake rate must be between 0.1-200 mmol/gDW/h",
    )

    validate_range(
        config.half_saturation_constant,
        0.001,
        100.0,
    ...
    if config.substrate_inhibition_constant is not None:
        validate_range(
            config.substrate_inhibition_constant,
            0.1,
            1000.0,
    ...
    validate_range(
        ...
        10.0,
        200.0,
        f"{config_name}.activation_energy",
        "Activation energy must be between 10-200 kJ/mol",
    )

    validate_range(
        config.enthalpy_change,
        -300.0,
        100.0,
        f"{config_name}.enthalpy_change",
        "Enthalpy change must be between -300 to 100 kJ/mol",
    )

    validate_range(
        config.entropy_change,
        -1.0,
        1.0,
        f"{config_name}.entropy_change",
...
def validate_substrate_degradation_pathway(
    pathway: SubstrateDegradationPathway,
    pathway_name: str = "degradation_pathway",
) -> bool:
    ...
    validate_range(
        pathway.substrate_stoichiometry,
        0.1,
        10.0,
    ...
    validate_range(
        pathway.electron_yield,
        1.0,
        50.0,
        f"{pathway_name}.electron_yield",
        "Electron yield must be between 1-50 electrons per substrate",
    )

    validate_range(
        pathway.biomass_yield,
        0.01,
        2.0,
        f"{pathway_name}.biomass_yield",
    ...
    validate_range(
        ...
        3.0,
        11.0,
        f"{pathway_name}.optimal_conditions.ph",
        "Optimal pH must be between 3-11",
    )

    validate_range(
        optimal["ionic_strength"],
        0.001,
        2.0,
...
def validate_comprehensive_substrate_config(
    config: ComprehensiveSubstrateConfig,
    config_name: str = "substrate_config",
) -> bool:
    ...
    validate_range(
        ...
        10.0,
        1000.0,
        f"{config_name}.molecular_weight",
        "Molecular weight must be between 10-1000 g/mol",
    )

    if not config.chemical_formula:
        msg = f"{config_name}.chemical_formula"
        raise ConfigValidationError(
            msg,
            config.chemical_formula,
    ...
    validate_range(
        config.diffusion_coefficient_biofilm,
        1e-8,
        1e-4,
    ...
    for species, kinetics in config.species_kinetics.items():
        validate_substrate_kinetics_config(
            kinetics,
            f"{config_name}.species_kinetics[{species.value}]",
        )
    ...
    for i, pathway in enumerate(config.degradation_pathways):
        validate_substrate_degradation_pathway(
            pathway,
            f"{config_name}.degradation_pathways[{i}]",
        )
    ...
    validate_range(
        config.biodegradability,
        0.0,
        1.0,
        f"{config_name}.biodegradability",
        "Biodegradability must be between 0-1",
    )

    validate_range(
        config.half_life_aerobic,
        0.1,
        10000.0,
        f"{config_name}.half_life_aerobic",
        "Aerobic half-life must be between 0.1-10000 hours",
    )

    validate_range(
        config.half_life_anaerobic,
        0.1,
        50000.0,
        f"{config_name}.half_life_anaerobic",
        "Anaerobic half-life must be between 0.1-50000 hours",
    )

    # Validate analytical properties
    validate_range(
        config.quantification_limit,
        1e-6,
        10.0,
        f"{config_name}.quantification_limit",
...
def validate_electrochemical_config(
    config: ElectrochemicalConfig,
    config_name: str = "electrochemical_config",
) -> bool:
    ...
    for couple, potential in config.standard_potentials.items():
        validate_range(
            ...
            -2.0,
            2.0,
            f"{config_name}.standard_potentials[{couple}]",
...
def validate_all_biological_configs(
    species_config: SpeciesMetabolicConfig,
    biofilm_config: BiofilmKineticsConfig,
    substrate_configs: dict[SubstrateType, ComprehensiveSubstrateConfig],
    electrochemical_config: ElectrochemicalConfig,
) -> bool:
    ...
    for substrate_type, substrate_config in substrate_configs.items():
        validate_comprehensive_substrate_config(
            substrate_config,
            f"substrate_configs[{substrate_type.value}]",
        )

    # Cross-validation checks

    # Check that species has kinetics for configured substrates
    for substrate_type in substrate_configs:
        if (
            species_config.species
            not in substrate_configs[substrate_type].species_kinetics
        ):
            msg = f"substrate_configs[{substrate_type.value}].species_kinetics"
            raise ConfigValidationError(
                msg,
    ...
    if (
        abs(
            biofilm_config.monod_kinetics["max_growth_rate"]
            - species_config.max_growth_rate,
        )
        > 0.5
    ):
        msg = "growth_rate_consistency"
        raise ConfigValidationError(
            msg,
            (
                biofilm_config.monod_kinetics["max_growth_rate"],
