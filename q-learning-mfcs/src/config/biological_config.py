...
class SpeciesMetabolicConfig:
    ...
    electron_transport_efficiency: float = (
        0.85  # Efficiency of electron transport to electrode
    )
...
class SubstrateProperties:
    ...
    uptake_kinetics: dict[BacterialSpecies, KineticParameters] = field(
...
class BiofilmKineticsConfig:
    ...
    # Growth kinetics
    monod_kinetics: dict[str, float] = field(
    ...
    nernst_monod: dict[str, float] = field(
        default_factory=lambda: {
    ...
    mass_transfer: dict[str, float] = field(
        default_factory=lambda: {
    ...
    structure: dict[str, float] = field(
        default_factory=lambda: {
...
class ElectrochemicalConfig:
    ...
    standard_potentials: dict[str, float] = field(
        default_factory=lambda: {
    ...
    # Electrode materials
    electrode_properties: dict[str, dict[str, float]] = field(
        default_factory=lambda: {
            "carbon_cloth": {
    ...
    # Membrane properties
    membrane_properties: dict[str, dict[str, float]] = field(
        default_factory=lambda: {
            "nafion_117": {
                "thickness": 0.175,  # mm
...
def get_default_substrate_properties() -> dict[SubstrateType, SubstrateProperties]:
    ...
    acetate_uptake = {
        ...
        BacterialSpecies.SHEWANELLA_ONEIDENSIS: KineticParameters(
            vmax=8.0,
            km=1.2,
            ea=48.0,  # Less efficient with acetate
            reference=LITERATURE_REFERENCES["marsili_2008"],
        ),
    }
    ...
    lactate_uptake = {
        BacterialSpecies.GEOBACTER_SULFURREDUCENS: KineticParameters(
            vmax=5.0,
            km=2.0,
            ea=52.0,  # Limited lactate utilization
            reference=LITERATURE_REFERENCES["lovley_2003"],
        ),
        BacterialSpecies.SHEWANELLA_ONEIDENSIS: KineticParameters(
            vmax=30.0,
            km=0.3,
            ea=42.0,  # Preferred substrate
            reference=LITERATURE_REFERENCES["marsili_2008"],
        ),
    }
