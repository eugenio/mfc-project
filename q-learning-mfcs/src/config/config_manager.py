...
class BiologicalConfigValidator(ConfigValidator):
    ...
    def validate(self, config: dict[str, Any]) -> bool:
        ...
        if not isinstance(config, dict):
            msg = "Biological config must be a dictionary"
        ...
        if "species_configs" in config:
            for species_name, species_config in config["species_configs"].items():
                if not isinstance(species_config, dict):
                    msg = f"Species config for {species_name} must be a dictionary"
                ...
                for field in required_fields:
                    if field not in species_config:
                        msg = (
                            f"Missing required field {field} in species {species_name}"
                        )
                        raise ConfigurationValidationError(
                            msg,
                        )
        ...
        if "substrate_configs" in config:
            for substrate_name, substrate_config in config["substrate_configs"].items():
                if not isinstance(substrate_config, dict):
                    msg = f"Substrate config for {substrate_name} must be a dictionary"
                    raise ConfigurationValidationError(
                        msg,
                    )
...
class ConfigMerger:
    ...
    def merge_dicts(
        ...
        for key, value in override.items():
            if (
                ...
                result[key] = ConfigMerger.merge_dicts(
                    result[key],
                    value,
                    deep_merge=True,
                )
    ...
    def merge_profiles(
        ...
        override_profile: ConfigProfile,
    ) -> ConfigProfile:
        ...
        if base_profile.biological and override_profile.biological:
            merged.biological = ConfigMerger.merge_dicts(
                base_profile.biological,
                override_profile.biological,
            )
        else:
        ...
        merged.visualization = (
            override_profile.visualization or base_profile.visualization
        )
...
class ConfigManager:
    ...
    def __init__(self, config_directory: str | None = None) -> None:
    ...
    def load_profile_from_file(
        ...
        try:
            ...
            msg = f"Failed to load profile {profile_name}: {e}"
    ...
    def _dict_to_visualization_config(
        self,
    ...
    def save_profile_to_file(
        ...
        config_data = {
            ...
            "visualization": (
                asdict(profile.visualization) if profile.visualization else None
            ),
            "metadata": {
        ...
        try:
            ...
            msg = f"Failed to save profile {profile_name}: {e}"
            raise ConfigurationError(msg)
    ...
    def delete_profile(self, profile_name: str) -> None:
        ...
        if self.current_profile == profile_name:
            msg = f"Cannot delete active profile: {profile_name}"
            raise ConfigurationError(msg)
    ...
    def get_configuration(
        ...
        if config_type == "visualization":
            return profile.visualization
        msg = f"Unknown configuration type: {config_type}"
        raise ConfigurationError(msg)
    ...
    def update_configuration(
        ...
        self.logger.info(
            f"Updated {config_type} configuration in profile {profile_name}",
        )
