...
class UnitConverter:
    ...
    def _define_units(self) -> Dict[str, UnitDefinition]:
        ...
        units = {
            ...
            'A/m²': UnitDefinition('amperes per square meter', 'A/m²', 'current_density', 1.0, 'A/m²'),
            'mA/cm²': UnitDefinition('milliamperes per square centimeter', 'mA/cm²', 'current_density', 10.0, 'A/m²', ['mA/cm2']),
            'A/cm²': UnitDefinition('amperes per square centimeter', 'A/cm²', 'current_density', 10000.0, 'A/m²', ['A/cm2']),
            'μA/cm²': UnitDefinition('microamperes per square centimeter', 'μA/cm²', 'current_density', 0.01, 'A/m²', ['uA/cm2', 'µA/cm²']),

            # Concentration Units
            'mM': UnitDefinition('millimolar', 'mM', 'concentration', 1.0, 'mol/m³', ['mmol/L']),
            'M': UnitDefinition('molar', 'M', 'concentration', 1000.0, 'mol/m³', ['mol/L']),
            'μM': UnitDefinition('micromolar', 'μM', 'concentration', 0.001, 'mol/m³', ['uM', 'µM', 'umol/L']),
            'g/L': UnitDefinition('grams per liter', 'g/L', 'mass_concentration', 1.0, 'g/L', ['g/l']),
            'mg/L': UnitDefinition('milligrams per liter', 'mg/L', 'mass_concentration', 0.001, 'g/L', ['mg/l']),
            'kg/m³': UnitDefinition('kilograms per cubic meter', 'kg/m³', 'mass_concentration', 1.0, 'g/L', ['kg/m3']),
            ...
            'μm': UnitDefinition('micrometers', 'μm', 'length', 1e-6, 'm', ['um', 'µm', 'micron']),
            'nm': UnitDefinition('nanometers', 'nm', 'length', 1e-9, 'm'),

            # Area Units
            'm²': UnitDefinition('square meters', 'm²', 'area', 1.0, 'm²', ['m2', 'sq m']),
            'cm²': UnitDefinition('square centimeters', 'cm²', 'area', 1e-4, 'm²', ['cm2', 'sq cm']),
            'mm²': UnitDefinition('square millimeters', 'mm²', 'area', 1e-6, 'm²', ['mm2', 'sq mm']),

            # Flow Rate Units
            'mL/h': UnitDefinition('milliliters per hour', 'mL/h', 'flow_rate', 2.778e-10, 'm³/s', ['ml/h', 'mL/hr']),
            'mL/min': UnitDefinition('milliliters per minute', 'mL/min', 'flow_rate', 1.667e-8, 'm³/s', ['ml/min']),
            'L/h': UnitDefinition('liters per hour', 'L/h', 'flow_rate', 2.778e-7, 'm³/s', ['l/h', 'L/hr']),
            'L/min': UnitDefinition('liters per minute', 'L/min', 'flow_rate', 1.667e-5, 'm³/s', ['l/min']),
            'm³/s': UnitDefinition('cubic meters per second', 'm³/s', 'flow_rate', 1.0, 'm³/s', ['m3/s']),

            # Voltage Units
            'V': UnitDefinition('volts', 'V', 'voltage', 1.0, 'V', ['volt']),
            'mV': UnitDefinition('millivolts', 'mV', 'voltage', 0.001, 'V', ['millivolt']),
            'V vs SHE': UnitDefinition('volts vs standard hydrogen electrode', 'V vs SHE', 'voltage', 1.0, 'V', ['V SHE']),
            'V vs Ag/AgCl': UnitDefinition('volts vs silver/silver chloride', 'V vs Ag/AgCl', 'voltage', 1.197, 'V', ['V AgAgCl']),
            ...
            # Rate Units
            'h⁻¹': UnitDefinition('per hour', 'h⁻¹', 'rate', 2.778e-4, 's⁻¹', ['1/h', '/h', 'h^-1']),
            's⁻¹': UnitDefinition('per second', 's⁻¹', 'rate', 1.0, 's⁻¹', ['1/s', '/s', 's^-1']),
            'd⁻¹': UnitDefinition('per day', 'd⁻¹', 'rate', 1.157e-5, 's⁻¹', ['1/d', '/d', 'd^-1']),

            # Conductivity Units
            'S/m': UnitDefinition('siemens per meter', 'S/m', 'conductivity', 1.0, 'S/m', ['siemens/m']),
            'mS/cm': UnitDefinition('millisiemens per centimeter', 'mS/cm', 'conductivity', 0.1, 'S/m', ['mS/cm']),
            'μS/cm': UnitDefinition('microsiemens per centimeter', 'μS/cm', 'conductivity', 0.0001, 'S/m', ['uS/cm', 'µS/cm']),

            # Power Density Units
            'W/m²': UnitDefinition('watts per square meter', 'W/m²', 'power_density', 1.0, 'W/m²', ['W/m2']),
            'mW/m²': UnitDefinition('milliwatts per square meter', 'mW/m²', 'power_density', 0.001, 'W/m²', ['mW/m2']),
            'W/cm²': UnitDefinition('watts per square centimeter', 'W/cm²', 'power_density', 10000.0, 'W/m²', ['W/cm2']),

            # Dimensionless
            'dimensionless': UnitDefinition('dimensionless', 'dimensionless', 'dimensionless', 1.0, 'dimensionless', ['']),
            '%': UnitDefinition('percent', '%', 'dimensionless', 0.01, 'dimensionless', ['percent']),
        }
    ...
    def convert(self, value: float, from_unit: str, to_unit: str) -> Optional[float]:
        ...
        converted_value = si_value / to_def.si_conversion_factor
