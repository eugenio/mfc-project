...
class ControlElectronics:
    ...
    def __init__(self, mcu_specs: MCUSpecs, adc_specs: ADCSpecs, 
        ...
        # ADC state
        self.adc_readings = {i: 0.0 for i in range(adc_specs.channels)}
        self.adc_noise_history = []
        
        # DAC state
        self.dac_outputs = {i: 0.0 for i in range(dac_specs.channels)}
        self.dac_settling_time = 0.0
        
        # GPIO state
        self.gpio_states = {i: False for i in range(gpio_specs.pins)}
        self.gpio_directions = {i: "input" for i in range(gpio_specs.pins)}  # input/output
        
        # Communication state
        self.comm_activity = {iface_type: 0.0 for iface_type in self.comm_interfaces.keys()}
        self.comm_error_counts = {iface_type: 0 for iface_type in self.comm_interfaces.keys()}
