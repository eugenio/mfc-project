...
class DynamicSubstrateController:
    def __init__(self, target_outlet_conc=8.0, kp=2.0, ki=0.05, kd=0.1) -> None:
...
class QLearningFlowController:
    def __init__(self, learning_rate=0.1, discount_factor=0.95, epsilon=0.3) -> None:
    ...
    def select_action(self, state, current_flow_rate):
        ...
        new_flow_rate = np.clip(
            current_flow_rate + flow_adjustment,
            0.005,
            0.050,
        )  # L/h bounds
    ...
    def calculate_reward(
        ...
        if substrate_change > 0:
            substrate_reward = (
                substrate_change * 30
            )  # Strong reward for consumption increase
        elif substrate_change < 0:
            substrate_reward = (
                substrate_change * 60
            )  # Strong penalty for consumption decrease
        else:
            substrate_reward = 0
...
class MFCDynamicSubstrateSimulation:
    def __init__(self, use_gpu=True, target_outlet_conc=2.0) -> None:
        ...
        self.optimal_biofilm_thickness = (
            1.3  # Optimal biofilm for max electron transfer
        )
        self.flow_rate_bounds = (
            0.018,
            0.18,
        )  # Flow rate bounds (L/h) - converted from m³/s
        ...
        self.substrate_controller = DynamicSubstrateController(
    ...
    def initialize_arrays(self) -> None:
        ...
        self.inlet_concentrations = array_func(
            self.num_steps,
        )  # Dynamic inlet concentrations
        self.outlet_concentrations = array_func(
            self.num_steps,
        ...
        self.acetate_concentrations[0, :] = (
            20.0  # Starting acetate concentration (mmol/L)
        )
    ...
    def update_cell(self, cell_idx, inlet_concentration, flow_rate, biofilm):
        ...
        if self.use_gpu:
            outlet_concentration = gpu_accelerator.maximum(
                0.001,
        ...
        if self.use_gpu:
            concentration_factor = gpu_accelerator.log(
                1.0 + inlet_concentration / self.K_AC,
            )
            biofilm_voltage_loss = 0.05 * gpu_accelerator.abs(
                biofilm - self.optimal_biofilm_thickness,
            )
            cell_voltage = gpu_accelerator.maximum(
                0.1,
    ...
    def update_biofilm(self, step, dt) -> None:
        ...
        for cell_idx in range(self.num_cells):
            ...
            growth_rate = (
                0.001 * substrate_conc / (0.5 + substrate_conc)
            ...
            if self.use_gpu:
                new_thickness = gpu_accelerator.clip(
    ...
    def simulate_step(self, step) -> None:
        ...
        self.control_errors[step] = (
            self.substrate_controller.target_outlet_conc - avg_outlet_conc
        )

        if self.use_gpu:
            biofilm_deviation = float(
                gpu_accelerator.mean(
                    gpu_accelerator.abs(
        ...
        if step > 1 and step % 60 == 0:
            ...
            if hasattr(self, "prev_state") and hasattr(self, "prev_action"):
                ...
                control_reward = (
                    -control_metrics["rmse"] * 10
                )  # Penalty for large control error
        ...
        control_error = abs(
            avg_outlet_conc - self.substrate_controller.target_outlet_conc,
        )
        control_objective = max(
            0.0,
            1.0 - control_error / self.substrate_controller.target_outlet_conc,
        )
    ...
    def run_simulation(self) -> None:
        ...
        if self.use_gpu:
            ...
            self.acetate_concentrations = gpu_accelerator.to_cpu(
            ...
            self.substrate_consumptions = gpu_accelerator.to_cpu(
            ...
            self.substrate_utilizations = gpu_accelerator.to_cpu(
            ...
            self.inlet_concentrations = gpu_accelerator.to_cpu(
                self.inlet_concentrations,
            )
            self.outlet_concentrations = gpu_accelerator.to_cpu(
    ...
    def save_data(self):
        ...
        csv_filename = get_simulation_data_path(
            f"mfc_dynamic_substrate_{timestamp}.csv",
        )
        ...
        json_filename = get_simulation_data_path(
            f"mfc_dynamic_substrate_{timestamp}.json",
        )
    ...
    def generate_plots(self, timestamp) -> None:
        ...
        plt.axhline(
            y=self.substrate_controller.target_outlet_conc,
            color="red",
            linestyle="--",
            alpha=0.8,
            label=f"Target ({self.substrate_controller.target_outlet_conc:.1f} mmol/L)",
        )
        ...
        ax12.text(
            ...
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "lightcyan", "alpha": 0.8},
        )
        ...
        dashboard_filename = get_figure_path(
            f"mfc_dynamic_substrate_dashboard_{timestamp}.png",
        )
        ...
        if np.any(valid_mask):
            plt.scatter(
                self.control_errors[valid_mask],
                self.inlet_concentrations[valid_mask],
                alpha=0.5,
                s=10,
                c=time_hours[valid_mask],
        ...
        if np.any(valid_flow_mask):
            plt.scatter(
                self.flow_rates[valid_flow_mask] * 1000,
                self.inlet_concentrations[valid_flow_mask],
                alpha=0.5,
                s=10,
                c=time_hours[valid_flow_mask],
                cmap="viridis",
            )
            plt.colorbar(label="Time (hours)")
        ...
        if len(self.control_errors) >= window_size:
            ...
            moving_time = time_hours[
                window_size : len(control_rmse_moving) + window_size
            ]
        ...
        control_filename = get_figure_path(
            f"mfc_substrate_control_analysis_{timestamp}.png",
        )
...
def main() -> None:
    ...
    sim.substrate_controller.get_control_metrics()
