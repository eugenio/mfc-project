...
def create_summary_plots() -> None:
    ...
    power_actual = np.array(
        [0.003, 0.098, 0.158, 0.155, 0.150, 0.145, 0.120, 0.190, 0.120, 0.189, 0.079],
    )
    ...
    # Main title
    fig.suptitle(
        "100-Hour MFC Stack Simulation - Key Performance Results",
        fontsize=18,
        fontweight="bold",
        y=0.95,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        get_figure_path("mfc_100h_summary_performance.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_technical_summary() -> None:
    ...
    for i, (control_type, (min_val, max_val)) in enumerate(
        zip(control_types, action_ranges, strict=False),
    ):
    ...
    ax3.set_title(
        "Learned Control Action Distributions",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for i, (baseline, qlearning) in enumerate(
        zip(baseline_scores, qlearning_scores, strict=False),
    ):
    ...
    plt.tight_layout()
    plt.savefig(
        get_figure_path("mfc_100h_technical_summary.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
