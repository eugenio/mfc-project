...
class EnhancedBiofilmModel(BiofilmKineticsModel):
    ...
    def step_biofilm_dynamics_with_sensors(
        self,
        dt: float,
        anode_potential: float,
        substrate_supply: float,
        time_hours: float = 0.0,
        external_measurements: dict | None = None,
    ) -> dict[str, Any]:
        ...
        if sensor_measurements:
            self.sensor_measurements.append(
                {"time": time_hours, "measurements": sensor_measurements},
            )
    ...
    def _get_sensor_measurements(
        ...
        if self.qcm_model and self.enable_qcm:
            try:
                ...
                electrode_area_m2 = (
                    self.qcm_model.electrode_area
                )  # m² (get from QCM model)
                thickness_m = predicted_thickness * 1e-6  # μm to m
                volume_m3 = electrode_area_m2 * thickness_m
                total_mass_ug = (
                    predicted_biomass * volume_m3 * 1e9
                )  # g/L to μg (1e3 L/m³ * 1e6 μg/g)
                ...
                qcm_properties = self.qcm_model.estimate_biofilm_properties(
                    qcm_measurement,
                )
    ...
    def _apply_sensor_feedback(
        ...
        if eis_data and qcm_data and self.sensor_fusion:
            try:
                ...
                eis_measurement = (
                    eis_data["measurements"][0] if eis_data["measurements"] else None
                )
                ...
                if eis_measurement and qcm_measurement:
                    ...
                    self.biomass_prediction_error = abs(
                        base_state["biomass_density"]
                        - fused_result.biomass_density_g_per_L,
                    )
                    ...
                    validated_state.update(
                        {
                            ...
                            "sensor_confidence": eis_data["properties"].get(
                                "measurement_quality",
                                0.5,
                            ),
                    ...
                    validated_state.update(
                        {
                            ...
                            "sensor_confidence": qcm_data["properties"].get(
                                "measurement_quality",
                                0.5,
                            ),
    ...
    def _update_adaptive_parameters(
        ...
        sensor_thickness = validated_state.get(
    ...
    def _recalibrate_parameters(
        ...
        try:
            ...
            correction_factor = (
                sensor_thickness / model_thickness if model_thickness > 0 else 1.0
            )
    ...
    def _update_confidence_levels(self, validated_state: dict[str, Any]) -> None:
        """Update model and sensor confidence levels."""
        # Update model confidence based on prediction accuracy
        if hasattr(self, "thickness_prediction_error"):
            error_factor = max(
                0.1,
                1.0 - self.thickness_prediction_error / 20.0,
            )  # 20 μm reference
    ...
    def reset_sensor_calibration(self) -> None:
    ...
    def validate_sensor_integration(self) -> dict[str, bool]:
        ...
        if self.eis_model:
            try:
                test_measurements = self.eis_model.simulate_measurement(
                    10.0,
                    5.0,
                    0.8,
                    303.0,
        ...
        if self.qcm_model:
            try:
                test_measurement = self.qcm_model.simulate_measurement(
                    100.0,
                    10.0,
                    303.0,
                    0.0,
        ...
        if (
            self.sensor_fusion
            and validation_results["can_simulate_eis"]
            and validation_results["can_simulate_qcm"]
        ):
            try:
