...
class MFCQLearningSimulationParallel:
    ...
    def update_cell_parallel(self, cell_idx, inlet_concentration, flow_rate, biofilm):
        ...
        if self.use_gpu:
            outlet_concentration = cp.maximum(
                0.001,
        ...
        if self.use_gpu:
            ...
            biofilm_voltage_loss = 0.05 * cp.abs(
                biofilm - self.optimal_biofilm_thickness,
            )
            cell_voltage = cp.maximum(
                0.1,
    ...
    def simulate_step(self, step) -> None:
        ...
        for cell_idx in range(self.num_cells):
            ...
            result = self.update_cell_parallel(
                cell_idx,
                inlet_concentration,
                flow_rate,
                biofilm,
        ...
        substrate_utilization = (
            (inlet_concentration - avg_outlet_conc) / inlet_concentration * 100.0
        )
    ...
    def save_data(self):
        ...
        csv_filename = get_simulation_data_path(
            f"mfc_qlearning_parallel_{timestamp}.csv",
        )
        ...
        json_filename = get_simulation_data_path(
            f"mfc_qlearning_parallel_{timestamp}.json",
        )
    ...
    def generate_plots(self, timestamp) -> None:
        ...
        flow_filename = get_figure_path(
            f"mfc_flow_rate_analysis_parallel_{timestamp}.png",
        )
        ...
        if np.any(valid_mask):
            ...
            if len(flow_bins) > 1:
                ...
                if bin_centers:
                    ...
                    plt.title(
                        "Substrate Utilization vs Flow Rate (PARALLEL)\n(Binned Average ± Std)",
                    )
                    plt.grid(True, alpha=0.3)
        ...
        flow_util_filename = get_figure_path(
            f"mfc_flow_vs_utilization_parallel_{timestamp}.png",
        )
        ...
        dashboard_filename = get_figure_path(
            f"mfc_qlearning_dashboard_parallel_{timestamp}.png",
        )
