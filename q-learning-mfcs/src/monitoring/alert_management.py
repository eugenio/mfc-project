...
class AlertDatabase:
    ...
    def save_threshold(self, threshold: AlertThreshold) -> None:
...
class EmailNotificationService:
    ...
    def send_alert_email(self, recipients: list[str], alert: Alert) -> None:
...
class AlertManager:
    ...
    def __init__(
        ...
        self.email_service = (
            EmailNotificationService(email_config) if email_config else None
        )
    ...
    def set_threshold(self, parameter: str, **kwargs) -> None:
    ...
    def check_value(self, parameter: str, value: float) -> Alert | None:
        ...
        if violated:
            ...
            if severity == "critical":
                ...
                threshold_desc = (
                    f"Below minimum ({threshold.min_value} {threshold.unit})"
                )
            else:
                threshold_desc = (
                    f"Above maximum ({threshold.max_value} {threshold.unit})"
                )
    ...
    def _check_escalation(self, alert: Alert) -> None:
        ...
        self.escalation_tracker[alert.parameter] = [
            (t, a) for t, a in self.escalation_tracker[alert.parameter] if t > cutoff
        ]
        ...
        for rule in self.escalation_rules:
            ...
            recent_alerts = [
                a
                for t, a in self.escalation_tracker[alert.parameter]
                if t > window_start and a.severity == rule.severity
            ]

            if len(recent_alerts) >= rule.threshold_count:
                # Check cooldown
                last_esc = self.last_escalation.get(
                    (alert.parameter, rule.escalation_action),
                )
                if last_esc and (now - last_esc) < timedelta(
    ...
    def _escalate_alert(self, alert: Alert, rule: EscalationRule) -> None:
        """Perform escalation action."""
        logger.warning(
            f"Escalating alert for {alert.parameter}: {rule.escalation_action}",
        )
    ...
    def export_alert_config(self) -> dict[str, Any]:
        ...
        return {
            "thresholds": {
                param: asdict(threshold) for param, threshold in self.thresholds.items()
            },
            "escalation_rules": [
                {
    ...
    def import_alert_config(self, config: dict[str, Any]) -> None:
