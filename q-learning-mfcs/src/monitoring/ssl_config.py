...
class CertificateManager:
    ...
    def check_certificate_validity(self) -> tuple[bool, datetime | None]:
        ...
        try:
            ...
            result = subprocess.run(
                ["openssl", "x509", "-in", self.config.cert_file, "-noout", "-dates"],
                check=False,
                capture_output=True,
                text=True,
                timeout=10,
                ...
                for line in result.stdout.split("\n"):
                    if line.startswith("notAfter="):
                        ...
                        try:
                            ...
                            is_valid = (
                                days_until_expiry > self.config.renewal_days_before
                            )
                            return is_valid, exp_date
                        except ValueError:
                            logger.warning(
                                f"Could not parse certificate date: {date_str}",
                            )
                            return False, None
            ...
            logger.exception(f"Error checking certificate validity: {e}")
            return False, None
    ...
    def generate_self_signed_certificate(self) -> bool:
        ...
        try:
            ...
            key_cmd = ["openssl", "genrsa", "-out", self.config.key_file, "2048"]
            ...
            cert_cmd = [
                ...
                "365",
                "-subj",
                f"/C=US/ST=State/L=City/O=MFC-Research/CN={self.config.domain}",
            ]
            ...
            logger.exception(f"Error generating self-signed certificate: {e}")
            return False
    ...
    def request_letsencrypt_certificate(self) -> bool:
        ...
        try:
            ...
            # Run certbot
            logger.info(
                f"Requesting Let's Encrypt certificate for {self.config.domain}",
            )
            result = subprocess.run(
                certbot_cmd,
                check=False,
            ...
            if result.returncode == 0:
                ...
                self.config.cert_file = (
                    f"/etc/letsencrypt/live/{self.config.domain}/fullchain.pem"
                )
                self.config.key_file = (
                    f"/etc/letsencrypt/live/{self.config.domain}/privkey.pem"
                )
            ...
            logger.exception(f"Error requesting Let's Encrypt certificate: {e}")
            return False
...
"""
            ...
            logger.exception(f"Error setting up auto-renewal: {e}")
            return False
...
class SSLContextManager:
    ...
    def create_ssl_context(self) -> ssl.SSLContext:
        ...
        try:
            ...
            logger.exception(f"Failed to load SSL certificate: {e}")
            raise
    ...
    for env_var, attr_name in env_mapping.items():
        ...
        if env_value is not None:
            ...
            if attr_name in [
                ...
                converted_value: bool | int | str = env_value.lower() in (
                    "true",
                    "1",
                    "yes",
                    "on",
...
def save_ssl_config(config: SSLConfig, config_file: str | None = None) -> bool:
    ...
    try:
        ...
        logger.exception(f"Failed to save config to {config_file}: {e}")
        return False


def initialize_ssl_infrastructure(
    config: SSLConfig | None = None,
) -> tuple[bool, SSLConfig]:
    ...
    if not cert_exists or not cert_valid:
        ...
        if config.use_letsencrypt and config.domain != "localhost":
            ...
            if not success:
                logger.warning(
                    "Let's Encrypt failed, falling back to self-signed certificate",
                )
...
def test_ssl_connection(host: str, port: int, timeout: int = 10) -> bool:
    ...
    try:
        ...
        logger.exception(f"SSL connection test failed for {host}:{port}: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if args.init:
        ...
        config = load_ssl_config()
        ports = [
            config.https_port_api,
            config.https_port_frontend,
            config.wss_port_streaming,
        ]
        for port in ports:
