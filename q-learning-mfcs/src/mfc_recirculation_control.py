...
class AnolytereservoirSystem:
    ...
    def __init__(self, initial_substrate_conc=20.0, volume_liters=1.0) -> None:
    ...
    def add_substrate(self, amount_mmol, dt_hours) -> None:
        ...
        if not self.substrate_halt:
            ...
            new_total_substrate = (
                self.substrate_concentration * self.volume + substrate_added
            )
            self.substrate_concentration = new_total_substrate / self.volume

    def circulate_anolyte(self, flow_rate_ml_h, stack_outlet_conc, dt_hours) -> None:
        ...
        if volume_returned > 0:
            ...
            effective_volume_fraction = volume_returned / (
                self.volume + self.pipe_dead_volume
            )
            ...
            overall_mixing_efficiency = (
                initial_mixing_factor * time_mixing_factor * density_factor
            )
            ...
            self.substrate_concentration = max(
                0.0,
                self.substrate_concentration + concentration_change,
            )
    ...
    def get_sensor_readings(self):
        ...
        return {
            ...
            "current_mixing_efficiency": (
                self.mixing_efficiency_history[-1]["overall_efficiency"]
                if self.mixing_efficiency_history
                else 1.0
            ),
...
class SubstrateConcentrationController:
    ...
    def __init__(self, target_outlet_conc=12.0, target_reservoir_conc=20.0) -> None:
        ...
        self.excess_threshold = (
            22.0  # Reduced from 25.0 mmol/L - halt addition if too high
        )
    ...
    def calculate_substrate_addition(
        self,
        outlet_conc,
        reservoir_conc,
        cell_concentrations,
        ...
        if self.previous_outlet_error is not None:
            outlet_error_derivative = (
                outlet_error - self.previous_outlet_error
            ) / dt_hours
        else:
            outlet_error_derivative = 0.0

        outlet_control = (
            self.kp_outlet * outlet_error
        ...
        if self.previous_reservoir_error is not None:
            reservoir_error_derivative = (
                reservoir_error - self.previous_reservoir_error
            ) / dt_hours
        else:
            reservoir_error_derivative = 0.0

        reservoir_control = (
            self.kp_reservoir * reservoir_error
        ...
        if self.control_mode == "emergency":
            ...
            base_addition_rate = (
                max(outlet_control, reservoir_control) * starvation_factor
            )
            ...
            base_addition_rate = (
                outlet_control + reservoir_control
            ) * starvation_factor
        ...
        if halt_addition:
            ...
            addition_rate = np.clip(
                base_addition_rate,
                self.min_addition_rate,
                self.max_addition_rate,
            )
...
class AdvancedQLearningFlowController:
    ...
    def setup_enhanced_state_action_spaces(self) -> None:
    ...
    def discretize_enhanced_state(
        ...
        power_idx = np.clip(
            np.digitize(power, self.power_bins) - 1,
            0,
            len(self.power_bins) - 2,
        )
        biofilm_idx = np.clip(
            np.digitize(biofilm_deviation, self.biofilm_bins) - 1,
            0,
            len(self.biofilm_bins) - 2,
        )
        substrate_idx = np.clip(
            np.digitize(substrate_utilization, self.substrate_bins) - 1,
            0,
            len(self.substrate_bins) - 2,
        )
        reservoir_idx = np.clip(
            np.digitize(reservoir_conc, self.reservoir_conc_bins) - 1,
            0,
            len(self.reservoir_conc_bins) - 2,
        )
        cell_idx = np.clip(
            np.digitize(min_cell_conc, self.cell_conc_bins) - 1,
            0,
            len(self.cell_conc_bins) - 2,
        )
        outlet_idx = np.clip(
            np.digitize(outlet_error, self.outlet_error_bins) - 1,
            0,
            len(self.outlet_error_bins) - 2,
        )
        time_idx = np.clip(
            np.digitize(time_hours, self.time_bins) - 1,
            0,
            len(self.time_bins) - 2,
        )
    ...
    def calculate_substrate_reward(
        self,
        reservoir_conc,
        cell_concentrations,
        outlet_conc,
        ...
        if reservoir_error < 2.0:  # Within 2 mM of target
            reward += self.config.reward_weights.substrate_target_reward * (
                1.0 - reservoir_error / 2.0
            )
        ...
        if outlet_error < 2.0:  # Within 2 mM of user-configurable threshold
            ...
            base_outlet_reward = (
                self.config.reward_weights.substrate_target_reward
                * proximity_factor
                * self.config.outlet_reward_scaling
            )
        ...
        if inlet_conc is not None:
            ...
            ):  # Outlet approximately equals inlet (< 0.5 mM difference)
                # Apply 15% penalty increase
                if base_outlet_reward > 0:
                    base_outlet_reward *= 1.0 - (
                        self.config.outlet_penalty_multiplier - 1.0
                    )  # Reduce reward
                else:
                    # Apply penalty
                    penalty = (
                        self.config.reward_weights.substrate_target_reward
                        * 0.5
                        * self.config.outlet_penalty_multiplier
                    )
        ...
        if reservoir_conc > self.config.substrate_max_threshold:
            ...
            penalty_multiplier = (
                self.config.substrate_penalty_base_multiplier
                + (excess / self.config.substrate_max_threshold)
                ** self.config.substrate_excess_penalty_exponent
            )
            reward += (
                self.config.reward_weights.substrate_excess_penalty
                * excess
                * penalty_multiplier
            )
        ...
        if min_cell_conc < self.config.substrate_min_threshold:
            ...
            reward += (
                self.config.reward_weights.substrate_starvation_penalty * starvation
            )

        # Penalty for substrate addition (encourages efficiency)
        if substrate_addition > 0:
            reward += (
                self.config.reward_weights.substrate_addition_penalty
                * substrate_addition
            )
    ...
    def update_q_value(self, state, action_idx, reward, next_state) -> None:
        """Update Q-value using Q-learning rule."""
        if next_state is not None:
            max_next_q = max(
                [self.q_table[next_state][a] for a in range(self.total_actions)],
            )
        else:
            max_next_q = 0.0
    ...
    def get_state_hash(self, inlet_conc, outlet_conc, current) -> str:
        ...
        # Create state hash
        return f"{inlet_disc}_{outlet_disc}_{current_disc}"
            ...
            "training_statistics": {
                ...
                "states_explored": len(
                    [
                        ...
                        if any(self.q_table[state].values())
                    ],
                ),
                "total_q_updates": sum(
...
class MFCCellWithMonitoring:
    ...
    def __init__(self, cell_id, initial_biofilm=1.0) -> None:
        ...
        self.biofilm_growth_rate = (
            0.05  # Increased 50x based on Shewanella literature (0.825 h⁻¹ max)
        )
    ...
    def update_concentrations(self, inlet_conc, flow_rate_ml_h, dt_hours):
        ...
        thiele_modulus = (
            thickness_m * (k_max / (D_eff * (Ks + substrate_conc_mol_m3))) ** 0.5
        )
        ...
        if thiele_modulus < 0.3:
            ...
            effectiveness_factor = (3.0 / thiele_modulus) * (
                1.0 / np.tanh(thiele_modulus) - 1.0 / thiele_modulus
            )

        # Biofilm activity considering both thickness and diffusion limitations
        biofilm_activity = (
            self.biofilm_thickness / biofilm_params.diffusion_length_scale
        ) * effectiveness_factor

        # Maximum consumption rate with Monod kinetics
        max_consumption = (
            self.max_reaction_rate * biofilm_activity * inlet_conc * residence_time_h
        ) / (Ks + inlet_conc)

        # Actual consumption limited by substrate availability
        substrate_consumed = min(
            max_consumption,
            inlet_conc * 0.95,
        )  # Max 95% consumption per cell
        ...
        monod_growth_rate = (
            biofilm_params.max_specific_growth_rate * limited_substrate_conc
        ) / (biofilm_params.half_saturation_constant + limited_substrate_conc)
        ...
        max_diffusion_thickness = (
            biofilm_params.diffusion_length_scale * 2.0
        )  # 200 μm max

        new_thickness = self.biofilm_thickness + biofilm_change
        self.biofilm_thickness = np.clip(
            new_thickness,
            biofilm_params.minimum_thickness,
            max_diffusion_thickness,
        )
        ...
        self.consumption_rate_history.append(
            substrate_consumed / dt_hours if dt_hours > 0 else 0,
        )
...
def simulate_mfc_with_recirculation(
    ...
    if checkpoint_path and Path(checkpoint_path).exists():
        q_controller.load_checkpoint(checkpoint_path)
    ...
    for step in range(n_steps):
        ...
        for cell in cells:
            current_conc = cell.update_concentrations(
                current_conc,
                flow_rate_ml_h,
                dt_hours,
            )
        ...
        total_power = sum(
            0.35 * cell.biofilm_thickness * cell.substrate_concentration * 0.002
            for cell in cells
        )  # Acetate-specific potential with enhanced efficiency
        ...
        results["biofilm_thicknesses"].append(
            [cell.biofilm_thickness for cell in cells],
        )
        results["substrate_halt"].append(
            int(addition_rate == 0.0),
        )  # 1 if no substrate added, 0 otherwise

        # Store Q-learning metrics
        current_q_value = (
            q_controller.q_table[state][action_idx]
            if state in q_controller.q_table
            else 0.0
        )
...
def run_mfc_simulation(
    duration_hours,
    output_dir,
    config=None,
    n_cells=5,
    initial_substrate_concentration=25.0,
    user_suffix="",
    verbose=True,
    generate_plots=True,
):
    ...
    results, cells, reservoir, controller, q_controller = (
        simulate_mfc_with_recirculation(duration_hours, config, None)
    )
    ...
    if verbose:
        log_filename = (
            f"{base_name}_{duration_str}_{timestamp}{user_suffix_str}_simulation.log"
        )
...
if __name__ == "__main__":
    ...
    results, cells, reservoir, controller, q_controller = (
        simulate_mfc_with_recirculation(args.duration, config, args.load_checkpoint)
    )
    ...
    data_json_filename = (
        f"{base_name}_{duration_str}{timestamp_str}{user_suffix}_data.json"
    )
    metadata_filename = (
        f"{base_name}_{duration_str}{timestamp_str}{user_suffix}_metadata.json"
    )
    log_filename = (
        f"{base_name}_{duration_str}{timestamp_str}{user_suffix}_simulation.log"
    )
    model_checkpoint_filename = (
        f"{base_name}_{duration_str}{timestamp_str}{user_suffix}_model_checkpoint.json"
    )
    ...
    logger.info(
        f"Configuration: {config.__dict__ if hasattr(config, '__dict__') else 'DEFAULT_CONFIG'}",
    )
        ...
        json.dump(
            {
                ...
                "data_info": {
                    ...
                    "time_range": {
                        ...
                        "time_step": (
                            float(df["time_hours"].iloc[1] - df["time_hours"].iloc[0])
                            if len(df) > 1
                            else 0
                        ),
                    },
    ...
    logger.info(
        f"Checkpoint contains {checkpoint_info['training_statistics']['q_table_size']} states",
    )
    logger.info(
        f"States with learned values: {checkpoint_info['training_statistics']['states_explored']}",
    )
    ...
    metadata = {
        ...
        "performance_summary": {
            ...
            "initial_outlet_concentration": (
                results["outlet_concentration"][0]
                if results["outlet_concentration"]
                else 0
            ),
            "final_outlet_concentration": (
                results["outlet_concentration"][-1]
                if results["outlet_concentration"]
                else 0
            ),
            "total_substrate_added": reservoir.total_substrate_added,
            "average_substrate_addition_rate": np.mean(
                results["substrate_addition_rate"],
            ),
            "initial_power_output": (
                results["total_power"][0] if results["total_power"] else 0
            ),
            "final_power_output": (
                results["total_power"][-1] if results["total_power"] else 0
            ),
            "average_power_output": np.mean(results["total_power"]),
            "initial_biofilm_thicknesses": (
                results["biofilm_thicknesses"][0]
                if results["biofilm_thicknesses"]
                else []
            ),
            "final_biofilm_thicknesses": [cell.biofilm_thickness for cell in cells],
            "average_biofilm_thickness": np.mean(
                [cell.biofilm_thickness for cell in cells],
            ),
        ...
        "model_checkpoint_info": {
            ...
            "states_explored": checkpoint_info["training_statistics"][
                "states_explored"
            ],
            "total_q_updates": checkpoint_info["training_statistics"][
                "total_q_updates"
            ],
            "training_episodes": checkpoint_info["training_statistics"][
                "episode_count"
            ],
        ...
        "controller_statistics": {
            "substrate_halt_percentage": (
                np.mean(results["substrate_halt"]) * 100
                if results["substrate_halt"]
                else 0
            ),
            "control_modes_used": (
                list({d["mode"] for d in controller.control_history})
                if controller.control_history
                else []
            ),
            "average_outlet_error": (
                np.mean([abs(d["outlet_error"]) for d in controller.control_history])
                if controller.control_history
                else 0
            ),
            "average_reservoir_error": (
                np.mean([abs(d["reservoir_error"]) for d in controller.control_history])
                if controller.control_history
                else 0
            ),
        },
        "cell_performance": {
            f"cell_{i}": {
                "initial_substrate_conc": (
                    results[f"substrate_conc_cell_{i}"][0]
                    if results[f"substrate_conc_cell_{i}"]
                    else 0
                ),
                "final_substrate_conc": (
                    results[f"substrate_conc_cell_{i}"][-1]
                    if results[f"substrate_conc_cell_{i}"]
                    else 0
                ),
                "average_substrate_conc": np.mean(results[f"substrate_conc_cell_{i}"]),
                "initial_biofilm": (
                    results[f"biofilm_thickness_cell_{i}"][0]
                    if results[f"biofilm_thickness_cell_{i}"]
                    else 0
                ),
                "final_biofilm": (
                    results[f"biofilm_thickness_cell_{i}"][-1]
                    if results[f"biofilm_thickness_cell_{i}"]
                    else 0
                ),
                "average_power": np.mean(results[f"power_cell_{i}"]),
                "final_power": (
                    results[f"power_cell_{i}"][-1] if results[f"power_cell_{i}"] else 0
                ),
            }
            for i in range(5)
        },
    ...
    if not args.no_plots:
        ...
        plot_timestamp = plot_mfc_simulation_results(
            csv_file,
            output_prefix=str(output_dir / plot_prefix),
        )
    ...
    if not args.no_plots:
        logger.info(
            f"  - Plots: {len(metadata['simulation_info']['files_generated']['plots'])} files",
        )

    logger.info("PERFORMANCE SUMMARY:")
    logger.info(
        f"   Reservoir concentration: {reservoir.substrate_concentration:.3f} mmol/L",
    )
    logger.info(
        f"   Final outlet concentration: {results['outlet_concentration'][-1]:.3f} mmol/L",
    )
    logger.info(f"   Total substrate added: {reservoir.total_substrate_added:.3f} mmol")
    logger.info(
        f"   Average biofilm thickness: {np.mean([cell.biofilm_thickness for cell in cells]):.3f}",
    )
