...
class MixedCultureConfig:
    ...
    def __init__(self, species_ratios: dict[BacterialSpecies, float]) -> None:
        ...
        if abs(total_ratio - 1.0) > 0.01:
            msg = f"Species ratios must sum to 1.0, got {total_ratio}"
    ...
    def _load_species_configurations(self) -> None:
        """Load individual species configurations."""
        for species in self.species_ratios:
            if species == BacterialSpecies.GEOBACTER_SULFURREDUCENS:
                ...
                msg = f"Unsupported species: {species}"
                raise ValueError(msg)

    def _load_substrate_configurations(self) -> None:
    ...
    def _load_biofilm_configuration(self) -> None:
        ...
        geobacter_fraction = self.species_ratios.get(
            BacterialSpecies.GEOBACTER_SULFURREDUCENS,
            0.0,
        )
        shewanella_fraction = self.species_ratios.get(
            BacterialSpecies.SHEWANELLA_ONEIDENSIS,
            0.0,
        )

        # Mixed culture biofilm is typically more porous and conductive
        self.biofilm_config.porosity = (
            0.75 + 0.1 * shewanella_fraction
        )  # Shewanella creates more porous biofilms
        self.biofilm_config.nernst_monod["biofilm_conductivity"] = (
            0.005 * geobacter_fraction
            + 0.003 * shewanella_fraction  # Geobacter more conductive
        )
        ...
        mixed_growth = (
            geobacter_growth * geobacter_fraction
            + shewanella_growth * shewanella_fraction
        )
        self.biofilm_config.monod_kinetics["max_growth_rate"] = mixed_growth

    def get_effective_substrate_kinetics(
        self,
            ...
            if species in substrate_config.species_kinetics:
                ...
                if uptake_contribution > 0:
                    ...
                    if kinetics.substrate_inhibition_constant:
                        weighted_ki += (
                            kinetics.substrate_inhibition_constant * weight * fraction
                        )

        # Create effective kinetics configuration
        return SubstrateKineticsConfig(
    ...
    def calculate_substrate_competition(
        self,
        substrate_concentrations: dict[SubstrateType, float],
    ) -> dict[SubstrateType, dict[BacterialSpecies, float]]:
        ...
        for substrate, concentration in substrate_concentrations.items():
            ...
            for species, fraction in self.species_ratios.items():
                if species in self.substrate_configs[substrate].species_kinetics:
                    kinetics = self.substrate_configs[substrate].species_kinetics[
                        species
                    ]

                    # Base Monod kinetics
                    base_rate = (
                        kinetics.max_uptake_rate
                        * concentration
                    ...
                    for other_substrate, other_conc in substrate_concentrations.items():
                        if other_substrate != substrate and other_conc > 0:
                            ...
                            if other_substrate.value in kinetics.competitive_inhibition:
                                ki = kinetics.competitive_inhibition[
                                    other_substrate.value
                                ]
                                competition_factor *= ki / (ki + other_conc)

                    # Final uptake rate
                    uptake_rates[substrate][species] = (
                        base_rate * competition_factor * fraction
                    )
                else:
    ...
    def calculate_synergy_factor(
        self,
        current_densities: dict[BacterialSpecies, float],
    ) -> float:
        ...
        """
        geobacter_current = current_densities.get(
            BacterialSpecies.GEOBACTER_SULFURREDUCENS,
            0.0,
        )
        shewanella_current = current_densities.get(
            BacterialSpecies.SHEWANELLA_ONEIDENSIS,
            0.0,
        )
        ...
        if geobacter_current > 0 and shewanella_current > 0:
            ...
            synergy_factor = 1.0 + 0.3 * min(
                geobacter_current,
                shewanella_current,
            ) / max(geobacter_current, shewanella_current)
        else:
            synergy_factor = 1.0
    ...
    def get_configuration_summary(self) -> dict[str, any]:
        ...
        return {
            "species_composition": {
                species.value: ratio for species, ratio in self.species_ratios.items()
            },
            "dominant_species": max(
                self.species_ratios,
                key=self.species_ratios.get,
            ).value,
            "biofilm_porosity": self.biofilm_config.porosity,
            "biofilm_conductivity": self.biofilm_config.nernst_monod[
                "biofilm_conductivity"
            ],
            "mixed_growth_rate": self.biofilm_config.monod_kinetics["max_growth_rate"],
            "supported_substrates": [
                substrate.value for substrate in self.substrate_configs
            ],
        }
...
def demonstrate_substrate_competition() -> None:
    ...
    for scenario in scenarios:
        ...
        for species in [
            ...
            species.value.split("_")[0].capitalize()
            uptake_rates[SubstrateType.ACETATE][species]
            uptake_rates[SubstrateType.LACTATE][species]
            uptake_rates[SubstrateType.PYRUVATE][species]


def analyze_culture_compositions() -> None:
    ...
    for comp in compositions:
        ...
        mixed_config.get_configuration_summary()


def validate_mixed_configurations() -> bool | None:
    ...
    try:
        ...
        for _i, config in enumerate(compositions):
            # Validate individual species configs
            for species_config in config.species_configs.values():
