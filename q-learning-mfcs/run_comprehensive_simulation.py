...
class SimulationMonitor:
    ...
    def start_monitoring(self) -> None:
        ...
        self.log_message(
            "Configuration: Mixed species, 20mM lactate, 10M CFU/L, recirculation mode, EIS+QCM sensors",
        )
    ...
    def update_progress(
        ...
        if time.time() - self.last_checkpoint > self.checkpoint_interval:
            progress = {
                ...
                "estimated_remaining": (
                    ((time.time() - (self.start_time or 0)) / current_hour)
                    * (total_hours - current_hour)
                    if current_hour > 0 and self.start_time
                    else 0
                ),
...
def run_comprehensive_simulation():
    ...
    try:
        ...
        monitor.log_message(
            f"GPU acceleration: {'Enabled' if model.gpu_available else 'Disabled'}",
        )
        ...
        for biofilm_model in model.biofilm_models:
            biofilm_model.initial_biomass_density = (
                initial_cfu_per_ml * 1e-6
        ...
        monitor.log_message(
            "Initial conditions set: 10M CFU/L, 20mM lactate, recirculation mode",
        )
        ...
        for hour in range(int(model.simulation_hours / dt)):
            ...
            current_substrate = np.mean(
                [
                    metabolic_model.metabolites["lactate"]
                    for metabolic_model in model.metabolic_models
                ],
            )
            ...
            if current_substrate < substrate_threshold:
                ...
                monitor.log_message(
                    f"Substrate addition at hour {hour}: {substrate_addition:.2f} mM (current: {current_substrate:.2f} mM)",
                )
            ...
            sensor_info = {
                ...
                "eis_sensors_active": sum(
                    1 for s in model.sensor_states if s.get("eis_status") == "good"
                ),
                "qcm_sensors_active": sum(
                    1 for s in model.sensor_states if s.get("qcm_status") == "good"
                ),
                "avg_fusion_confidence": (
                    sum(s.get("fusion_confidence", 0) for s in model.sensor_states)
                    / len(model.sensor_states)
                    if model.sensor_states
                    else 0
                ),
            }
        ...
        try:
            ...
            plot_data = {
                ...
                "coulombic_efficiency_series": np.array(
                    time_series.get("coulombic_efficiency", []),
                ),
                "total_energy": results.get("total_energy", 0),
                "average_power": results.get("average_power", 0),
                "peak_power": results.get("peak_power", 0),
                "coulombic_efficiency": results.get("average_coulombic_efficiency", 0)
                * 100,
            ...
            if model.history:
                plot_data["cell_voltages"] = np.mean(
                    [state.cell_voltages for state in model.history],
                    axis=1,
                )
                plot_data["cell_currents"] = np.mean(
                    [state.current_densities for state in model.history],
                    axis=1,
                )
                plot_data["substrate_concentrations"] = np.mean(
                    [state.substrate_concentration for state in model.history],
                    axis=1,
                )

            # Add substrate concentration from time series if available
            if "substrate_concentration" in time_series:
                plot_data["substrate_concentrations"] = np.array(
                    time_series.get("substrate_concentration", []),
                )
        ...
        try:
            ...
            safe_pickle_data = {
                ...
                "simulation_info": {
                    ...
                    "sensors_enabled": getattr(model, "enable_eis", False)
                    or getattr(model, "enable_qcm", False),
                },
            }
        ...
        monitor.log_message(
            f"Average CE: {results.get('average_coulombic_efficiency', 0):.2%}",
        )
        monitor.log_message(
            f"Final Biofilm Thickness: {results.get('final_biofilm_thickness', 0):.1f} μm",
        )
        ...
        monitor.log_message(f"Simulation failed with error: {e!s}")
        import traceback
