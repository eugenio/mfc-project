...
pid_data = pd.read_csv(
    "data/simulation_data/sensor_mfc_simulation_data_20250725_153257.csv",
)
qlearning_old = pd.read_csv(
    "data/simulation_data/mfc_recirculation_control_100h_validated_20250725_160622.csv",
)
qlearning_improved = pd.read_csv(
    "data/simulation_data/mfc_recirculation_control_improved_epsilon_20250725_161813.csv",
)

# Create comprehensive comparison plot
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
fig.suptitle(
    "Q-Learning Substrate Control: Problem Analysis and Solution",
    fontsize=16,
    fontweight="bold",
)
...
ax1.plot(
    pid_data["time_hours"],
    pid_data["substrate_concentrations"],
...
ax1.plot(
    qlearning_old["time_hours"],
    qlearning_old["reservoir_concentration"],
...
ax1.plot(
    qlearning_improved["time_hours"],
    qlearning_improved["reservoir_concentration"],
    label="Q-Learning (Improved)",
    color="blue",
    linewidth=2,
    alpha=0.8,
)
...
ax2.plot(
    qlearning_old["time_hours"],
    qlearning_old["total_power"] * 1000,
    label="Q-Learning (Original)",
    color="orange",
    linewidth=2,
    alpha=0.8,
...
ax2.plot(
    qlearning_improved["time_hours"],
    qlearning_improved["total_power"] * 1000,
    label="Q-Learning (Improved)",
    color="blue",
    linewidth=2,
    alpha=0.8,
...
pid_additions = np.diff(
    pid_data["substrate_concentrations"],
    prepend=pid_data["substrate_concentrations"].iloc[0],
)
...
ax3.plot(
    pid_data["time_hours"],
    pid_additions,
    label="PID Controller (estimated)",
    color="red",
    linewidth=2,
    alpha=0.8,
)
ax3.plot(
    qlearning_old["time_hours"],
    qlearning_old["substrate_addition_rate"],
    label="Q-Learning (Original)",
    color="orange",
    linewidth=2,
    alpha=0.8,
...
ax3.plot(
    qlearning_improved["time_hours"],
    qlearning_improved["substrate_addition_rate"],
    label="Q-Learning (Improved)",
    color="blue",
    linewidth=2,
    alpha=0.8,
...
ax4 = axes[1, 0]
critical_hours = (qlearning_old["time_hours"] >= 45) & (
    qlearning_old["time_hours"] <= 65
)
ax4.plot(
    qlearning_old[critical_hours]["time_hours"],
    qlearning_old[critical_hours]["reservoir_concentration"],
    label="Original Q-Learning",
    color="orange",
    linewidth=2,
)
ax4.plot(
...
old_exploration = (
    pd.Series(old_additions.astype(int)).rolling(window=window, center=True).sum()
)
improved_exploration = (
    pd.Series(improved_additions.astype(int)).rolling(window=window, center=True).sum()
)

ax5.plot(
    qlearning_old["time_hours"],
    old_exploration,
    label="Original (High Exploration)",
    color="orange",
    linewidth=2,
)
ax5.plot(
    qlearning_improved["time_hours"],
    improved_exploration,
    label="Improved (Low Exploration)",
    color="blue",
    linewidth=2,
)
...
plt.tight_layout()
plt.savefig(
    "data/figures/qlearning_substrate_control_analysis.png",
    dpi=300,
    bbox_inches="tight",
)
...
fig2.suptitle(
    "Q-Learning Substrate Control: Final Solution Performance",
    fontsize=16,
    fontweight="bold",
)
...
ax1.plot(
    qlearning_improved["time_hours"],
    qlearning_improved["reservoir_concentration"],
    label="Reservoir Concentration",
    color="blue",
    linewidth=2,
)
ax1.plot(
    qlearning_improved["time_hours"],
    qlearning_improved["outlet_concentration"],
