...
class GitLabIssueAnalyzer:
    ...
    def analyze_issue_urgency(self, issue: dict[str, Any]) -> IssueUrgency:
        ...
        if age_days <= 7 and issue["user_notes_count"] > 3:
            return IssueUrgency.URGENT
        if age_days <= 30 and issue["user_notes_count"] > 1:
            return IssueUrgency.HIGH
        if age_days <= 90:
            return IssueUrgency.MEDIUM
        return IssueUrgency.LOW
    ...
    def analyze_issue_type(self, issue: dict[str, Any]) -> IssueType:
        ...
        if any(
            ...
            for word in ["error", "fail", "broken", "crash", "bug", "issue", "problem"]
        ):
            return IssueType.BUG

        if any(
            word in title or word in description
            for word in ["performance", "slow", "optimize", "speed", "memory", "cpu"]
        ):
        ...
        if any(
            word in title or word in description
            for word in ["documentation", "docs", "readme", "guide", "tutorial"]
        ):
            return IssueType.DOCUMENTATION

        if any(
            word in title or word in description
            for word in ["test", "testing", "unittest", "pytest", "coverage"]
        ):
    ...
    def generate_priority_report(self, analyzed_issues: list[dict[str, Any]]) -> str:
        ...
        for i, issue in enumerate(top_issues, 1):
            report.append(
                f"#{i} [Score: {issue['priority_score']}] Issue #{issue['iid']}",
            )
            report.append(f"    Title: {issue['title']}")
            report.append(
                f"    Type: {issue['analyzed_type']} | "
        ...
        critical_issues = [
            i for i in analyzed_issues if i["analyzed_severity"] == "critical"
        ]
        if critical_issues:
        ...
        if critical_issues:
            report.append(
                f"1. 🚨 Address {len(critical_issues)} critical issues immediately",
            )

        high_severity = [i for i in analyzed_issues if i["analyzed_severity"] == "high"]
        if high_severity:
            report.append(
                f"2. ⚡ Focus on {len(high_severity)} high-severity issues next",
            )

        urgent_issues = [
            i for i in analyzed_issues if i["analyzed_urgency"] == "urgent"
        ]
        if urgent_issues:
            report.append(
                f"3. ⏱️  Handle {len(urgent_issues)} urgent issues within this week",
            )
        ...
        security_issues = [
            i for i in analyzed_issues if i["analyzed_type"] == "security"
        ]
        if security_issues:
            report.append(
                f"5. 🔒 Review {len(security_issues)} security issues immediately",
            )
...
def main():
    ...
    try:
        ...
        analyzer.generate_priority_report(analyzed_issues)

        # Save detailed results to file
        output_file = (
            f"gitlab_issues_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
