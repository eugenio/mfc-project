    ...
    fn _get_self_ptr(py_self: PythonObject) -> UnsafePointer[Self]:
        try:
            ...
            return abort[UnsafePointer[Self]](
                String(
                    "Python method receiver object did not have the expected type: ",
                    e,
                )
            )

    @staticmethod
    fn mfc_odes(py_self: PythonObject, t: PythonObject, y: PythonObject, i_fc: PythonObject) raises -> PythonObject:
        """
        ...
        var r1: Float64 = self_ptr[].k1_0 * math.exp((self_ptr[].alpha * self_ptr[].F) / (self_ptr[].R * self_ptr[].T) * eta_a) * (C_AC / (self_ptr[].K_AC + C_AC)) * X 
        var r2: Float64 = -self_ptr[].k2_0 * (C_O2 / (self_ptr[].K_O2 + C_O2)) * math.exp((self_ptr[].beta - 1.0) * self_ptr[].F / (self_ptr[].R * self_ptr[].T) * eta_c)
        var N_M: Float64 = (3600.0 * Float64(i_fc)) / self_ptr[].F

        var dC_AC_dt: Float64 = (self_ptr[].Q_a * (self_ptr[].C_AC_in - C_AC) - self_ptr[].A_m * r1) / self_ptr[].V_a
        var dC_CO2_dt: Float64 = (self_ptr[].Q_a * (self_ptr[].C_CO2_in - C_CO2) + 2.0 * self_ptr[].A_m * r1) / self_ptr[].V_a
        var dC_H_dt: Float64 = (self_ptr[].Q_a * (self_ptr[].C_H_in - C_H) + 8.0 * self_ptr[].A_m * r1) / self_ptr[].V_a
        var dX_dt: Float64 = (self_ptr[].Q_a * (self_ptr[].X_in - X) / self_ptr[].f_x + self_ptr[].A_m * self_ptr[].Y_ac * r1) / self_ptr[].V_a - self_ptr[].K_dec * X

        var dC_O2_dt: Float64 = (self_ptr[].Q_c * (self_ptr[].C_O2_in - C_O2) + r2 * self_ptr[].A_m) / self_ptr[].V_c
        var dC_OH_dt: Float64 = (self_ptr[].Q_c * (self_ptr[].C_OH_in - C_OH) - 4.0 * r2 * self_ptr[].A_m) / self_ptr[].V_c
        var dC_M_dt: Float64 = (self_ptr[].Q_c * (self_ptr[].C_M_in - C_M) + N_M * self_ptr[].A_m) / self_ptr[].V_c
        
        var d_eta_a_dt: Float64 = (3600.0 * Float64(i_fc) - 8.0 * self_ptr[].F * r1) / self_ptr[].C_a
        var d_eta_c_dt: Float64 = (-3600.0 * Float64(i_fc) - 4.0 * self_ptr[].F * r2) / self_ptr[].C_c
        
        var derivatives: List[Float64] = [dC_AC_dt, dC_CO2_dt, dC_H_dt, dX_dt, dC_O2_dt, dC_OH_dt, dC_M_dt, d_eta_a_dt, d_eta_c_dt  ]
