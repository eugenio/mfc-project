...
try:
    from test_advanced_electrode_model import (
        TestAdvancedElectrodeModel,
        TestBiofilmDynamics,
        TestCellGeometry,
        TestFluidDynamicsProperties,
        TestIntegrationTests,
        TestMassTransportProperties,
    )
    advanced_electrode_tests_available = True
except ImportError as e:
    print(f"Advanced electrode model tests not available: {e}")
    advanced_electrode_tests_available = False
...
try:
    from test_biological_constraints import (
        TestBiologicalConstraints,
        TestChemicalConstraints,
        TestPhysicalConstraints,
        TestThermodynamicConstraints,
    )
...
def create_test_suite(test_category="all"):
    ...
    if advanced_electrode_tests_available and test_category in ["all", "core", "advanced_electrode"]:
...
def run_tests(verbosity=2, test_category="all"):
    ...
    if test_category in ["all", "enhanced", "advanced_electrode"]:
        print("📊 Test Module Availability:")
        print(f"  Advanced Electrode: {'✅' if advanced_electrode_tests_available else '❌'}")
...
def run_specific_test_class(test_class_name, verbosity=2):
    ...
    # Add advanced electrode test classes if available
    if advanced_electrode_tests_available:
        test_classes.update({
...
if __name__ == "__main__":
    ...
    if args.list_tests:
        ...
        if advanced_electrode_tests_available:
            print("\nAdvanced Electrode tests:")
            electrode_tests = ["cell_geometry", "fluid_dynamics", "mass_transport", "biofilm_dynamics", "advanced_electrode", "integration_tests"]
            for test in electrode_tests:
                print(f"  {test}")
