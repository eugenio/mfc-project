...
def _chromium_installed() -> bool:
    ...
    for cdir in chromium_dirs:
        executables = (
            list(cdir.glob("**/chrome"))
            + list(cdir.glob("**/chromium"))
            + list(cdir.glob("**/chrome-headless-shell"))
        )
        if executables:
            return True
    return False
...
_BROWSERS_AVAILABLE = _chromium_installed()


_PLAYWRIGHT_DIR = str(Path(__file__).resolve().parent)
...
def pytest_collection_modifyitems(
    ...
    """Skip playwright tests if browsers are not installed."""
    ...
    for item in items:
        if str(Path(item.fspath).resolve()).startswith(_PLAYWRIGHT_DIR):
...
def pytest_sessionfinish(
    ...
    xfailed = sum(1 for r in results if r["outcome"] == "skipped")
    total = len(results)
    ...
    if failed > 0:
        ...
        for r in results:
            if r["outcome"] == "failed":
                ...
                lines.append("```")
                lines.append(
                    r["longrepr"][:500] if r["longrepr"] else "No details",
                )
                lines.append("```")
                lines.append("")
    ...
    for r in results:
        ...
        lines.append(
            f"| {r['nodeid'].split('::')[-1]} | {icon} | {r['duration']}s |",
        )
