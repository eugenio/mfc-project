...
> **Note**: The Claude Code hooks system (including the edit threshold hook documented here) has been extracted to a separate repository. This documentation is preserved for historical reference. The `.claude/hooks/` directory contains infrastructure tooling that is maintained independently from the MFC application codebase.

## Summary

This document describes a pre-tool hook that monitors file editing operations and manages commits when large changes are detected. This system helps maintain clean git history and encourages incremental development.
...
**Note**: This documentation is preserved for historical reference. The hooks system is now maintained in a separate repository.
