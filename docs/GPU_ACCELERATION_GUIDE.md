...
1. AMD ROCm (if available) - Good performance with PyTorch
1. CPU fallback - Baseline NumPy performance
...
1. **Minimize Transfers**: Keep data on GPU throughout computation
1. **Check Availability**: Always verify GPU availability before assuming acceleration
1. **Error Handling**: The module automatically falls back to CPU on GPU errors
...
1. **CUDA version mismatch**: Match CuPy version to CUDA toolkit
1. **Out of memory**: Reduce batch sizes or use CPU fallback

### AMD ROCm Issues

1. **PyTorch not detecting GPU**: Install ROCm-compatible PyTorch
1. **HIP errors**: Update ROCm drivers
1. **Performance issues**: Ensure GPU is in compute mode
...
1. **Mixed Precision**: FP16/BF16 support for newer GPUs
1. **Custom Kernels**: Optimized kernels for MFC-specific operations
1. **Profiling Tools**: Built-in performance profiling
