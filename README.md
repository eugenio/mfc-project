...
**Q-learning** is a model-free reinforcement learning algorithm that learns the optimal action-selection policy for finite Markov decision processes.

### Key Concepts

- **Q-function**: Maps state-action pairs to expected cumulative rewards
...
2. Selects action (ε-greedy or other policy)
3. Receives reward and observes next state
4. Updates Q-table using learning rate α

### MFC Application

In this project, Q-learning is applied to optimize MFC control by:
- Learning optimal PWM duty cycles for different system states
- Balancing power output with system stability
- Preventing cell reversals through intelligent action selection
- Adapting to changing biofilm and substrate conditions
